...
source_url='https://github.com/iamyusen1314/noteai.git'
oci_source='https://github.com/iamyusen1314/noteai'
task_root='/var/lib/noteai/admin-5335-current-v3'
...
inspect_container='noteai-admin-item20-inspect-v3'
phase='preflight'
...
source_fetch_failure_is_retryable() {
  ...
  [ "$fetch_rc" = '128' ] || return 1
  [ -f "$stdout_path" ] && [ ! -L "$stdout_path" ] && [ ! -s "$stdout_path" ] || return 1
  [ -f "$stderr_path" ] && [ ! -L "$stderr_path" ] || return 1
  cmp -s \
    <(sed $'s/\r$//' "$stderr_path") \
    <(printf '%s\n' \
...
prepare_source_clean_room() {
  local task_dir="$1"
  local source_dir="$2"
  local init_git="$3"

  [ -n "$task_dir" ] && [ "$task_dir" != '/' ] || return 90
  [ "$source_dir" = "$task_dir/source" ] || return 90
  [ -d "$task_dir" ] && [ ! -L "$task_dir" ] || return 90
  case "$task_dir" in
    /var/lib/noteai/admin-5335-current-v3) ;;
    "${TMPDIR:-/tmp}"/noteai-admin-stage-a-v3.*/fetch-*) ;;
    *) return 90 ;;
  esac
  [ ! -L "$source_dir" ] || return 90
  [ -x "$init_git" ] || return 90
  rm -rf -- "$source_dir" || return
  install -d -m 0700 "$source_dir" || return
  "$init_git" -C "$source_dir" init -q || return
  "$init_git" -C "$source_dir" remote add origin "$source_url" || return
}

run_source_fetch_transport() {
  local timeout_bin="$1"
  local fetch_git="$2"
  local source_dir="$3"
  local stdout_path="$4"
  ...
  [ -x "$timeout_bin" ] && [ -x "$fetch_git" ] || return 90
  (
    cd "$source_dir" || exit 90
    "$timeout_bin" --foreground --signal=TERM --kill-after=15s 300s \
      env LC_ALL=C GIT_HTTP_MAX_REQUESTS=1 \
      "$fetch_git" \
...
fetch_source_with_retry() {
  ...
  local fetch_log_root="$task_dir/source-fetch-logs"
  local attempt
  local fetch_rc
  local stdout_path
  local stderr_path
  ...
  [ ! -e "$fetch_log_root" ] || return 90
  install -d -m 0700 "$fetch_log_root" || return
  source_fetch_attempts=0

  for attempt in 1 2; do
    source_fetch_attempts="$attempt"
    stdout_path="$fetch_log_root/attempt-${attempt}.stdout"
    stderr_path="$fetch_log_root/attempt-${attempt}.stderr"
    prepare_source_clean_room "$task_dir" "$source_dir" "$init_git" || return
    if run_source_fetch_transport \
      "$timeout_bin" "$fetch_git" "$source_dir" "$stdout_path" "$stderr_path"; then
      fetch_rc=0
    else
      fetch_rc=$?
    fi
    if [ "$fetch_rc" = '0' ]; then
      if [ -s "$stdout_path" ] || [ -s "$stderr_path" ]; then
        ...
        rm -rf -- "$fetch_log_root" || return
        return 0
      fi
    fi
    if [ "$attempt" = '1' ] && \
      source_fetch_failure_is_retryable "$fetch_rc" "$stdout_path" "$stderr_path"; then
      sleep "$delay_seconds" || return
      continue
    fi
    if [ -s "$stdout_path" ]; then
      printf 'NOTEAI_SOURCE_FETCH_STDOUT attempt=%s\n' "$attempt"
      tail -n 20 "$stdout_path" || true
    fi
    if [ -s "$stderr_path" ]; then
      printf 'NOTEAI_SOURCE_FETCH_STDERR attempt=%s\n' "$attempt" >&2
...
offline_self_test() (
  ...
  local repo_root="$1"
  local fixture_root
  fixture_root="$(mktemp -d "${TMPDIR:-/tmp}/noteai-admin-stage-a-v3.XXXXXX")"
  case "$fixture_root" in
    "${TMPDIR:-/tmp}"/noteai-admin-stage-a-v3.*) ;;
    *) exit 95 ;;
  esac
  ...
  if docker_auth_entry_count "$fixture_root/invalid-docker-config.json" >/dev/null 2>&1; then
    exit 1
  fi

  model_fixture="$fixture_root/model-fixture"
  while IFS= read -r relative_path; do
    mkdir -p "$(dirname "$model_fixture/$relative_path")"
    cp "$repo_root/$relative_path" "$model_fixture/$relative_path"
  done < <(
    jq -er '.artifacts[].path' \
      "$repo_root/model/artifacts/model_release_manifest.v04.json"
  )
  cp "$repo_root/model/artifacts/model_release_manifest.v04.json" \
    "$model_fixture/model/artifacts/model_release_manifest.v04.json"
  verify_model_artifacts "$model_fixture"
  printf '\n' >> "$model_fixture/model/artifacts/model_v04_composite_train_report.json"
  if verify_model_artifacts "$model_fixture" >/dev/null 2>&1; then
    exit 1
  fi

  cp "$repo_root/scripts/ci/native_release_evidence.sh" "$fixture_root/admin-native-release.sh"
...
STALE_TRIVY_METADATA
  ...
  local fixture_real_git
  local fixture_stub_root="$fixture_root/source-fetch-stubs"
  local fixture_timeout="$fixture_stub_root/timeout"
  local fixture_fetch_git="$fixture_stub_root/git"
  fixture_real_git="$(command -v git)"
  [ -x "$fixture_real_git" ]
  install -d -m 0700 "$fixture_stub_root"

  cat > "$fixture_timeout" <<'TIMEOUT_STUB'
#!/bin/bash
set -euo pipefail
[ "$#" -ge 5 ]
[ "$1" = '--foreground' ]
...
TIMEOUT_STUB
  ...
  cat > "$fixture_fetch_git" <<'GIT_STUB'
#!/bin/bash
set -euo pipefail
state_root="${NOTEAI_FIXTURE_STATE_ROOT:?}"
source_root="${NOTEAI_FIXTURE_SOURCE_ROOT:?}"
scenario="${NOTEAI_FIXTURE_SCENARIO:?}"
[ "$(pwd -P)" = "$source_root" ]
count=0
if [ -f "$state_root/fetch-count" ]; then
  read -r count < "$state_root/fetch-count"
fi
count=$((count + 1))
printf '%s\n' "$count" > "$state_root/fetch-count"
[ "$count" -le 2 ] || exit 99
{
  printf 'LC_ALL=%s\0' "${LC_ALL-}"
  printf 'GIT_HTTP_MAX_REQUESTS=%s\0' "${GIT_HTTP_MAX_REQUESTS-}"
  printf 'PWD=%s\0' "$(pwd -P)"
  printf '%s\0' "$@"
} > "$state_root/argv.$count"
[ "$#" = '10' ]
[ "$1" = '-c' ]
[ "$2" = 'http.version=HTTP/1.1' ]
[ "$3" = '-c' ]
[ "$4" = 'http.maxRequests=1' ]
[ "$5" = 'fetch' ]
[ "$6" = '--quiet' ]
[ "$7" = '--no-tags' ]
[ "$8" = '--depth=1' ]
[ "$9" = 'origin' ]
[ "${10}" = "${NOTEAI_FIXTURE_RELEASE:?}" ]
if [ "$count" = '2' ]; then
  [ ! -e "$source_root/attempt1.sentinel" ]
  [ ! -e "$source_root/.git/FETCH_HEAD" ]
  [ ! -e "$source_root/.git/objects/partial" ]
  printf 'pass\n' > "$state_root/clean-room-second"
fi
write_partial_state() {
  touch \
    "$source_root/attempt1.sentinel" \
    "$source_root/.git/FETCH_HEAD" \
    "$source_root/.git/objects/partial"
}
write_exact_transient() {
  printf '%s\n' \
    'error: RPC failed; curl 52 Empty reply from server' \
    "fatal: expected 'packfile'" >&2
}
case "$scenario" in
  success)
    exit 0
  ...
  transient_twice)
    ...
    ;;
  missing_line)
    printf '%s\n' 'error: RPC failed; curl 52 Empty reply from server' >&2
    exit 128
    ;;
  extra_line)
    write_exact_transient
    printf '%s\n' 'unexpected third line' >&2
    exit 128
    ;;
  stdout_with_transient)
    printf '%s\n' 'unexpected stdout'
    write_exact_transient
    exit 128
    ;;
  embedded_cr)
    printf 'error: RPC failed; curl 52 Empt\ry reply\n%s\n' \
      "fatal: expected 'packfile'" >&2
    exit 128
    ;;
  curl56)
    printf '%s\n' 'error: RPC failed; curl 56 Recv failure' "fatal: expected 'packfile'" >&2
    exit 128
    ;;
  timeout124)
    write_exact_transient
    ...
    ;;
  tls)
    printf '%s\n' 'fatal: unable to access repository: SSL certificate problem' >&2
    exit 128
    ;;
  auth)
    ...
    ;;
  repository)
    printf '%s\n' 'remote: Repository not found.' 'fatal: repository not found' >&2
    exit 128
    ;;
  ref)
    printf '%s\n' 'fatal: could not find remote ref release' >&2
    exit 128
    ;;
  permission)
    printf '%s\n' 'fatal: cannot create temporary file: Permission denied' >&2
    exit 128
    ;;
  disk)
    printf '%s\n' 'fatal: write error: No space left on device' >&2
    exit 128
    ;;
  *)
    exit 98
...
GIT_STUB
  chmod 0500 "$fixture_timeout" "$fixture_fetch_git"

  run_source_fetch_fixture() {
    local scenario="$1"
    local expected_rc="$2"
    local expected_attempts="$3"
    local case_root="$fixture_root/fetch-$scenario"
    local state_root="$case_root/state"
    local actual_rc
    local actual_attempts
    install -d -m 0700 "$case_root/task" "$state_root"
    task_root="$case_root/task"
    source_root="$task_root/source"
    export NOTEAI_FIXTURE_STATE_ROOT="$state_root"
    export NOTEAI_FIXTURE_SOURCE_ROOT="$(cd "$task_root" && pwd -P)/source"
    export NOTEAI_FIXTURE_SCENARIO="$scenario"
    export NOTEAI_FIXTURE_RELEASE="$release"
    if fetch_source_with_retry \
      "$task_root" "$source_root" "$fixture_real_git" \
      "$fixture_timeout" "$fixture_fetch_git" 0 \
      >/dev/null 2>&1; then
      actual_rc=0
    else
      actual_rc=$?
    ...
    actual_attempts="$(cat "$state_root/fetch-count")"
    [ "$actual_attempts" = "$expected_attempts" ]
    if [ "$expected_attempts" = '2' ]; then
      [ "$(cat "$state_root/clean-room-second")" = 'pass' ]
      cmp "$state_root/argv.1" "$state_root/argv.2"
    else
      [ ! -e "$state_root/argv.2" ]
    fi
    if [ "$expected_rc" = '0' ]; then
      [ ! -e "$task_root/source-fetch-logs" ]
    else
      [ -d "$task_root/source-fetch-logs" ]
    fi
  }

  run_source_fetch_fixture success 0 1
  run_source_fetch_fixture transient_then_success 0 2
  run_source_fetch_fixture transient_crlf_then_success 0 2
  run_source_fetch_fixture transient_twice 128 2
  run_source_fetch_fixture missing_line 128 1
  run_source_fetch_fixture extra_line 128 1
  run_source_fetch_fixture stdout_with_transient 128 1
  run_source_fetch_fixture embedded_cr 128 1
  run_source_fetch_fixture curl56 128 1
  run_source_fetch_fixture timeout124 124 1
  run_source_fetch_fixture tls 128 1
  run_source_fetch_fixture auth 128 1
  run_source_fetch_fixture repository 128 1
  run_source_fetch_fixture ref 128 1
  run_source_fetch_fixture permission 128 1
  run_source_fetch_fixture disk 128 1
...
fetch_source_with_retry \
  "$task_root" "$source_root" "$real_git" "$real_timeout" "$real_git" 2
...
model_source='/var/lib/noteai/b55-release-build/src'
...
timeout --foreground --signal=TERM --kill-after=30s 1200s   env DOCKER_CONFIG="$task_root/docker-empty"   "$task_root/tools/trivy-real" image     --cache-dir "$task_root/trivy-cache"     --db-repository ghcr.io/aquasecurity/trivy-db:2     --download-db-only --no-progress     >"$task_root/scanner-db.log" 2>&1
...
trap - EXIT
printf 'NOTEAI_ADMIN_STAGE_A=PASS invocation=3 release=%s tree=%s image_id=%s local_tag=%s evidence_files=11 vuln_rows=23 secrets=0 browser=0 source_fetch_attempts=%s trivy_db_sha256=%s trivy_metadata_sha256=%s\n'   "$release" "$release_tree" "$image_id" "$local_image" "$source_fetch_attempts" "$trivy_db_sha" "$trivy_metadata_sha"
