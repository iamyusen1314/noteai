...
task_root='/var/lib/noteai/admin-5335-current-v2'
...
inspect_container='noteai-admin-item20-inspect-v2'
phase='preflight'
...
offline_self_test() (
  ...
  local repo_root="$1"
  local fixture_root
  fixture_root="$(mktemp -d "${TMPDIR:-/tmp}/noteai-admin-stage-a-v2.XXXXXX")"
  case "$fixture_root" in
    "${TMPDIR:-/tmp}"/noteai-admin-stage-a-v2.*) ;;
    *) exit 95 ;;
  esac
...
case "$task_root" in
  /var/lib/noteai/admin-5335-current-v2) ;;
...
install -d -o root -g root -m 0700 "$task_root" "$source_root" "$evidence_root"
...
timeout --foreground --signal=TERM --kill-after=15s 300s   git -C "$source_root" fetch --quiet --no-tags --depth=1 origin "$release"
...
trap - EXIT
printf 'NOTEAI_ADMIN_STAGE_A=PASS invocation=2 release=%s tree=%s image_id=%s local_tag=%s evidence_files=11 vuln_rows=23 secrets=0 browser=0 trivy_db_sha256=%s trivy_metadata_sha256=%s\n'   "$release" "$release_tree" "$image_id" "$local_image" "$trivy_db_sha" "$trivy_metadata_sha"
