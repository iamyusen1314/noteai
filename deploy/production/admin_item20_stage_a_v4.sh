...
task_root='/var/lib/noteai/admin-5335-current-v4'
...
inspect_container='noteai-admin-item20-inspect-v4'
phase='preflight'
...
offline_self_test() (
  ...
  fixture_root="$(mktemp -d "${TMPDIR:-/tmp}/noteai-admin-stage-a-v4.XXXXXX")"
  case "$fixture_root" in
    "${TMPDIR:-/tmp}"/noteai-admin-stage-a-v4.*) ;;
    *) exit 95 ;;
  esac
...
[ "$(readlink -f "$(dirname "$source_bundle_path")")" = \
  '/root/noteai-admin-stage-a-v4-transfer' ] || {
...
case "$task_root" in
  /var/lib/noteai/admin-5335-current-v4) ;;
...
trap - EXIT
printf 'NOTEAI_ADMIN_STAGE_A=PASS invocation=4 release=%s tree=%s image_id=%s local_tag=%s evidence_files=11 vuln_rows=23 secrets=0 browser=0 source_bundle_sha256=%s trivy_db_sha256=%s trivy_metadata_sha256=%s\n'   "$release" "$release_tree" "$image_id" "$local_image" "$source_bundle_sha256" "$trivy_db_sha" "$trivy_metadata_sha"
