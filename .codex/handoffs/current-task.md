...
| Browserless build source / current HEAD | `33d04d4560f9b7dda43a53527f8ae1026dc335d3` (`feat(prod): add bounded XHS HTTP runtime [skip render]`) |
| RAP-fix release candidate | the full `[skip render]` application commit containing the independently verified `PROD-XHS-RAP-FIX-001` implementation, tests and this Handoff; resolve and record its exact 40-character SHA after commit creation |
| Next production build source | the RAP-fix release candidate above; never reuse `33d04d4` as an accepted release, and never use `f3a1a3b`, `93b03d5`, a Handoff-only descendant or a relabelled old image |
...
- **Rollback:** revert the single release commit and keep every old local image unused. No registry, deployment or production rollback is required because none was touched.
- **Next gate:** `PROD-IMG-XHS-BROWSERLESS-REBUILD-001` requires separate approval. Recheck the retained isolated ECS, then build and scan all three browserless native AMD64 roles from a clean detached checkout of the new exact RAP-fix commit; repeat provenance/content/SBOM/Secret/unsuppressed vulnerability checks and `network=none` basic/RAP wrapper smokes. Stop before ACR or deployment.
...
Application release `33d04d4560f9b7dda43a53527f8ae1026dc335d3` was already pushed normally and remains the exact source of the three rejected local browserless images. `PROD-XHS-RAP-FIX-001` is independently verified in the full application commit containing this section. That new commit must use `[skip render]` and push normally without force; resolve its exact SHA from Git after creation. It becomes the only permissible source of the separately approved successor image rebuild, but the commit itself must not trigger image build, ACR access, deployment, database access, cloud change or real provider call.
