...
- 2026-07-18 发布证据：当前远端分支已推进到 `ecc4702`，包含 `199bf5f`、`0ccf3f6`、`0a05a98` 与独立验证记录。push 与 pull_request 两条 GitHub CI 均在同一 SHA 完整通过（LFS制品拉取、Python/Shell编译、578项单测、Quality Gate、Production Readiness 48/48、Docker Compose）。Render Staging 切换时 API 曾短暂返回502，随后 API/Admin/Web/Gateway 均恢复HTTP200；API readiness为PostgreSQL + V0.4 composite，并出现本次发布新增的 `access_status/access_error_code`，Admin readiness与Claude Gateway多实例readiness均通过。由于尚无不泄露凭据的公开migration版本接口，也未人为制造小红书服务端登出，`CAP-001A1/A2A`与`BUG-002G`仍保持 READY_TO_VERIFY，待Staging PostgreSQL结构和自然/合成安全停采Smoke闭环后再标VERIFIED。
- ACR发布状态：产品负责人已批准并完成华南1（深圳）企业版经济版实例 `noteai-prod-shenzhen` 的首月付款（`¥117.00`、1个月、自动续费关闭）；命名空间 `noteai` 与私有仓库 `noteai/app` 已建立，匿名公开拉取保持关闭，除 `latest` 外的镜像版本不可覆盖。PrivateZone已开通，ACR专有网络已绑定生产VPC和 `noteai-prod-app-c` 交换机，私网入口 `10.42.10.45` 状态为“已生效”。主控从干净detached HEAD `ecc4702e7cd7c0d6b6a0cc8db8500a8734aa5d6e` 构建 `linux/amd64` 镜像，本地与容器内模型制品均4/4通过，独立镜像Verification Agent对clean HEAD、架构、非root、启动/healthcheck、制品与敏感信息边界全部判定PASS。首次临时公网推送失败的根因已确认：本机代理分流使通用出口IP与访问中国区ACR时的直连出口IP不同，最初 `/32` 白名单加入了错误的代理出口；TLS在鉴权前EOF，不是密码或镜像问题。旧条目删除后，仅临时加入真实直连出口 `/32`，`/v2/`返回预期401，随后用户在本机Terminal完成不回显凭据的登录。镜像已成功推送为 `noteai-prod-shenzhen-registry.cn-shenzhen.cr.aliyuncs.com/noteai/app:git-ecc4702e`；远端OCI manifest可读取，独立pull确认 `linux/amd64`、完整revision一致，远端不可变digest为 `sha256:b7a7d47dba6a314fae2fde6d1d35cafe9f209da41f385c266f463a31f66f6916`。推送完成后临时白名单已删除，ACR公网入口已关闭并确认临时条目不存在，本机已 `docker logout`，临时Colima已停止；私网入口继续有效。该证据仅闭环“生产镜像发布到私有ACR”，不代表ECS应用已经部署。经济版镜像拉取QPS 30只影响部署拉取，不等于AI请求并发；默认OSS仍可能产生少量按量存储/流量费。
...
- 尚未完成：生产应用部署、ALB/健康后端、证书绑定、业务 DNS 切流、Admin 入口保护、生产前端落点、备份/PITR恢复演练。ACR安全推送、远端不可变摘要、PrivateZone/ACR私网接入及可追溯生产镜像已经闭环，不再列为未完成。
...
### ARCH-002P-F — 独立生产 Gateway 身份、凭据与阿里云切流门禁

- 状态：**IMPLEMENTING**
- 优先级：Critical（阿里云生产主API启用Claude远程传输的直接前置门禁）。
- 问题描述：现有Render Singapore Gateway及其HMAC、OIDC、DynamoDB控制面均为Staging身份，不能把Staging服务改名、复用或直接写入阿里云生产配置；必须建立一套独立生产服务和可审计的切流契约，并在两个真实实例验证后才交付生产URL。
- 证据：新增`render.gateway.production.yaml`定义独立服务`noteai-prod-claude-gateway`，Singapore Starter、min2/max4、CPU60%、memory70%、shutdown240、Auto Deploy关闭、`/health/ready`；生产HMAC、Anthropic Key、DynamoDB表、principal和AWS Role均为`sync:false`独立凭据。生产Blueprint明确禁止业务数据库、Moonshot Key、静态AWS Key、模型文件、Cron和磁盘。新增语义测试已通过Gateway packaging 6/6、Gateway全量78/78、Production Readiness48/48、py_compile及diff-check；真实Render生产服务、生产OIDC角色和生产控制表尚未创建，因此当前不得交付URL或标记VERIFIED。
- 根因是否确认：是；缺口是生产环境隔离身份和云端证据，不是现有Staging Gateway代码故障。
- 涉及文件：`render.gateway.production.yaml`, `tests/test_claude_gateway.py`，以及本任务账本；云端仅涉及新的Render生产Gateway、AWS Singapore精确OIDC角色和独立DynamoDB控制表。不得修改Staging名称/凭据、阿里云业务数据库、Kimi配置或用户业务数据。
- 风险：生产误复用Staging密钥会造成环境串扰；通配OIDC subject会扩大信任；URL在服务创建前只是推测；只验证单实例无法证明共享控制面；凭据或Prompt进入聊天/日志会泄露敏感数据。
- 执行代理：主CTO串行创建生产身份与服务；现有Render/Release Reviewer只读复核隔离和发布边界。
- 验证代理：独立Release Verification Agent检查Blueprint、实际Render服务身份、两个不同实例、同commit/control marker、exact-once和签名拒绝语义；不得只接受实施结论。
- 验收标准：生产服务与Staging同时存在且身份/密钥/表/角色完全独立；Render真实运行2个不同instance marker且同commit、同config/key epoch、同生产principal/table-role；签名readiness通过，错误authority/旧或错误key固定拒绝且provider0；同一synthetic operation跨两个实例恰1×200、1×409、FakeProvider总计1、真实Claude0；单实例替换期间另一实例持续ready，旧operation仍409、新operation仍exact-once；生产URL经TLS和`/health/ready`核验后才交付阿里云；日志和账本不含Secret、Prompt、正文、URL query或响应正文。
- 是否需要用户决定：生产2个Starter、可扩4个及对应14–28美元/月上限已获批；用户已明确批准本任务。若Render、AWS或Anthropic控制台要求登录、MFA或输入不可读取的生产Secret，只暂停对应输入步骤交给用户，不重复申请相同资源预算。
- 是否涉及真实外部调用：是，创建Render生产服务、AWS Singapore生产控制面并进行零Claude合成验证；真实Claude0次、阿里云业务数据库0写。只有后续另行批准的最小真实Claude Smoke才会产生模型费用。
- 是否已部署到 Render：否；本地生产Blueprint和门禁已通过，等待提交分支、创建云端资源并完成独立多实例验证。
...
- 是否需要用户决定：是；API-F增量费用、两台ECS、生产纯结构migration与临时ACR公网推送均已批准并完成，ACR固定密码重设也已获明确授权并闭环。产品负责人已明确当前进入正式部署阶段，不采用额外的“最小权限修复包”作为前置阻断；但应用运行账号仍不得被提升为数据库超级账号。下一步运行时Secret仍必须通过不进入镜像、Git、cloud-init或任务账本的受控方式注入；Admin仍需固定办公IP/VPN/堡垒机访问方案，根域仍需确定生产前端落点。
- 是否涉及真实外部调用：是；涉及付费RDS/Tair/NAT/EIP/ECS/ALB创建、生产migration和证书部署。RDS、Tair、VPC级SNAT及C/F两台ECS已独立核验为运行中/可用，生产纯结构migration已执行并独立验证；私有ACR镜像发布、API-C精确digest私网拉取与凭据注销也已闭环。应用部署、ALB和证书部署尚未执行。
