...
>
> Scope: independently verified completion of `PROD-XHS-HTTP-ADAPTER-001`. The commercially authorized Spider_XHS source is commit-pinned, the bounded read-only adapter and browser-free production roles are implemented, and the production Browser Worker is removed from Docker/Compose/Render role selection. No image was built, no real Xiaohongshu or supplier call occurred, and ACR/deployment/cloud/production state was not changed.
>
...
`PROD-PREBUILD-001` through read-only `PROD-WORKER-FIXED-RUNTIME-DESIGN-001`, and now `PROD-XHS-HTTP-ADAPTER-001`, are `VERIFIED`. Historical local `linux/amd64` API and Worker images remain immutable evidence for `93b03d5c40fb64fa264c0ada1c055f8e7696b161`; they are not the new release and must not be relabelled or pushed. The new code path replaces the selected production browser architecture with browser-free `api-runtime`, `admin-runtime`, and `xhs-http-runtime` targets, while retaining explicitly opt-in local/legacy browser tooling outside production. The new release commit containing this Handoff becomes the only future image build revision, but no corresponding image, SBOM, scan result, ACR digest, service or production traffic exists yet.
...
| Task-start HEAD / parent | `f3a1a3bb28acd4a2b7e33221d0d8d0100a79c9a9` / `93b03d5c40fb64fa264c0ada1c055f8e7696b161` |
| New release message | `feat(prod): add bounded XHS HTTP runtime [skip render]` |
| Next production build source | the full commit containing this verified implementation and Handoff; resolve its exact SHA from Git after commit creation, never use `f3a1a3b`, `93b03d5`, a Handoff-only descendant or a relabelled old image |
| Upstream | `origin/codex/quality-stabilization-real-chain` |
| Ahead / behind at task start | `0 / 0`; task-start HEAD and upstream both equal `f3a1a3bb28acd4a2b7e33221d0d8d0100a79c9a9` |
| Worktree before release commit | implementation, tests and Handoff are intentionally modified; staged only after final verification and sensitive-file review |
| Push state | the verified application release is authorized for one normal non-force push on the current branch with `[skip render]` |
...
- No retained role image may be deployed. After `PROD-XHS-HTTP-ADAPTER-001`, each browserless production role may be considered only after a fresh build from the new exact application commit, complete role-specific acceptance/scans, a later approved ACR push/read-back recording its own immutable registry digest, and closure of every residual release blocker.
...
- Any future build must use the exact full release commit containing the verified `PROD-XHS-HTTP-ADAPTER-001` implementation and this Handoff. Do not use `f3a1a3b`, `93b03d5`, `89e3505`, `8f7d9c2`, `b6abaa7`, `ff030f5` or a later Handoff-only HEAD as the post-adapter application revision.
...
- Only recommended next approval: `PROD-IMG-XHS-BROWSERLESS-BUILD-001` as recorded below. Continue to prohibit ACR, exception/VEX registration, deployment and real Xiaohongshu calls. Do **not** approve `PROD-IMG-002` for either retained `93b03d5` image.
...
- **Next gate:** this browser-runtime path is retained as historical evidence but is no longer the selected production architecture. Do not repeat its build, probes or vendor-fix review unless the product decision changes. The bounded direct-HTTP implementation below is complete; the next gate is a separately approved native AMD64 browser-free build/scan task.

### PROD-XHS-HTTP-ADAPTER-001 — Replace production browser collection with a bounded direct-HTTP adapter

- **Status:** `VERIFIED` on 2026-07-22. The independently verified application/Handoff release commit is the commit containing this section; its exact full SHA is emitted after local commit creation and becomes the only future production-image revision.
...
- **Future image set:** after verified implementation, separately approve fresh native AMD64 builds and scans for the new exact commit. Expected production roles are browserless `api-runtime`, `admin-runtime` and `xhs-http-runtime`; no production browser Worker image is expected. Local image IDs remain distinct from future ACR immutable digests.
...
- **Next approval:** `PROD-IMG-XHS-BROWSERLESS-BUILD-001` only: recheck the existing isolated ECS boundary, then native AMD64-build and locally scan `api-runtime`, `admin-runtime`, and `xhs-http-runtime` from the new exact release commit; perform offline synthetic Node/`crypto-js` signer smoke without a real XHS request; stop before ACR or deployment.

### PROD-IMG-XHS-BROWSERLESS-BUILD-001 — Build and scan the three browser-free AMD64 roles

- **Status:** `READY_TO_EXECUTE`, but not authorized or started until the user explicitly approves this exact task.
- **Exact source:** the full release commit containing the verified adapter implementation and this Handoff. Resolve and record its 40-character SHA after commit creation; use a clean detached worktree and verify Git LFS/model assets before building.
- **Execution:** first recheck that the existing isolated Shenzhen native `x86_64` ECS still has no production RDS/Tair route, no production Secret, no ACR login and no unexpected container. Then locally build `api-runtime`, `admin-runtime`, and `xhs-http-runtime` for `linux/amd64` with exact OCI provenance; generate SBOMs; scan content/history/Secret/vulnerabilities; verify non-root users, role markers, entrypoint/CMD, models and `mttravel`; prove no production target contains an installed Playwright/Chromium/browser graphics capability. Run only a synthetic offline Node/`crypto-js` signer contract smoke with network disabled and no real Cookie/session.
- **Prohibited:** ACR login/push, deployment/service start, production DB/Tair/data access, cloud-resource mutation, real Xiaohongshu or supplier calls, proxy/challenge behavior, VEX/production exception registration and deletion of retained evidence.
- **Acceptance:** all three images identify the same exact release revision; role/content/provenance/SBOM/Secret gates pass; `xhs-http-runtime` signer assets and `crypto-js` pass offline; scan results are independently reconciled and every Critical/High remains unsuppressed until a later explicit decision. Local image IDs must never be described as ACR digests.
- **Cost/rollback:** existing isolated ECS compute/disk only, subject to a fresh cost/lifecycle read before execution. On failure, stop, retain bounded evidence, and leave images unpushed; deleting local images/cache or releasing the builder remains a separate lifecycle action.
...
- **Status:** `BLOCKED`
- **Priority:** Critical
- **Current evidence:** exact hardened split AMD64 candidates exist locally for `93b03d5`, but the Worker is now excluded from the selected future architecture and the API predates the planned adapter release. Neither is the final release candidate. No production exception/VEX or ACR registry digest exists.
- **Prerequisites:** complete and independently verify `PROD-XHS-HTTP-ADAPTER-001`; create the new exact application release commit; separately approve and pass native AMD64 builds, provenance/content/SBOM/Secret/vulnerability scans and runtime acceptance for the browserless production roles; close any residual release-blocking finding; explicitly accept the new role images and approve an ACR access window. Public access remains closed except an explicitly approved, time-bounded exception.
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| LEGACY-XHS-001 — XHS acquisition/account-policy recovery | `VERIFIED` through `PROD-XHS-HTTP-ADAPTER-001`; real-session validation remains separately gated | High | Browser-free bounded direct adapter and production roles passed offline independent verification; commercial authorization is stated resolved, but Xiaohongshu platform authorization is not asserted and no real session call has been made. | New exact release image build/scan, then a separately approved bounded real-session task. | Build/scan locally first; later validate only approved read operations with strict count/time/stop gates and default suspension. | Interface/signature churn, account restriction, platform enforcement, incomplete/bad evidence. | Re-enable suspension and retain manual/screenshot evidence input; never silently restore Playwright. | New images pass content/SBOM/Secret/vulnerability gates; any later real smoke yields bounded evidence without write/challenge bypass. | None now; later smoke is a real platform call. | Engineering/build-host cost first; later egress/account impact must be bounded. | Required before build, again before real platform call, and again before deployment. |
...
- The new `PROD-XHS-HTTP-ADAPTER-001` release worktree passed independent socket-blocked full suite `625 OK (skipped=5)` with `network_attempts=0`, targeted `130/130` with `network_attempts=0`, readiness `83/83`, Python/Shell/Compose/diff checks, exact source/assets, tracking state transitions and high-confidence sensitive scan. No image was built and the exact release SHA is the commit containing this Handoff.
...
- The superseded combined local native AMD64 candidate remains blocked at SBOM `298`, Secret `0`, `32 High / 7 Critical`. The predecessor split candidates at `89e3505` and current hardened candidates at `93b03d5` show the same role-specific counts: API SBOM `174`, Secret `0`, `3 Critical / 19 High`; Worker SBOM `288`, Secret `0`, `5 Critical / 32 High`. Current hardened images remain local-only; API and Worker runtime acceptance passed, while the unsuppressed residual vulnerability disposition remains open.
...
- Do not use `git-ff030f5`, `git-ff030f5-amd64-r2`, `git-ecc4702e`, `git-b6abaa7-amd64-r1`, `git-8f7d9c2-amd64-r1`, either `git-89e3505` local candidate, either retained `git-93b03d5` local image, `latest`, or any unverified tag for the future production release. `93b03d5c40fb64fa264c0ada1c055f8e7696b161` remains the immutable source identity of the retained evidence images and the code baseline for `PROD-XHS-HTTP-ADAPTER-001`; the new verified adapter release commit must become the only future build source. The Worker browser findings are not waived—they are avoided only by proving the browser Worker is absent from the new production role graph. Do not repeat symbol searches, parser/API fixtures or exploitability probes for those rows.
...
1. Verify the current branch and exact full release commit containing `PROD-XHS-HTTP-ADAPTER-001`; also verify upstream divergence and a clean worktree. Keep `93b03d5c40fb64fa264c0ada1c055f8e7696b161` only as the immutable identity of retained historical images.
...
4. Treat the historical `32 High / 7 Critical` evidence only as the superseded combined-image baseline. Use the current hardened scan reports under `/opt/noteai-build/evidence-93b03d5-hardened/api-final-r4/` for API (`3 Critical / 19 High`) and `worker-final-r4/` for Worker (`5 Critical / 32 High`); do not silently suppress, merge or project one role's result onto the other.
5. Keep **PROD-IMG-002** blocked from registry/release action until separately approved `PROD-IMG-XHS-BROWSERLESS-BUILD-001` creates fresh native AMD64 role images from the new exact commit, all scans/acceptance pass, and every remaining release blocker is closed. The adapter implementation and all older browser-path audits/builds are complete; do not repeat old source import, builds/scans, sandbox smoke, symbol probing, parser fixtures, role-level reconciliation or runtime-candidate comparison without a new evidence-based reason and explicit approval.
...
The next session must not repeat the completed production Gateway creation, domain/DNS purchase, certificate purchase/issuance, clean RDS schema migration, prior ARM64 push, historical builds/scans, browser hardening/runtime probes, vulnerability disposition, fixed-runtime design or `PROD-XHS-HTTP-ADAPTER-001` implementation and offline verification. The only recommended next approval is `PROD-IMG-XHS-BROWSERLESS-BUILD-001`; it must use the new exact release commit, build/scan locally on the isolated native AMD64 host, make zero real Xiaohongshu/supplier calls, and stop before ACR or deployment.

## 10. Checkpoint condition

Historical image release `93b03d5c40fb64fa264c0ada1c055f8e7696b161` and task-start Handoff commit `f3a1a3bb28acd4a2b7e33221d0d8d0100a79c9a9` were already pushed normally. The new commit containing code, tests, configuration, provenance and this completed Handoff is the `PROD-XHS-HTTP-ADAPTER-001` application release and sole future image-build revision; its exact SHA is learned only after commit creation and must be reported from Git. It must use `[skip render]`, push normally to the existing tracking branch, never force-push, and must not trigger any image build, ACR access, deployment, database access, cloud change or provider call.
