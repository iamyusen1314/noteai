...
>
> Scope: independently verified repository closure for `PROD-PILLOW-12.3.0-FIX-001`. The shared browserless production dependency pin is upgraded from Pillow `12.2.0` to `12.3.0`, with a production-readiness pin guard and a real image round-trip/feature-extraction regression test. Python 3.11 wheel compatibility, targeted/full unit tests, readiness, model/quality/static checks and independent review passed. Existing local images remain unchanged and release-blocked until a separately approved rebuild and unsuppressed rescan; no image build, ACR access, deployment, VEX/exception, production/provider access or cloud-resource mutation occurred.
>
...
NoteAI is in **production infrastructure preparation and controlled release**, not general availability. Render Staging remains the validated staging environment. The production Claude Gateway is live on Render Singapore, while the Alibaba Cloud production application path has not been released: the split AMD64 candidates remain local-only and are not release-accepted because their unsuppressed Critical/High findings lack an approved disposition, API-C/API-F application services are not accepted, no ALB is configured, TLS certificates are not attached to a production listener, and production DNS has not been switched.

`PROD-PREBUILD-001` through read-only `PROD-WORKER-FIXED-RUNTIME-DESIGN-001`, `PROD-XHS-HTTP-ADAPTER-001`, `PROD-XHS-RAP-FIX-001`, the execution scope of `PROD-IMG-XHS-BROWSERLESS-REBUILD-001`, and now `PROD-PILLOW-12.3.0-FIX-001` are `VERIFIED`. Existing local `ca023355` images still contain Pillow `12.2.0` and remain release-blocked at `4 Critical / 29 High` per role; repository verification cannot change an immutable image. Exact Pillow-fixed application revision `a635692a899ee02c6905cd694611c14e0da4594a` is the only permissible source of the next separately approved browserless rebuild/rescan. The ten Pillow rows are expected—but not yet proven—to disappear only after that rebuild; the remaining Debian rows still require later read-only disposition.
...
| Pillow-fixed application revision | `a635692a899ee02c6905cd694611c14e0da4594a` (`fix(prod): upgrade Pillow to 12.3.0 [skip render]`), independently verified and pushed; this is the next image build source |
| CI/Handoff verification tip | `344897c36f7cb69a9dc4d8fe1794b6170722e6fd` (`test(ci): isolate XHS fixture environment`), containing only CI fixture isolation and the prior Handoff; never use it as the OCI application revision |
| Verified local image source | exact application revision `ca02335513a752ece2dabe71d08105ae86db6c5a`; never reuse `33d04d4`, substitute `344897c`, or relabel an old image |
| Upstream | `origin/codex/quality-stabilization-real-chain` |
| Ahead / behind at Pillow task start | `0 / 0`; task-start HEAD and upstream both equal `344897c36f7cb69a9dc4d8fe1794b6170722e6fd` |
| Worktree at Pillow task start | only the already-reviewed local rebuild Handoff was modified; Pillow implementation then changed only the shared dependency pin and two directly related test files |
| Push state | Pillow-fixed application revision `a635692a899ee02c6905cd694611c14e0da4594a` was pushed normally with `[skip render]`. A later Handoff-only descendant must never replace it as an OCI revision. |
...
- `a635692` — upgrade Pillow to `12.3.0`; exact application revision for the next separately approved browserless rebuild/rescan.
- `344897c` — prior CI fixture-isolation/Handoff checkpoint only; never an application/image revision.
- `ca02335` — isolate and verify the XHS RAP signer runtime; exact application revision of the three current local browserless images.
...
- Exact source revision for both retained hardened role images: `93b03d5c40fb64fa264c0ada1c055f8e7696b161` (`93b03d5`). It is the baseline for the planned adapter work, not the final post-adapter build revision.
...
- No retained role image may be deployed. The exact `ca023355` browserless roles have now been freshly built and accepted only as local execution evidence; their `4 Critical / 29 High` findings still block release. The next build source must be exact Pillow-fixed application commit `a635692a899ee02c6905cd694611c14e0da4594a`, followed by complete role-specific acceptance/scans, disposition of every residual blocker, and only then a separately approved ACR push/read-back recording immutable registry digests.
...
| `noteai-local:git-ca02335-amd64-api-r1` | Local image ID `sha256:5fa36abe1c603aa0abc1295912fe6ee16a0b468d8be216475be06a4daa9a6454`; no ACR digest | `linux/amd64` | **Current local build, release-blocked:** exact RAP-fixed revision; Secret `0`, `4 Critical / 29 High`. |
| `noteai-local:git-ca02335-amd64-admin-r1` | Local image ID `sha256:6d3b5ee2dd4619a60394ad4e026287fe4b5a9a39dd722c37954fe58627369865`; no ACR digest | `linux/amd64` | **Current local build, release-blocked:** exact RAP-fixed revision; Secret `0`, `4 Critical / 29 High`. |
| `noteai-local:git-ca02335-amd64-xhs-http-r1` | Local image ID `sha256:755e337c2932ef53187b5af333b1daa47be578a81af9a57d6c1af7a9368b3b60`; no ACR digest | `linux/amd64` | **Current local build, release-blocked:** exact RAP-fixed revision; Secret `0`, `4 Critical / 29 High`; basic/RAP `network=none` smoke passed. |
...
- A separately approved rebuild/rescan of all three browserless roles from exact Pillow-fixed commit `a635692a899ee02c6905cd694611c14e0da4594a`, followed by read-only disposition of any remaining Debian rows. No production exception or VEX has been registered.
...
- Historical blocking acceptance: Trivy `0.70.0` reported `32 High` and `7 Critical` for the superseded combined image; the later split/hardened results were API `3 Critical / 19 High` and Worker `5 Critical / 32 High`. The current browserless API, Admin and XHS images each report `4 Critical / 29 High`; no ignore, VEX or production exception was used or registered.
...
- The current retained browserless evidence uses exact application revision `ca02335513a752ece2dabe71d08105ae86db6c5a`. The next build must use exact verified Pillow-fixed application commit `a635692a899ee02c6905cd694611c14e0da4594a`. Never use `344897c`, any other Handoff-only commit, or historical `f3a1a3b`, `93b03d5`, `89e3505`, `8f7d9c2`, `b6abaa7` or `ff030f5` as that application revision.
...
- Acceptance evidence required before any push decision, separately for future browserless `api-runtime`, `admin-runtime` and `xhs-http-runtime`: AMD64 child manifest/architecture, full OCI revision/source/version/created labels, expected role marker and owner, entrypoint/user/layers/model and role-appropriate contents, absence of installed Playwright/Chromium/browser graphics capability and of any production-reachable browser command, SBOM, vulnerability scan, Secret scan and an explicit release decision for every Critical or High finding.
...
- **Next gate:** this browser-runtime path is retained as historical evidence but is no longer the selected production architecture. Do not repeat its build, probes or vendor-fix review unless the product decision changes. The bounded direct-HTTP implementation, RAP-fixed browserless rebuild and repository Pillow fix are complete; the current next gate is a separately approved three-role rebuild/rescan from `a635692a899ee02c6905cd694611c14e0da4594a`.
...
- **Image set:** the separately approved RAP-fixed native AMD64 build/scan has completed for browserless `api-runtime`, `admin-runtime` and `xhs-http-runtime`; no production browser Worker image exists. The local image IDs remain distinct from future ACR immutable digests and are release-blocked pending a rebuild/rescan of the verified Pillow fix and residual Debian disposition.
...
- **Successor sequence:** the first browserless build failed RAP acceptance; `PROD-XHS-RAP-FIX-001`, the clean successor rebuild and `PROD-PILLOW-12.3.0-FIX-001` are now complete. The only recommended next approval is a clean three-role rebuild/rescan from exact `a635692a899ee02c6905cd694611c14e0da4594a`; do not approve ACR or deployment.
...
- **Next task:** `PROD-PILLOW-12.3.0-FIX-001` completed independently as recorded below. Existing images remain immutable and unchanged; a clean three-role rebuild/rescan from exact Pillow-fixed application revision `a635692a899ee02c6905cd694611c14e0da4594a` is the only recommended next approval. After that rebuild, remaining Debian rows require a new read-only `PROD-BROWSERLESS-OS-VULN-DISPOSITION-001`; no ACR task is next.
...
- **Image impact:** none. Existing `ca023355` local images still contain Pillow `12.2.0` and retain their recorded `4 Critical / 29 High`; no vulnerability row is considered removed until a separately approved rebuild and unsuppressed rescan proves it.
...
- **Next gate:** separately approve a clean native AMD64 rebuild/rescan of `api-runtime`, `admin-runtime` and `xhs-http-runtime` from exact commit `a635692a899ee02c6905cd694611c14e0da4594a`. Stop before ACR or deployment.

### PROD-IMG-002 — Push to ACR and verify digest

- **Status:** `BLOCKED`
- **Priority:** Critical
- **Current evidence:** exact RAP-fixed browserless native AMD64 candidates now exist locally for `ca02335513a752ece2dabe71d08105ae86db6c5a`; provenance/content/SBOM/Secret/vulnerability and signer checks completed, but every role still has unsuppressed `4 Critical / 29 High`. No production exception/VEX or ACR registry digest exists.
- **Prerequisites:** rebuild and unsuppressed-rescan exact Pillow-fixed application revision `a635692a899ee02c6905cd694611c14e0da4594a`; separately disposition every remaining Debian Critical/High row; explicitly accept the successor role images and approve an ACR access window. Public access remains closed except an explicitly approved, time-bounded exception.
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| LEGACY-XHS-001 — XHS acquisition/account-policy recovery | `VERIFIED` through `PROD-PILLOW-12.3.0-FIX-001`; real-session validation remains separately gated | High | Browser-free bounded direct adapter, RAP fix, local browserless image execution and repository Pillow fix passed offline verification; commercial authorization is stated resolved, but Xiaohongshu platform authorization is not asserted and no real session call has been made. | Separately approved rebuild/rescan from exact Pillow-fixed revision, remaining Debian disposition, then a separately approved bounded real-session task. | Rebuild/rescan separately; only after release-risk closure validate approved read operations with strict count/time/stop gates and default suspension. | Interface/signature churn, account restriction, platform enforcement, incomplete/bad evidence. | Re-enable suspension and retain manual/screenshot evidence input; never silently restore Playwright. | Successor images pass content/SBOM/Secret/vulnerability gates with an approved disposition; any later real smoke yields bounded evidence without write/challenge bypass. | None now; later smoke is a real platform call. | Engineering/build-host cost first; later egress/account impact must be bounded. | Required before rebuild, again before real platform call and deployment. |
...
- Do not use `git-ff030f5`, `git-ff030f5-amd64-r2`, `git-ecc4702e`, `git-b6abaa7-amd64-r1`, `git-8f7d9c2-amd64-r1`, either `git-89e3505` local candidate, either retained `git-93b03d5` local image, any `git-33d04d4` image, any current `git-ca02335` local image, `latest`, or any unverified tag for production release. `ca02335513a752ece2dabe71d08105ae86db6c5a` remains the immutable source identity of the current local evidence images; exact Pillow-fixed application commit `a635692a899ee02c6905cd694611c14e0da4594a` is the only successor build source. The Worker browser findings are not waived—they are avoided only by proving the browser Worker is absent from the production role graph. Do not repeat symbol searches, parser/API fixtures or exploitability probes for those rows.
...
1. Verify current branch/upstream and worktree explicitly: `ca02335513a752ece2dabe71d08105ae86db6c5a` is the immutable application/OCI revision of retained current images; `a635692a899ee02c6905cd694611c14e0da4594a` is the only permissible source of the next rebuild; `344897c36f7cb69a9dc4d8fe1794b6170722e6fd` and any later Handoff-only descendant are documentation provenance only.
...
4. Treat the historical combined/API/Worker reports only as superseded baselines. Current browserless reports are under `/opt/noteai-build/evidence-ca02335-xhs-browserless-rebuild-r1/scans/`: API/Admin/XHS are each `4 Critical / 29 High`, with distinct SBOM counts `174/174/175` and Secret `0`. Do not silently suppress, merge, waive or replace these raw results.
5. Keep **PROD-IMG-002** blocked from registry/release action. `PROD-PILLOW-12.3.0-FIX-001` is repository-verified, but every current local role image still has unsuppressed `4 Critical / 29 High`. Do not repeat the completed root-cause audit, signer fix or old build. The next recommended approval is a clean three-role rebuild/rescan from exact application revision `a635692a899ee02c6905cd694611c14e0da4594a`.
...
The next session must not repeat the completed production Gateway creation, domain/DNS purchase, certificate purchase/issuance, clean RDS schema migration, prior ARM64 push, historical builds/scans, browser hardening/runtime probes, fixed-runtime design, `PROD-XHS-HTTP-ADAPTER-001`, `PROD-XHS-RAP-FIX-001`, `PROD-IMG-XHS-BROWSERLESS-REBUILD-001`, `PROD-PILLOW-12.3.0-FIX-001`, or the completed `33d04d4` browserless build/scans. The only recommended next approval is a clean native AMD64 three-role rebuild/rescan from `a635692a899ee02c6905cd694611c14e0da4594a`; it must not implicitly authorize ACR, deployment, VEX/exception or production access.

## 10. Checkpoint condition

Application release `ca02335513a752ece2dabe71d08105ae86db6c5a` remains the exact OCI revision of the three current local images. Pillow-fixed application release `a635692a899ee02c6905cd694611c14e0da4594a` is independently verified and pushed with `[skip render]`, but no image has yet been built from it. Any later Handoff-only commit is documentation provenance only. Current local IDs are not ACR digests and remain blocked; the next approved build must use exact `a635692a899ee02c6905cd694611c14e0da4594a`, never relabel an existing image.
