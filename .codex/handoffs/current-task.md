...
>
> Scope: completed `PROD-BROWSERLESS-VEX-REVIEW-001` for the exact local `a635692a899ee02c6905cd694611c14e0da4594a` native AMD64 `api-runtime`, `admin-runtime` and `xhs-http-runtime` candidates. A separate CycloneDX 1.6 VEX now records twelve exact-product `not_affected` dispositions with 69 package BOM-Link assertions across the three retained SBOMs; an independent agent verified the schema, mappings, immutable evidence binding and no-waiver boundary. The canonical raw Trivy reports remain unchanged and unsuppressed at `4 Critical / 19 High` per role. The VEX is not a production exception, ACR authorization or deployment authorization, and local image IDs remain explicitly distinct from registry digests. No image build, ACR access, deployment, production/provider access or cloud-resource mutation occurred.
>
...
NoteAI is in **production infrastructure preparation and controlled release**, not general availability. Render Staging remains the validated staging environment. The production Claude Gateway is live on Render Singapore, while the Alibaba Cloud production application path has not been released: the exact three AMD64 candidates remain local-only, API-C/API-F application services are not accepted, no ALB is configured, TLS certificates are not attached to a production listener, and production DNS has not been switched.

`PROD-PREBUILD-001` through read-only `PROD-WORKER-FIXED-RUNTIME-DESIGN-001`, `PROD-XHS-HTTP-ADAPTER-001`, `PROD-XHS-RAP-FIX-001`, `PROD-IMG-XHS-BROWSERLESS-REBUILD-001`, `PROD-PILLOW-12.3.0-FIX-001`, the local execution scope of `PROD-IMG-XHS-PILLOW-REBUILD-001`, read-only `PROD-BROWSERLESS-OS-VULN-DISPOSITION-001`, read-only `PROD-BROWSERLESS-RESIDUAL-DECISION-001`, `PROD-BROWSERLESS-CONSTRAINT-PROOF-001`, and `PROD-BROWSERLESS-VEX-REVIEW-001` are `VERIFIED`. Exact Pillow-fixed application revision `a635692a899ee02c6905cd694611c14e0da4594a` remains the OCI revision of the three current local candidates; deployment-control release `253fde55fa609676a9fca5c14fdf6aeceef74000` and later Handoff/VEX commits are not image revisions. Image contents and canonical raw reports remain unchanged and unsuppressed at `4 Critical / 19 High`. `PROD-IMG-002` now requires only a separate explicit ACR push/read-back approval; any pushed artifact must then receive a registry-digest-bound VEX reissue before deployment.
...
| Task-start HEAD | `64660ff7bc5330425e54403a331a7841615624af`; Handoff-only descendant, never an OCI application revision |
...
| Ahead / behind at task start | `0 / 0`; HEAD and upstream both equaled `64660ff7bc5330425e54403a331a7841615624af` |
...
- No retained role image may be deployed. The exact `a635692a899ee02c6905cd694611c14e0da4594a` Pillow-fixed browserless roles have been built and accepted only for authorized local evidence. Their raw reports remain canonical and unsuppressed at `4 Critical / 19 High`; the separate formal VEX review is now complete for all twelve unique CVEs. Only another explicit approval may authorize an ACR push/read-back, and registry-digest VEX reissue must then precede deployment.
...
- **Artifact identity:** `security/vex/a635692-browserless.vex.cdx.json` SHA256 `cf30b880f95c5b0b889bce79478b2b26aa71b66aee884c7380c9dbf9adde2746`; machine evidence manifest SHA256 `7deb56837f833075e4579d4aaa0dd26c6ea5d4cbfcf9505ce109324f8af6d1db`; independent review record SHA256 `9fd0e25950a7970248e36aeed823858731b8500a08cea448e8bce945820137f9`. The review record binds the first two hashes and explicitly denies production exception, ACR authorization and deployment authorization.
...
- **Next approval:** `PROD-IMG-002` may now be proposed as the only next release task: push the three exact retained candidates once, read back immutable `linux/amd64` ACR digests, then rebind/reissue VEX before any deployment. It still requires explicit product-owner approval.

### PROD-IMG-002 — Push to ACR and verify digest

- **Status:** `BLOCKED`
- **Priority:** Critical
- **Current evidence:** exact Pillow-fixed browserless native AMD64 candidates exist locally for `a635692a899ee02c6905cd694611c14e0da4594a`; provenance/content/SBOM/Secret/vulnerability/signer checks and the independent exact-product VEX review are complete. Canonical raw reports remain unsuppressed at `4 Critical / 19 High`; no ACR registry digest exists and no production exception was created.
- **Prerequisites:** explicit product-owner approval of the three exact role image IDs, role-specific tags, ACR access window, expected cost/impact and stop point. Public access remains closed unless a separately approved time-bounded route is strictly necessary.
- **Execution steps:** login only within the approved window; push each exact role image once; read back manifest/index, immutable digest and `linux/amd64`; compare remote/local provenance; reissue VEX against the three registry digests; stop before service start or deployment.
...
- **Acceptance:** ACR read-back proves one allowed `linux/amd64` manifest per exact role/revision, records immutable registry digests, and a new independent review confirms the registry-digest-bound VEX before any deployment.
...
5. Keep **PROD-IMG-002** blocked from registry action until the product owner explicitly approves the exact three local image IDs, role tags, ACR window, cost/impact and stop point. Do not repeat the completed Pillow fix, three-role rebuild, residual Debian audit, residual decision, constraint proof or VEX review. The VEX is a separate impact statement and raw Trivy evidence remains canonical and unsuppressed.
...
The next session must not repeat the completed production Gateway creation, domain/DNS purchase, certificate purchase/issuance, clean RDS schema migration, prior ARM64 push, historical builds/scans, browser hardening/runtime probes, fixed-runtime design, `PROD-XHS-HTTP-ADAPTER-001`, `PROD-XHS-RAP-FIX-001`, `PROD-IMG-XHS-BROWSERLESS-REBUILD-001`, `PROD-PILLOW-12.3.0-FIX-001`, `PROD-IMG-XHS-PILLOW-REBUILD-001`, `PROD-BROWSERLESS-OS-VULN-DISPOSITION-001`, `PROD-BROWSERLESS-RESIDUAL-DECISION-001`, `PROD-BROWSERLESS-CONSTRAINT-PROOF-001`, `PROD-BROWSERLESS-VEX-REVIEW-001`, or the completed `33d04d4` browserless build/scans. The only recommended next approval is `PROD-IMG-002` scoped to exact role-image ACR push/read-back and registry-digest VEX reissue; it must stop before deployment or service start.

## 10. Checkpoint condition

Application release `a635692a899ee02c6905cd694611c14e0da4594a` is the exact OCI revision of the three current local images and remains their only application provenance. Deployment-control release `253fde55fa609676a9fca5c14fdf6aeceef74000`, the VEX/review release and any later Handoff descendant are not OCI revisions. Current local IDs are not ACR digests. Canonical raw reports remain unchanged and unsuppressed at `4 Critical / 19 High` per role, while the separately authored and independently verified CycloneDX VEX records twelve exact-product `not_affected` decisions without granting an exception. The next task requiring explicit approval is `PROD-IMG-002`; after push/read-back, reissue VEX against actual ACR registry digests before any deployment.
