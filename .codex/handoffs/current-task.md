...
- **Acceptance:** one immutable `linux/amd64` manifest exists for each exact role/revision, all provenance and digest read-backs match, VEX version 2 is digest-bound, and all temporary access is closed.
...
- **Next approval:** only read-only `PROD-IMG-003` may be proposed next. It must inspect these exact digests and may not deploy or start services.
