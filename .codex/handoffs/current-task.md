...
>
> Scope: verified implementation result for `PROD-XHS-RAP-FIX-001`. The application-owned signer wrapper now isolates the pinned RAP asset from the main signer's process-global shims, and real-wrapper basic/RAP synthetic tests pass with network modules disabled. The two SHA-pinned upstream assets remain byte-identical. No image build, ACR access, deployment, production access, real Xiaohongshu/supplier call, production exception/VEX or cloud-resource mutation occurred.
>
...
`PROD-PREBUILD-001` through read-only `PROD-WORKER-FIXED-RUNTIME-DESIGN-001`, `PROD-XHS-HTTP-ADAPTER-001`, and now `PROD-XHS-RAP-FIX-001`, are `VERIFIED`. The three local `linux/amd64` candidates for old revision `33d04d4560f9b7dda43a53527f8ae1026dc335d3` remain rejected historical evidence because they predate the RAP fix. They have empty `RepoDigests` and must not be relabelled, pushed or deployed. The next image source is exact application revision `ca02335513a752ece2dabe71d08105ae86db6c5a`; it requires a separately approved clean native AMD64 rebuild and complete acceptance before any ACR decision. A later CI/Handoff-only descendant is verification provenance, not a replacement OCI application revision.
...
| CI/Handoff verification tip | current `[skip render]` commit `test(ci): isolate XHS fixture environment`, containing only CI fixture isolation and this Handoff; identify it from Git history and never use it as the OCI application revision |
| Next production build source | exact application revision `ca02335513a752ece2dabe71d08105ae86db6c5a`; never reuse `33d04d4` as an accepted release, and never substitute a CI/Handoff-only descendant or relabelled old image |
| Upstream | `origin/codex/quality-stabilization-real-chain` |
| Ahead / behind at task start | `0 / 0`; task-start HEAD and upstream both equal `33d04d4560f9b7dda43a53527f8ae1026dc335d3` |
| Worktree before CI/Handoff closure commit | only `.github/workflows/ci.yml` and this Handoff are intentionally modified; staged only after final independent verification and sensitive-file review |
| Push state | application revision `ca02335` is already pushed; one normal non-force `[skip render]` follow-up push is authorized for CI/Handoff closure |
...
- `ca02335` — isolate and verify the XHS RAP signer runtime; exact application revision for the next separately approved image rebuild.
...
- `PROD-IMG-XHS-BROWSERLESS-BUILD-001` has now executed and failed RAP runtime acceptance as recorded below. The only recommended next approval is `PROD-XHS-RAP-FIX-001`. Continue to prohibit ACR, exception/VEX registration, deployment and real Xiaohongshu calls; do **not** approve `PROD-IMG-002` for any current local image.
...
- **Next approval:** superseded by the executed build task below. Because its RAP signer acceptance failed, the only recommended successor is a separately approved `PROD-XHS-RAP-FIX-001`; do not approve ACR or deployment.
...
- **Successor:** `PROD-XHS-RAP-FIX-001` is now independently verified below. These old images remain blocked; the only recommended next approval is the clean successor rebuild `PROD-IMG-XHS-BROWSERLESS-REBUILD-001`.
...
- **Next gate:** `PROD-IMG-XHS-BROWSERLESS-REBUILD-001` requires separate approval. Recheck the retained isolated ECS, then build and scan all three browserless native AMD64 roles from a clean detached checkout of exact application revision `ca02335513a752ece2dabe71d08105ae86db6c5a`; repeat provenance/content/SBOM/Secret/unsuppressed vulnerability checks and `network=none` basic/RAP wrapper smokes. Stop before ACR or deployment.

### PROD-IMG-002 — Push to ACR and verify digest

- **Status:** `BLOCKED`
- **Priority:** Critical
- **Current evidence:** exact browserless native AMD64 candidates exist locally for `33d04d4560f9b7dda43a53527f8ae1026dc335d3`; provenance/content/SBOM/Secret/vulnerability scans completed, but they predate the now-verified RAP fix and are not final release candidates. No production exception/VEX or ACR registry digest exists.
- **Prerequisites:** create and push the exact RAP-fix application release commit; separately approve and pass `PROD-IMG-XHS-BROWSERLESS-REBUILD-001`; close every residual release-blocking finding; explicitly accept the successor role images and approve an ACR access window. Public access remains closed except an explicitly approved, time-bounded exception.
...
5. Keep **PROD-IMG-002** blocked from registry/release action. `PROD-XHS-RAP-FIX-001` is independently verified, but every current local image predates it. Do not repeat the completed root-cause audit or modify the pinned upstream assets. The next approved execution scope is `PROD-IMG-XHS-BROWSERLESS-REBUILD-001`, and every successor image must use the exact new RAP-fix application commit.
...
The next session must not repeat the completed production Gateway creation, domain/DNS purchase, certificate purchase/issuance, clean RDS schema migration, prior ARM64 push, historical builds/scans, browser hardening/runtime probes, vulnerability disposition, fixed-runtime design, `PROD-XHS-HTTP-ADAPTER-001`, `PROD-XHS-RAP-FIX-001`, or the completed portions of the `33d04d4` browserless build/scans. The only recommended next approval is `PROD-IMG-XHS-BROWSERLESS-REBUILD-001`; it must use a clean detached checkout of the new exact RAP-fix release, make zero real Xiaohongshu/supplier calls, and stop before ACR or deployment.

## 10. Checkpoint condition

Application release `33d04d4560f9b7dda43a53527f8ae1026dc335d3` was already pushed normally and remains the exact source of the three rejected local browserless images. `PROD-XHS-RAP-FIX-001` is independently verified at exact application revision `ca02335513a752ece2dabe71d08105ae86db6c5a`, which was pushed normally with `[skip render]`. It is the only permissible application source of the separately approved successor image rebuild. The later CI/Handoff closure commit records deterministic test-environment evidence only and must not replace `ca02335` as the OCI revision. Neither commit authorizes image build, ACR access, deployment, database access, cloud change or real provider call.
