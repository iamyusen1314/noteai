...
>
> Scope: completed `PROD-BROWSERLESS-CONSTRAINT-PROOF-001` for the exact local `a635692a899ee02c6905cd694611c14e0da4594a` native AMD64 `api-runtime`, `admin-runtime` and `xhs-http-runtime` candidates. Repository deployment controls now enforce UID/GID `999`, `cap_drop: ALL`, `no-new-privileges`, read-only root, bounded tmpfs/writable data mounts and immutable digest references for every production role. Non-invasive `network=none` runtime read-back and offline ELF `DT_NEEDED` closure prove that neither `libblkid` nor `libacl` is in the accepted role execution graph. The raw scanner reports remain unchanged and unsuppressed at `4 Critical / 19 High`; formal VEX/exception was not written, so release remains blocked pending a separately approved formal disposition review. No image build, ACR access, deployment, production/provider access or cloud-resource mutation occurred.
>
...
`PROD-PREBUILD-001` through read-only `PROD-WORKER-FIXED-RUNTIME-DESIGN-001`, `PROD-XHS-HTTP-ADAPTER-001`, `PROD-XHS-RAP-FIX-001`, `PROD-IMG-XHS-BROWSERLESS-REBUILD-001`, `PROD-PILLOW-12.3.0-FIX-001`, the local execution scope of `PROD-IMG-XHS-PILLOW-REBUILD-001`, read-only `PROD-BROWSERLESS-OS-VULN-DISPOSITION-001`, read-only `PROD-BROWSERLESS-RESIDUAL-DECISION-001`, and `PROD-BROWSERLESS-CONSTRAINT-PROOF-001` are `VERIFIED`. Exact Pillow-fixed application revision `a635692a899ee02c6905cd694611c14e0da4594a` remains the OCI revision of the three current local candidates; deployment-control release `253fde55fa609676a9fca5c14fdf6aeceef74000` is not an image revision. The image contents and raw reports remain unchanged and unsuppressed at `4 Critical / 19 High`. `PROD-IMG-002` remains blocked only by a separately approved independent formal VEX/disposition review and subsequent explicit ACR approval, not by another constraint-proof or image build.
...
| Current Handoff parent before this documentation commit | `253fde55fa609676a9fca5c14fdf6aeceef74000`; the later Handoff-only descendant is also never an OCI application revision |
...
| Ahead / behind at task start | `0 / 0`; HEAD and upstream both equaled `4704c1617a169f6d925a1b16e90aba0f8b88d02b` |
| Worktree at task start | application/configuration tree was clean; `.codex/handoffs/current-task.md` already carried the approved preceding read-only decision update and was preserved |
| Push state | Constraint release and this Handoff checkpoint are authorized for normal push with `[skip render]`. Neither may replace `a635692a899ee02c6905cd694611c14e0da4594a` as the OCI revision. |
...
- No retained role image may be deployed. The exact `a635692a899ee02c6905cd694611c14e0da4594a` Pillow-fixed browserless roles have been built and accepted only for authorized local evidence. Their raw `4 Critical / 19 High` findings still block release. Read-only disposition, residual-path decision and the constrained-deployment proof are complete: all twelve unique CVEs now have exact-product evidence ready for independent formal VEX/disposition review. Only after that separate review and another explicit approval may an ACR push/read-back be considered.
...
| `noteai-local:git-a635692-amd64-api-r1` | Local image ID `sha256:b1983bab928ef93495d8be020030917d4ae54364234390047c3163af5414fedb`; no ACR digest | `linux/amd64` | **Current local candidate, release-blocked:** exact Pillow-fixed revision; Secret `0`, `4 Critical / 19 High`, Pillow rows `0`. |
| `noteai-local:git-a635692-amd64-admin-r1` | Local image ID `sha256:2283095764622e373e30b51ba749819751e6bfb0c37c6bb82e2d3bfe4937760f`; no ACR digest | `linux/amd64` | **Current local candidate, release-blocked:** exact Pillow-fixed revision; Secret `0`, `4 Critical / 19 High`, Pillow rows `0`. |
| `noteai-local:git-a635692-amd64-xhs-http-r1` | Local image ID `sha256:5b44114d4bd9c28a8e93c39140466c542e8babeead038fb0d1cfe45c3cd75966`; no ACR digest | `linux/amd64` | **Current local candidate, release-blocked:** exact Pillow-fixed revision; Secret `0`, `4 Critical / 19 High`, Pillow rows `0`; basic/RAP `network=none` smoke passed. |
...
- **Successor sequence:** the first browserless build failed RAP acceptance; `PROD-XHS-RAP-FIX-001`, its successor rebuild, `PROD-PILLOW-12.3.0-FIX-001`, the clean exact `a635692` three-role rebuild/rescan, read-only residual Debian disposition, residual-path decision and constraint proof are now complete. The only recommended next approval is `PROD-BROWSERLESS-VEX-REVIEW-001`; do not approve ACR or deployment.
...
- **Successor status:** `PROD-PILLOW-12.3.0-FIX-001`, the clean three-role rebuild/rescan from exact application revision `a635692a899ee02c6905cd694611c14e0da4594a`, residual Debian disposition, residual decision and constraint proof all completed independently as recorded below. Existing images remain immutable and unchanged; no ACR task is next before formal VEX/disposition review.
...
- **Release interpretation at completion of this read-only task:** six unique CVEs had decisive non-affected evidence (`5 NOT_INSTALLED`, `1 NOT_APPLICABLE_ARCH`), four had high-confidence static `NO_CONFIGURED_PATH` evidence, and two High CVEs remained `UNKNOWN`. The successor constraint proof now closes those two technical evidence gaps, but no VEX/exception or release acceptance has been written; all raw rows therefore remain scanner/process blockers.
- **Minimum remediation decision:** a same-digest rebuild or ordinary Trixie `apt upgrade` cannot remove the rows, and importing individual forky/sid packages into Trixie is rejected. The narrowest non-waiting paths were (a) a separately approved formal, independently reviewed release disposition for the ten high-confidence CVEs plus deployment constraints/evidence for the two then-`UNKNOWN` libraries, or (b) a separately designed minimal/distroless final runtime. The successor constraint proof completed Path A's technical evidence; formal review is still pending.
...
- **Next gate:** `PROD-BROWSERLESS-CONSTRAINT-PROOF-001` completed as recorded below. The next separately approved task is an independent formal VEX/disposition authoring and review scoped only to the exact immutable local images and retained raw evidence. It must not implicitly authorize ACR or deployment.
...
- **Status:** `BLOCKED`
- **Priority:** Critical
- **Current evidence:** exact Pillow-fixed browserless native AMD64 candidates now exist locally for `a635692a899ee02c6905cd694611c14e0da4594a`; provenance/content/SBOM/Secret/vulnerability and signer checks completed, Pillow rows are `0`, but every role still has unsuppressed `4 Critical / 19 High`. No production exception/VEX or ACR registry digest exists.
- **Prerequisites:** complete a separately approved independent formal VEX/disposition authoring and review for all twelve accepted `not_affected` conclusions; explicitly accept the exact role images and approve an ACR access window. Public access remains closed except an explicitly approved, time-bounded exception.
...
1. Verify current branch/upstream and worktree explicitly: `a635692a899ee02c6905cd694611c14e0da4594a` is the immutable application/OCI revision of the current local candidates; `253fde55fa609676a9fca5c14fdf6aeceef74000` is deployment-control provenance and any later Handoff-only descendant is documentation provenance only.
...
5. Keep **PROD-IMG-002** blocked from registry/release action. Do not repeat the completed Pillow fix, three-role rebuild, residual Debian audit, residual decision or constraint proof. The next recommended approval is `PROD-BROWSERLESS-VEX-REVIEW-001`: independently author and review exact-product formal dispositions for the ten prior high-confidence findings plus `libblkid` and `libacl`, retaining raw scanner evidence. It must not implicitly authorize image build, ACR or deployment.
...
The next session must not repeat the completed production Gateway creation, domain/DNS purchase, certificate purchase/issuance, clean RDS schema migration, prior ARM64 push, historical builds/scans, browser hardening/runtime probes, fixed-runtime design, `PROD-XHS-HTTP-ADAPTER-001`, `PROD-XHS-RAP-FIX-001`, `PROD-IMG-XHS-BROWSERLESS-REBUILD-001`, `PROD-PILLOW-12.3.0-FIX-001`, `PROD-IMG-XHS-PILLOW-REBUILD-001`, `PROD-BROWSERLESS-OS-VULN-DISPOSITION-001`, `PROD-BROWSERLESS-RESIDUAL-DECISION-001`, `PROD-BROWSERLESS-CONSTRAINT-PROOF-001`, or the completed `33d04d4` browserless build/scans. The only recommended next approval is `PROD-BROWSERLESS-VEX-REVIEW-001`; it must not implicitly authorize image build, ACR, deployment or production access.

## 10. Checkpoint condition

Application release `a635692a899ee02c6905cd694611c14e0da4594a` is the exact OCI revision of the three current local images and remains their only application provenance. Deployment-control release `253fde55fa609676a9fca5c14fdf6aeceef74000` and any later Handoff descendant are not OCI revisions. Current local IDs are not ACR digests and remain blocked at unsuppressed `4 Critical / 19 High`. The independently verified constraint proof closes the technical evidence gap for `libblkid` and `libacl` under the exact least-privilege production contract, but it does not suppress or formally disposition any scanner row. The next task is `PROD-BROWSERLESS-VEX-REVIEW-001`, not another image build, ACR push or deployment.
