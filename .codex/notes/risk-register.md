...
- 2026-07-27恢复点: legacy-ledger修复已在`e5883abc01c4b009907bee550209d7036d383771`完成checkpoint并正常push。新一轮受控执行已确认新鲜成功备份并只暂存精确执行材料；当前仅有一个可用的任务专用临时Super账户，API-C受保护目录保留加密凭据和精确源码，`runner.env=0`、维护容器=0、API/Admin仍active。缺失的registry evidence尚未传入API-C，已有import失败均发生在`_connect()`之前，因此数据库连接/事务/业务写入仍为0。该中间态不提高readiness；恢复时必须先补齐并hash校验该文件、通过零数据库import preflight，再允许最多一次生产事务，且取证后必须删除临时账户、任务目录和Cloud Shell文件。
