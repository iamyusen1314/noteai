...
## Confirmed Production Target (not yet implemented)
...
- ARCH-001 has placed the production runtime behind an SDK-independent `ClaudeTransport` with a behavior-compatible local Anthropic implementation. ARCH-002 now runs a verified minimal single-instance Gateway on Render Singapore Starter through a separate Blueprint; the existing full-stack Staging service intentionally remains on Local transport, so no business traffic has been cut over. Production 2–4 instance use remains blocked until shared atomic replay/rate state and the other `ARCH-002P` gates are complete.
- ARCH-002P is split into four serial gates. Gate A (outcome-safe retry/fallback, partial-stream termination, usage audit and cancellation/ASGI cleanup ownership) is VERIFIED and deployed only to the existing single-instance Staging Gateway at commit `b5d4b7e`. Remaining gates are: a Singapore HA shared atomic control plane for replay, logical operation state, global rate and renewable concurrency leases; exact host/remote readiness/timeout trust boundaries; and finally the 2→4 instance Render deployment/chaos rehearsal. A single Render Key Value is not an acceptable Production final design. The Gateway must make zero new Claude calls when the control plane is unavailable, while the Alibaba business system keeps user operations available through safe Kimi routing only for proven-not-dispatched work, durable queues, status/replay and reconciliation. The shared control plane stores only hashes, enums, timestamps and usage summaries and never stores prompts or generated content. Durable business-result replay remains a `BILL-002` Production prerequisite on the Alibaba main system.
- ARCH-002P-B now has a locally and independently verified DynamoDB implementation, but no AWS/Render resource is live yet. Each Router Claude leaf call owns a signed opaque operation ID that is stable across safe retry/fallback. DynamoDB mode uses a stable service principal for atomic nonce, monotonic single-item rate, non-expiring live operation state and renewable fenced slots; only terminal operations receive cleanup TTL. Anthropic is called only after a durable fenced provider-start transition. Render web identity is mandatory; static AWS keys and metadata fallback are rejected. SDK retries are disabled, control/provider deadlines are bounded, and lease duration exceeds provider deadline. The retained Singapore on-demand table and exact-subject least-privilege role are described by `infra/aws/claude_gateway_control_plane.yaml`; real AWS OIDC/table/budget and 2–4 instance validation are still required.
