...
- ARCH-002P is split into four serial gates. Gate A (outcome-safe retry/fallback, partial-stream termination, usage audit and cancellation/ASGI cleanup ownership) is locally VERIFIED but not deployed. Remaining gates are: a Singapore shared atomic control plane for replay, logical operation state, global rate and renewable concurrency leases; exact host/remote readiness/timeout trust boundaries; and finally the 2→4 instance Render deployment/chaos rehearsal. The shared control plane stores only hashes, enums, timestamps and usage summaries, fails closed with no local fallback, and never stores prompts or generated content. Durable business-result replay remains a separate `BILL-002` responsibility on the Alibaba main system.
