...
# V13 carries the core-validated cache-record hash into the fresh consumer
# check. The Dockerfile and full replay remain byte-for-byte frozen from V11.
...
then
  ...
  printf 'noteai_v11_retained_import_diagnostic='
  ...
  if [[ -f "${pre_predicate_diagnostic}" ]]; then
    ...
    printf 'noteai_v11_retained_pre_predicate_diagnostic='
  ...
  if [[ -f "${pair_summary}" ]]; then
    ...
    printf 'noteai_v11_retained_pair='
    jq -cS . "${pair_summary}"
  fi
  exit 3
fi
...
if [[ "${NOTEAI_IMPORT_MODE}" == "ci_portability" ]]; then
  ...
  jq -n \
    --arg schema_version "noteai.admin-dependency-cache-portability.v11" \
    ...
    --slurpfile pre_predicate_diagnostic \
    "${pre_predicate_diagnostic}" \
    --slurpfile import_diagnostic "${import_diagnostic}" \
    ...
    '{
      ...
      pre_predicate_diagnostic: $pre_predicate_diagnostic[0],
      import_diagnostic: $import_diagnostic[0],
