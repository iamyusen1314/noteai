...
# Export the exact dependency prefix plus the V11 zero-network anchor as a
# portable BuildKit local cache. The sibling import observer is present in the
# same byte-pinned Dockerfile but is outside the producer solve. This script
# never creates an application image, logs in, pushes, deploys or contacts a
...
jq -n \
  --arg schema_version "noteai.admin-dependency-cache-export.v11" \
  ...
  '{
    ...
    release: {
      ...
      minimal_context_files: [
        ...
        {path: "Dockerfile", sha256: $combined_dockerfile_sha256},
        {
