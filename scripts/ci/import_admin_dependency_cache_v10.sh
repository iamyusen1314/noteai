...
# V10 validates an imported cache through a consumer-only zero-network observer.
# The producer prefix and full replay remain byte-for-byte frozen.
...
expected_observer_prefix="8a2692fb460557e0159bc9e000364b515ce80aaf08eeeb186bf4930ce6357068"
...
printf '%s\n' \
  ...
  >> "${prefix_dockerfile}"
check_sha256 "${expected_observer_prefix}" "${prefix_dockerfile}"
test "$(wc -c < "${prefix_dockerfile}" | tr -d ' ')" = "5062"
test "$(wc -l < "${prefix_dockerfile}" | tr -d ' ')" = "83"
