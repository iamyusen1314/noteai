...
# V16 carries the core-validated cache-record hash into the fresh consumer
...
then
  ...
  printf 'noteai_v16_retained_import_identity='
  ...
  if [[ -f "${pair_summary}" ]]; then
    ...
    printf 'noteai_v16_retained_pair='
    jq -cS . "${pair_summary}"
  fi
  exit 3
fi
...
then
  if [[ -s "${replay_identity}" ]]; then
    ...
    printf 'noteai_v16_retained_replay_identity='
...
if [[ "${NOTEAI_IMPORT_MODE}" == "ci_portability" ]]; then
  ...
  jq -n \
    --arg schema_version "noteai.admin-dependency-cache-portability.v16" \
