...
for role in "${roles[@]}"; do
  ...
  cryptography_component_count="$(jq '[.components[]? | select(.name == "cryptography" and .version == "50.0.0")] | length' "${sbom}")"
  ...
  jq -n \
    ...
    --argjson cryptography_components "${cryptography_component_count}" \
    ...
    '{
      ...
      findings: {
        ...
        cryptography_components: $cryptography_components,
...
jq -n \
  --arg schema_version "noteai.native-release-evidence.v2" \
