...
# Export the exact dependency prefix plus the V16 zero-network anchor from one
...
expected_source_identity_fixture="2010890934f05c8ada09328c433962d5ae648346351accdeec62e1aafdb1f701"
...
if ! BUILDX_SEND_GIT_QUERY_AS_INPUT=0 \
...
then
  if [[ -s "${producer_identity}" ]]; then
    ...
    printf 'noteai_v16_retained_producer_identity='
...
jq -n \
  --arg schema_version "noteai.admin-dependency-cache-export.v16" \
