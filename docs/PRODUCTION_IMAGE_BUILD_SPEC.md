...
- The next production output is five browser-free role-specific `linux/amd64`
  images from the same exact commit: `api-runtime` for the public API,
  `admin-runtime` for Admin, `payment-runtime` for the isolated callback
  boundary, `ai-worker-runtime` for durable AI processing, and
  `xhs-http-runtime` shared by the separately configured market-timing and
  note-tracking services. The earlier three accepted ACR candidates predate
  the payment/AI targets and are not evidence for this next release.
  QEMU/cross-build output is not accepted as
...
- `payment-runtime` extends the browser-free base, starts only the dedicated
  callback app, defaults callback processing off, carries no writable data
  mount and listens on port 8002. It cannot start API/Admin/Worker commands.
- `ai-worker-runtime` extends the browser-free base, has no listener, defaults
  durable processing suspended and can run only the exact bounded worker
  command.
...
- Compose selects Docker targets directly. Render uses only the non-secret
  `NOTEAI_RUNTIME_TARGET` build argument (`api-runtime`, `admin-runtime`,
  `payment-runtime`, `ai-worker-runtime`, or `xhs-http-runtime`) and
  declares the matching `NOTEAI_RUNTIME_ROLE` at runtime. Never use a credential
  or configuration secret as a build argument.

After a new exact release commit is approved for build, build the five candidates
...
docker build --platform linux/amd64 --target payment-runtime \
  --build-arg NOTEAI_OCI_REVISION="$RELEASE_COMMIT" \
  --build-arg NOTEAI_OCI_SOURCE=https://github.com/iamyusen1314/noteai \
  --build-arg NOTEAI_OCI_VERSION="$PAYMENT_VERSION" \
  --build-arg NOTEAI_OCI_CREATED="$RELEASE_CREATED" \
  -t "$PAYMENT_LOCAL_TAG" .

docker build --platform linux/amd64 --target ai-worker-runtime \
  --build-arg NOTEAI_OCI_REVISION="$RELEASE_COMMIT" \
  --build-arg NOTEAI_OCI_SOURCE=https://github.com/iamyusen1314/noteai \
  --build-arg NOTEAI_OCI_VERSION="$AI_WORKER_VERSION" \
  --build-arg NOTEAI_OCI_CREATED="$RELEASE_CREATED" \
  -t "$AI_WORKER_LOCAL_TAG" .
...
- Payment permits only `/app/scripts/render_start_payment.sh`; callback
  processing remains disabled by default and the production runtime verifies
  the exact `noteai_payment` database role.
- AI Worker permits only the exact `python durable_ai_worker.py --once`
  command and cannot run API, Admin, payment or pre-deploy.
...
2. Build all five role-specific candidates locally on the approved native AMD64
...
3. Before any push, scan each local image filesystem, config and history for
   ...
   all five images have no Playwright, Chromium, browser launch
