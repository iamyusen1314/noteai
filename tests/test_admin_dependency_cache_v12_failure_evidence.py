...
import verify_admin_dependency_cache_v12_failure_evidence as evidence  # noqa: E402


class AdminDependencyCacheV12FailureEvidenceTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_diagnostic_and_cleanup_self_hashes_are_bound(self) -> None:
        failure = self.payload["failure"]
        self.assertEqual(
            evidence.diagnostic_payload_sha256(
                failure["producer_v9_diagnostic"]
            ),
            failure["producer_v9_diagnostic"]["diagnostic_sha256"],
        )
        self.assertEqual(
            evidence.diagnostic_payload_sha256(
                failure["producer_v11_diagnostic"]
            ),
            failure["producer_v11_diagnostic"]["diagnostic_sha256"],
        )
        self.assertEqual(
            evidence.cleanup_compact_payload_sha256(self.payload["cleanup"]),
            self.payload["cleanup"]["compact_log_payload_sha256"],
        )

    def test_each_terminal_boundary_mutation_fails_closed(self) -> None:
        cases = (
            ("run", ("github_run", "run_id"), 1),
            ("failure", ("failure", "primary_message"), "wrong"),
            (
                "diagnostic",
                ("failure", "producer_v11_diagnostic", "verdict"),
                "fail",
            ),
            (
                "source",
                (
                    ...
                    "cache_key_digest_and_vertex_digest_are_distinct_fields",
                ),
                False,
            ),
            ("cleanup", ("cleanup", "overall_pass"), False),
            ("ledger", ("run_ledger", "repository_run_count"), 469),
            ("artifact", ("provider_artifact", "api_total_count"), 1),
            ("log", ("native_job_log", "line_count"), 2930),
            ("ci", ("control_head_ci", "push_ci_test_count"), 0),
            (
                "authorization",
                ("authorization_outcome", "v12_attempt_may_not_be_rerun"),
                False,
            ),
    ...
    def test_source_inventory_cardinality_fails_closed(self) -> None:
        candidate = copy.deepcopy(self.payload)
        candidate["buildkit_source_review"]["sources"].pop()
        errors = evidence.verify(candidate, verify_git_state=False)
        self.assertIn("V12 BuildKit source review changed", errors)
    ...
    def test_exact_git_terminal_state_passes_after_checkpoint(self) -> None:
        ...
        if checkpoint is None:
            self.assertIn(
                "V12 terminal addition anchor is not exact",
                errors,
            )
