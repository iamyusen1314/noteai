...
CLEANUP = ROOT / "scripts" / "ci" / "cleanup_admin_dependency_cache_v3.sh"
TRANSIENT_VERIFIER = (
    ROOT / "tools" / "verify_admin_dependency_cache_transient_state_v3.py"
)


class AdminDependencyCacheCleanupV3Tests(unittest.TestCase):
    ...
    def _fixture(self, root: Path) -> tuple[Path, Path, Path, dict[str, str]]:
        ...
        docker_config = runner_temp / "noteai-empty-docker-config-v3"
        buildx_config = runner_temp / "noteai-buildx-state-v3"
        ...
        environment.update(
            {
                ...
                "NOTEAI_PRODUCER_BUILDER": "noteai-v3-producer",
                "NOTEAI_CONSUMER_BUILDER": "noteai-v3-consumer",
