...
import verify_admin_dependency_cache_export_plan_v7 as verifier  # noqa: E402


class AdminDependencyCacheExportPlanV7Tests(unittest.TestCase):
    @classmethod
    ...
    def test_exact_v7_plan_passes_before_and_after_activation(self) -> None:
    ...
    def test_v6_chain_and_terminal_receipt_are_frozen(self) -> None:
        self.assertEqual(verifier.v6_plan.validate_plan(), [])
        evidence = verifier.v6_failure.load_strict()
        self.assertEqual(verifier.v6_failure.verify(evidence), [])
        self.assertEqual(
            verifier.sha256_bytes(verifier.V6_REQUEST_PATH.read_bytes()),
            verifier.V6_REQUEST_SHA256,
        )
        self.assertEqual(
            verifier.sha256_bytes(
                verifier.V6_FAILURE_EVIDENCE_PATH.read_bytes()
            ),
            verifier.V6_FAILURE_EVIDENCE_SHA256,
        )
        self.assertEqual(
            verifier.v6_plan._request_additions(),
            [verifier.V6_CONTROL_COMMIT],
        )
    ...
    def test_workflow_timeout_and_permissions_fail_closed(self) -> None:
        ...
        for old, new, expected in cases:
            with self.subTest(expected=expected):
                ...
                self.assertIn("V7 workflow hash drift", errors)
    ...
    def test_trigger_branch_and_path_are_exact_singletons(self) -> None:
        cases = (
            ...
            (
                (
                    ...
                    b"admin-5335bda-dependency-cache-v7.json"
                ),
                (
                    b"    paths:\n"
                    b"      - .github/release-requests/"
        ...
        for old, new in cases:
            with self.subTest(old=old):
                ...
                self.assertIn("V7 workflow hash drift", errors)
                self.assertIn("V7 trigger block changed", errors)
    ...
    def test_deadline_wrappers_and_atomic_baseline_fail_closed(self) -> None:
        cases = (
            ...
            (
                (
                    b"    steps:\n"
                    b"      - name: Initialize V7 bounded job deadlines"
                ),
                (
                    b"    steps:\n"
                    b"      - name: Delayed V7 bounded job deadlines"
                ),
    ...
    def test_provenance_driver_option_matrix_fails_closed(self) -> None:
        ...
        for replacement in mutations:
            with self.subTest(replacement=replacement):
                ...
                errors = verifier.validate_plan(workflow_bytes=broken)
                self.assertIn("V7 isolated builder options changed", errors)
                self.assertIn("V7 provenance driver option changed", errors)
    ...
    def test_provenance_gate_shape_and_order_fail_closed(self) -> None:
        cases = (
            (
                ...
                'V7 provenance gate missing: "${root}"/.[!.]*.json',
            ),
            (
                b"</dev/null >/dev/null 2>&1",
                b"</dev/null",
                "V7 provenance gate missing: </dev/null >/dev/null 2>&1",
            ),
            (
                b'docker exec "${container_name}" sh -eu -c',
                b'docker exec "${NOTEAI_PRODUCER_BUILDER}" sh -eu -c',
                (
                    "V7 provenance gate missing: "
    ...
    def test_bypass_controls_are_forbidden(self) -> None:
        ...
        for replacement in mutations:
            with self.subTest(replacement=replacement):
                ...
                errors = verifier.validate_plan(workflow_bytes=broken)
                self.assertTrue(
                    "V7 workflow hash drift" in errors
                    and (
                        "V7 workflow forbidden token: "
                        in "\n".join(errors)
                        or "V7 workflow contract missing" in "\n".join(errors)
                        or "V7 max provenance setting changed" in errors
                    )
                )
    ...
    def test_transient_cleanup_and_fixture_mutations_fail_closed(self) -> None:
        ...
        self.assertIn(
            "V7 transient verifier hash drift",
        ...
        self.assertIn("V7 cleanup helper hash drift", errors)
        self.assertIn(
            "V7 cleanup contract missing: cleanup_effective",
            errors,
        )
        ...
        self.assertIn("V7 provenance source fixture hash drift", errors)
        self.assertIn(
        ...
        self.assertIn("V7 rawjson source fixture hash drift", errors)
        self.assertIn("V7 rawjson source projection changed", errors)
        ...
        self.assertIn("V7 structural source fixture hash drift", errors)
        self.assertIn("V7 structural source projection changed", errors)
        ...
        self.assertIn("V7 incremental source fixture hash drift", errors)
        self.assertIn("V7 incremental source projection changed", errors)
        for section, key in (
            ...
            with self.subTest(section=section, key=key):
                ...
                self.assertIn(
                    "V7 incremental source fixture hash drift",
                    errors,
                )
                self.assertIn(
    ...
    def test_four_layer_bundle_trust_matrix_fails_closed(self) -> None:
        cases = (
            (
    ...
    def test_outer_v7_and_frozen_inner_v5_namespaces_fail_closed(
        self,
    ) -> None:
        inner_cases = (
            (
                b"noteai-empty-docker-config-v5",
                b"noteai-empty-docker-config-v7",
            ),
            (
                b"noteai-buildx-state-v5",
                b"noteai-buildx-state-v7",
            ),
            (
                b"noteai-admin-cache-v5-producer-",
                b"noteai-admin-cache-v7-producer-",
            ),
            (
                b"noteai-docker-baseline-v5.complete",
                b"noteai-docker-baseline-v7.complete",
            ),
            (
                b"noteai-admin-dependency-cache-cleanup-v5.json",
                b"noteai-admin-dependency-cache-cleanup-v7.json",
            ),
        )
        ...
        outer_cases = (
            (
                ...
                b"admin-dependency-prefix-cache-5335bda-v7",
            ),
            (
                b"noteai-admin-dependency-cache-bundle-v7",
                b"noteai-admin-dependency-cache-bundle-v5",
            ),
            (
                b"unused-v7-import-evidence",
                b"unused-v5-import-evidence",
            ),
            (
                b"noteai-final-cache-v7-validation",
        ...
        for old, new in outer_cases:
            with self.subTest(outer=old):
                ...
                errors = verifier.validate_plan(workflow_bytes=broken)
                self.assertTrue(
                    "V7 outer workflow identity changed" in errors
                    or "V7 outer workflow retains V5 identity" in errors
                    or "V7 frozen provider artifact identity changed"
                    in errors
                )
    ...
    def test_authorized_caps_and_predecessor_chain_fail_closed(self) -> None:
        ...
        predecessor_cases = (
            verifier.V6_PLAN_PARENT,
            verifier.V6_CONTROL_COMMIT,
            verifier.V6_REQUEST_SHA256,
            verifier.V6_FAILURE_EVIDENCE_SHA256,
            verifier.V6_FAILURE_VERIFIER_SHA256,
            verifier.V6_WORKFLOW_SHA256,
            verifier.V6_TEMPLATE_SHA256,
            verifier.V6_PLAN_VERIFIER_SHA256,
    ...
    def test_template_predecessor_limits_and_authority_fail_closed(self) -> None:
        mutations = (
            ...
            (
                ...
                "artifact_name",
                "admin-dependency-prefix-cache-5335bda-v7",
            ),
        ...
        for parent, key, value in mutations:
            with self.subTest(parent=parent, key=key):
                ...
                self.assertIn("V7 template hash drift", errors)
                self.assertIn("V7 request template drift", errors)
    ...
    def test_parent_bound_active_request_passes_only_exactly(self) -> None:
        ...
        self.assertIn(
            "V7 active request does not bind its direct parent",
            errors,
        )
    ...
    def test_git_history_requires_one_request_only_addition_across_refs(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn(
                "V7 active request addition history is not unique",
                errors,
            )
