...
import collect_item26_manual_cost_stop_raw_v1 as collector  # noqa: E402
import extract_item26_manual_cost_stop_raw_v1 as extractor  # noqa: E402
from tests.test_extract_item26_manual_cost_stop_raw_v1 import (  # noqa: E402
    CLONE_ID,
    CONTROL_REVISION,
    SOURCE_ID,
    SOURCE_NAME,
...
class ManualCostStopCollectorTests(unittest.TestCase):
        ...
        self.activation_patch = mock.patch.object(
            ...
            return_value={
                ...
                "root_value": {
                    "ci": {
                        "issuer": (
                            "noteai-item26-manual-cost-stop-ci-v1"
                        ),
                        "audience": (
                            "noteai-item26-manual-cost-stop-verifier-v1"
                        ),
                    },
                },
                "authority_keys": {
                    "ci": (b"test-ci-public-key", "c" * 64),
                },
            },
        )
    ...
    def activation_receipt(self):
        def row(run_id, event, completed_at):
            return {
                "run_id": run_id,
                "job_id": run_id + 100,
                "event": event,
                ...
                "created_at_utc": "2026-08-16T23:59:58Z",
                "started_at_utc": "2026-08-16T23:59:59Z",
        ...
        payload = {
            ...
            "authority_root_file_sha256": "d" * 64,
            "source_file_sha256": {
                collector.COLLECTOR_REF: "a" * 64,
                collector.EXTRACTOR_REF: "b" * 64,
                collector.AUTHORITY_REF: "c" * 64,
                collector.authority.CI_WORKFLOW_REF: "f" * 64,
            },
            "control_ci": {
                "push": row(101, "push", "2026-08-17T00:00:01Z"),
                "pull_request": row(
                    102,
                    "pull_request",
        ...
        return {
            "authority": "ci",
            "issuer": "noteai-item26-manual-cost-stop-ci-v1",
            "audience": "noteai-item26-manual-cost-stop-verifier-v1",
            "payload": payload,
            "signature_base64": base64.b64encode(
                b"test-activation-signature"
            ).decode("ascii"),
        }
    ...
    def test_runtime_receipt_binds_a0_a1_root_and_source_blobs(self):
        ...
        receipt_path.write_bytes(
            collector._canonical(self.activation_receipt())
        )
        ...
        with mock.patch.object(
            ...
            digest = VALIDATE_RUNTIME_ACTIVATION(
                control_revision=CONTROL_REVISION,
                authority_root_file_sha256="d" * 64,
        ...
        for mutation in (
            ...
            with self.subTest(mutation=mutation):
                ...
                if mutation == "failed":
                    ...
                    receipt["payload"]["source_file_sha256"][
                ...
                with mock.patch.object(
                    ...
                    collector.authority,
                    "_verify_signature",
                    return_value=mutation != "signature",
                ), mock.patch.object(
                    collector,
                    "_utc_now",
                    return_value="2026-08-17T00:00:04Z",
                    ...
                    VALIDATE_RUNTIME_ACTIVATION(
                        ...
                        source_hashes=(
                            ...
                            if mutation == "installed_source_drift"
                            else self.source_hashes
                        ),
                        root_value={
                            "ci": {
                                "issuer": (
    ...
    def test_event_writer_recovers_partial_inside_timestamp_value(self):
        ...
        value = collector._event_value(
            ...
            payload=request,
            source_hashes={**self.source_hashes, "activation_receipt_sha256": "e" * 64},
    ...
    def test_production_style_import_isolated_and_writes_no_bytecode(self):
