...
import verify_item26_manual_cost_stop_evidence_v2 as verifier  # noqa: E402
import verify_item26_manual_cost_stop_authority_v2 as authority  # noqa: E402
import build_item26_manual_cost_stop_evidence_v2 as builder  # noqa: E402
from build_item26_manual_cost_stop_evidence_v2 import (  # noqa: E402
    build_checkpoint,
    build_evidence,
    build_receipt,
)
...
PRODUCTION_AUTHORITY_ROOT_SHA256 = (
...
def candidate():
    value = {
        ...
        "authority_generation_id": verifier.AUTHORITY_GENERATION_ID,
        "authority_epoch_id": verifier.AUTHORITY_EPOCH_ID,
        "kind": verifier.KIND,
        ...
        "raw_closure": {
            "authority_epoch_id": verifier.AUTHORITY_EPOCH_ID,
            "authority_root_file_sha256": "2" * 64,
            "authority_root_git_blob_sha256": "3" * 64,
...
class ManualCostStopEvidenceTests(unittest.TestCase):
    def setUp(self):
        ...
        builder_root_patcher = mock.patch.object(
            builder,
            "EXPECTED_AUTHORITY_ROOT_FILE_SHA256",
            "f" * 64,
        )
        builder_root_patcher.start()
        self.addCleanup(builder_root_patcher.stop)
    ...
    def test_v1_receipt_schema_is_not_accepted_by_v2(self):
        receipt = candidate()
        receipt["schema_version"] = 1
        receipt["schema"] = (
            "noteai.item26.manual-cost-stop-provider-receipt.v1"
        )
        receipt["terminal_acceptance_sha256"] = (
            verifier.terminal_acceptance_sha256(receipt)
        )
    ...
    def test_authority_epoch_drift_is_rejected(self):
        receipt = candidate()
        receipt["authority_epoch_id"] = "legacy-v1"
        receipt["terminal_acceptance_sha256"] = (
            verifier.terminal_acceptance_sha256(receipt)
        )
        ...
        self.assertIn("manual receipt authority_epoch_id mismatch", errors)
    ...
    def test_receipt_builder_uses_only_verified_projection(self):
        ...
        projection = SimpleNamespace(
            ...
            provider={
                ...
                "authority_epoch": verifier.AUTHORITY_EPOCH_ID,
                "authority_root_file_sha256": "2" * 64,
                "authority_root_git_blob_sha256": "3" * 64,
                "activation_receipt_schema": (
                    "noteai.item26.manual-cost-stop-runtime-activation-receipt.v3"
    ...
    def test_default_terminal_authority_fails_before_read(self):
        ...
        self.assertTrue(errors)
        self.assertIn("external authority is not finalized", errors[0])
        self.assertIsNone(binding)
        read_artifact.assert_not_called()

    def test_candidate_builder_fails_closed_while_root_v2_is_absent(self):
        receipt = candidate()
        receipt["terminal_acceptance_sha256"] = (
            verifier.terminal_acceptance_sha256(receipt)
        )
        with mock.patch.object(
            ...
            ValueError,
            "external authority is not finalized",
        ):
            build_evidence(
                receipt,
                expected_control_revision=control_revision,
    ...
    def terminal_fixture(self):
        ...
        authority_binding = {
            ...
            "authority_root_file_sha256": receipt["raw_closure"][
                "authority_root_file_sha256"
            ],
            "authority_root_git_blob_sha256": receipt["raw_closure"][
    ...
    def test_terminal_artifacts_bind_git_stages_and_raw_authority(self):
        ...
        self.assertEqual(
            ...
            verifier.AUTHORITY_GENERATION_ID,
        )
        self.assertEqual(
            binding["authority_epoch_id"], verifier.AUTHORITY_EPOCH_ID
        )
        self.assertEqual(
            binding["authority_root_git_blob_sha256"], "3" * 64
        )
        self.assertEqual(
            binding["activation_receipt_sha256"], "8" * 64
        )
