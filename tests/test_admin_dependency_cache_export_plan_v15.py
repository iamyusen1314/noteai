...
import verify_admin_dependency_cache_export_plan_v15 as verifier  # noqa: E402
...
class AdminDependencyCacheExportPlanV15Tests(unittest.TestCase):
    @staticmethod
    def _template() -> dict:
    ...
    def test_reviewed_authorities_validate_without_git_state(self):
        ...
        self.assertEqual(verifier._validate_v14_superseded_predecessor(), [])

    def test_v14_predecessor_uses_only_narrow_frozen_semantics(self):
        v14 = verifier._load_v14_plan_verifier()
        v14_paths = {
            verifier.V14_WORKFLOW_FILE.resolve(),
            verifier.V14_TEMPLATE_PATH.resolve(),
            verifier.V14_PLAN_TEST_PATH.resolve(),
            verifier.V14_PLAN_VERIFIER_PATH.resolve(),
        }
        observed_modes: list[tuple[Path, int | None]] = []
        read_frozen_bytes = verifier._read_frozen_bytes

        def record_mode(path: Path, **kwargs):
            resolved = path.resolve()
            if resolved in v14_paths:
                observed_modes.append((resolved, kwargs.get("required_mode")))
            return read_frozen_bytes(path, **kwargs)

        forbidden = (
            "validate_plan",
            "_validate_frozen_worktree",
            "_validate_same_anchor",
            "plan_state",
        )
        patches = [
            mock.patch.object(
                v14,
                name,
                side_effect=AssertionError(f"forbidden V14 call: {name}"),
            )
        ...
        with mock.patch.object(
            ...
            v14,
            "_strict_json_bytes",
            wraps=v14._strict_json_bytes,
        ) as strict_json:
            self.assertEqual(verifier._validate_v14_superseded_predecessor(), [])
        strict_json.assert_called_once()
        self.assertEqual({path for path, _mode in observed_modes}, v14_paths)
        self.assertTrue(
            all(mode == 0o644 for _path, mode in observed_modes),
            observed_modes,
        )

    def test_v14_predecessor_rejects_current_authority_mode_drift(self):
        with tempfile.TemporaryDirectory() as temporary:
            workflow = Path(temporary) / verifier.V14_WORKFLOW_FILE.name
            workflow.write_bytes(verifier.V14_WORKFLOW_FILE.read_bytes())
            workflow.chmod(0o600)
            with mock.patch.object(verifier, "V14_WORKFLOW_FILE", workflow):
                errors = verifier._validate_v14_superseded_predecessor()
        self.assertTrue(
            any("V14 workflow file contract changed" in error for error in errors),
            errors,
        )
    ...
    def test_template_fail_closed_matrix(self):
        for name, mutate in (
            (
                "predecessor",
                lambda value: value["predecessor"].update(workflow_run_count=1),
            ),
            (
                "terminal-hash",
                lambda value: value["predecessor"].update(
                    plan_verifier_sha256="0" * 64
                ),
            ),
            (
                "supersession-reason",
                lambda value: value["predecessor"].update(
                    supersession_reason="UNREVIEWED"
                ),
            ),
            (
                "checkpoint-shape",
        ...
        for name, candidate in (
            ...
            (
                ...
                workflow.replace(
                    ...
                    "NOTEAI_V15_LEDGER_CONTRACT_PATH_REMOVED: true",
                ),
            ),
            (
    ...
    def test_live_ledger_pre_and_post_cleanup_snapshots(self):
        ...
        self.assertEqual(len(pre["legacy"]), 13)
        self.assertEqual(pre["legacy"][9]["runs"], [])
        self.assertEqual(
            pre["legacy"][10]["runs"][0]["id"],
            30696298423,
        )
        self.assertEqual(pre["legacy"][11]["version"], "v13")
        self.assertEqual(pre["legacy"][11]["runs"], [])
        self.assertEqual(pre["legacy"][12]["version"], "v14")
    ...
    def test_live_ledger_rejects_history_and_current_drift(self):
        ...
        v13.extra_repository_runs.append(
            {"id": 126, "path": verifier.V13_WORKFLOW_PATH}
        )
        cases.append(("v13-run", v13))

        v14 = FakeLedger()
        v14.extra_repository_runs.append(
            {"id": 125, "path": verifier.V14_WORKFLOW_PATH}
        )
        cases.append(("v14-run", v14))

        v15 = FakeLedger()
        v15.extra_repository_runs.append(
            {"id": 125, "path": verifier.V15_WORKFLOW_PATH}
        )
        cases.append(("v15-rerun", v15))
    ...
    def test_live_ledger_contract_copy_requires_private_mode(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                mock.patch.dict(
                    os.environ,
                    {"NOTEAI_V15_LEDGER_CONTRACT_PATH": str(contract)},
                ),
    ...
    def test_exact_checkpoint_receipt_and_activation_git_history(self):
        with (
            ...
            git("init", "--quiet")
            git("config", "user.name", "NoteAI V15 Test")
            git("config", "user.email", "noteai-v15@example.invalid")
            ...
            with mock.patch.object(verifier, "PREDECESSOR_COMMIT", base):
                ...
                with mock.patch.dict(
                    ...
                    self.assertEqual(
                        verifier._canonical_branch_head(root),
                        activation,
                    )
                    self.assertEqual(
    ...
    def test_canonical_head_projects_pr_second_parent(self):
        merge = "a" * 40
        second_parent = "c" * 40
        with (
            mock.patch.object(verifier, "_git_at", return_value=merge),
            mock.patch.object(
                verifier,
                "_commit_parents",
                return_value=["b" * 40, second_parent],
            ),
            mock.patch.dict(
                os.environ,
                {
    ...
    def test_python_optimized_mode_has_no_assert_contract(self):
        ...
        self.assertNotIn("__V15_", source)
