...
def provider_capture():
    ...
    return {
        ...
        "authority_epoch": extractor.AUTHORITY_EPOCH_ID,
        "authority_root_file_sha256": "f" * 64,
        "authority_root_git_blob_sha256": "f" * 64,
        "activation_receipt_schema": (
            extractor.EXPECTED_ACTIVATION_RECEIPT_SCHEMA
        ),
...
class ManualCostStopRawExtractorTests(unittest.TestCase):
    def test_history_and_v2_authority_generation_are_distinct(self):
        self.assertEqual(
            extractor.HISTORICAL_OPERATION_ID,
            extractor.TASK_ID + ":MANUAL-POST-ACTION-COST-STOP:v1",
        )
        self.assertEqual(
            extractor.AUTHORITY_GENERATION_ID,
            extractor.TASK_ID
            + ":MANUAL-POST-ACTION-COST-STOP-AUTHORITY:v2",
        )
        self.assertEqual(
            extractor.OPERATION_ID,
            extractor.HISTORICAL_OPERATION_ID,
        )
        self.assertEqual(
            extractor.CAPTURE_OPERATION_ID,
            extractor.AUTHORITY_GENERATION_ID,
        )
    ...
    def test_v1_schema_epoch_and_receipt_v2_are_rejected(self):
        mutations = {
            "v1_schema": (
                "schema",
                "noteai.item26.manual-cost-stop-provider-raw.v1",
            ),
            "v1_epoch": (
                "authority_epoch",
                "noteai.item26.manual-cost-stop-authority-generation.v1",
            ),
            "receipt_v2": (
                "activation_receipt_schema",
