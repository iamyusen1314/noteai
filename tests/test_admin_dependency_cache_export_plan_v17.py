            ...
            return self._page(
                [
                    ...
                    },
                    {
                        ...
                        "path": verifier.V17_WORKFLOW_PATH,
                    },
...
class AdminDependencyCacheExportPlanV17Tests(unittest.TestCase):
    ...
    def test_exact_checkpoint_receipt_activation_and_authority_sets(self) -> None:
        self.assertEqual(len(verifier.INERT_CHECKPOINT_FILES), 18)
        self.assertEqual(len(verifier.INERT_RECEIPT_FILES), 4)
        self.assertEqual(len(verifier.CHECKPOINT_IMMUTABLE_FILES), 14)
        self.assertEqual(len(verifier.SAME_ANCHOR_FILES), 11)
        self.assertEqual(
            set(verifier.CHECKPOINT_IMMUTABLE_FILES),
            set(verifier.INERT_CHECKPOINT_FILES)
            - set(verifier.INERT_RECEIPT_FILES),
        )
        self.assertEqual(verifier.PREDECESSOR_COMMIT, verifier.V16_TERMINAL_RECEIPT)

    def test_reviewed_static_authorities_and_v16_predecessor_validate(self) -> None:
        self.assertEqual(verifier.validate_plan(verify_git_state=False), [])
        self.assertEqual(verifier._validate_v16_predecessor(), [])

    def test_template_fail_closed_matrix(self) -> None:
        template = self._template()
        self.assertEqual(verifier._template_errors(template), [])
        mutations = (
            ("schema", lambda value: value.__setitem__("schema_version", 16)),
            (
                "predecessor",
            ...
            (
                ...
                lambda value: value["recovery_basis"].__setitem__(
                    "v17_import_helper_sha256", "0" * 64
                ),
            ),
            (
                "checkpoint-count",
                lambda value: value["control_plane"].__setitem__(
                    "inert_checkpoint_exact_changed_path_count", 17
                ),
            ),
            (
                "ledger-v16",
    ...
    def test_plan_state_classifier(self) -> None:
        ...
        self.assertEqual(
            ...
            "V17_CONSUMED_OR_INVALID",
        )

    def test_same_anchor_enforces_exact18_and_immutable14(self) -> None:
        anchor = "b" * 40
    ...
    def test_live_ledger_pre_and_post_cleanup_snapshots(self) -> None:
        ...
        self.assertEqual(len(pre["legacy"]), 15)
        ...
        self.assertEqual(pre["legacy"][14]["runs"][0]["id"], 30739167701)
    ...
    def test_live_ledger_rejects_v15_or_v16_rerun_and_current_artifact(self) -> None:
        v15 = FakeLedger()
        v15.extra_repository_runs.append(
            {"id": 30724578320, "path": verifier.V15_WORKFLOW_PATH}
        )
        v16 = FakeLedger()
        v16.extra_repository_runs.append(
            {"id": 30739167702, "path": verifier.V16_WORKFLOW_PATH}
        )
        artifact = FakeLedger()
    ...
    def test_live_ledger_contract_copy_requires_private_mode(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                mock.patch.dict(
                    os.environ,
                    {"NOTEAI_V17_LEDGER_CONTRACT_PATH": str(contract)},
                ),
    ...
    def test_python_optimized_mode_keeps_static_checks(self) -> None:
        command = (
            ...
            "import verify_admin_dependency_cache_export_plan_v17 as v; "
