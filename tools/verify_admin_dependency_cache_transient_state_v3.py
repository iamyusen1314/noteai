#!/usr/bin/env python3
"""Verify task-owned Docker/Buildx client state before V3 cache cleanup."""
...
BUILDER_NAME_RE = re.compile(
    r"^noteai-admin-cache-v3-(?:producer|consumer)-[0-9]+$"
)
...
def _validate_node_group(payload: bytes, label: str) -> None:
    ...
    require(node["Flags"] in (None, []), f"{label}.flags changed")
