#!/usr/bin/env python3
"""Verify the append-only V15 Admin dependency-cache successor plan."""
...
WORKFLOW_PATH = ROOT / ".github" / "workflows" / "admin-dependency-cache-export-v15.yml"
CI_WORKFLOW_PATH = ROOT / ".github" / "workflows" / "ci.yml"
TEMPLATE_PATH = ROOT / "deploy" / "production" / "plans" / "admin-dependency-cache-export-request-v15.json"
ACTIVE_REQUEST_PATH = ROOT / ".github" / "release-requests" / "admin-5335bda-dependency-cache-v15.json"
...
PLAN_TEST_PATH = ROOT / "tests" / "test_admin_dependency_cache_export_plan_v15.py"
...
V14_WORKFLOW_FILE = ROOT / ".github" / "workflows" / "admin-dependency-cache-export-v14.yml"
V14_TEMPLATE_PATH = ROOT / "deploy" / "production" / "plans" / "admin-dependency-cache-export-request-v14.json"
V14_PLAN_TEST_PATH = ROOT / "tests" / "test_admin_dependency_cache_export_plan_v14.py"
V14_PLAN_VERIFIER_PATH = ROOT / "tools" / "verify_admin_dependency_cache_export_plan_v14.py"
...
V15_TERMINAL_VERIFIER_PATH = (
    ROOT / "tools" / "verify_admin_dependency_cache_v15_failure_evidence.py"
)
V15_TERMINAL_TEST_PATH = (
    ROOT / "tests" / "test_admin_dependency_cache_v15_failure_evidence.py"
)
V15_EVIDENCE_PATH = ROOT / "deploy" / "production" / "evidence" / (
    "admin-dependency-cache-v15-attempt1-failed-20260802.json"
)

WORKFLOW_SHA256 = "33a57116c7462bb4f73f48f0d94f41158b8bf676886c19518610d172fc6ba457"
TEMPLATE_SHA256 = "b89820b318f98363b768a43663bdf6c730a4d78894c3d1c28547fdb86e42d777"
...
PLAN_TEST_SHA256 = "e9a4520cbbf859a242d2240325be82dd2ff25f5a379ded84ad654b631c7f7e8a"
CI_WORKFLOW_SHA256 = "d4830c55563e18304c60c04cbe729e9d6c657c2fb5caa61d983eef50fe8a28e9"
V14_WORKFLOW_SHA256 = "98850e28f74d5b4d4bafacffed07d118cba3378c81a0c93457cb46dc83ac7db1"
V14_TEMPLATE_SHA256 = "9fd2f7d07eb7d9495ba4b27b2b71e6df650d8ef6dd6f599fb8a0da2edeadb4ca"
V14_PLAN_TEST_SHA256 = "f908bf33fda2e6c49e95fda43589a9f3376aec9681a038245b12ac55d9055a7d"
V14_PLAN_VERIFIER_SHA256 = "940c86bf337eb23307e3a2bbb8ce12724f984a8064c5419a17244fb8ada1991a"
V14_CI_WORKFLOW_SHA256 = "cf21455c03887657ecafb0c4a224144bf4fbe63c3a5de204ffdb31fd0480201e"
...
V14_INERT_CHECKPOINT = "e18d24a204c33127a95fe3035b7fce43bdb0b4f8"
V14_FAILURE_RECEIPT = "1ca885d61c48f3cfdb4e99eeedc6fb9f17238bb4"
PREDECESSOR_COMMIT = V14_FAILURE_RECEIPT
...
LEDGER_SCHEMA = "noteai.admin-dependency-cache-ledger.v15"
...
LEGACY_LEDGER_SHA256 = "056103160f2ba8521032073ae7b5e523e82716ba1ae264b2a6b44a788ce30fe9"
...
V14_CORE_AUTHORITIES = {
    Path(".github/workflows/admin-dependency-cache-export-v14.yml"): (
        V14_WORKFLOW_SHA256
    ),
    Path("deploy/production/plans/admin-dependency-cache-export-request-v14.json"): (
        V14_TEMPLATE_SHA256
    ),
    Path("tests/test_admin_dependency_cache_export_plan_v14.py"): (
        V14_PLAN_TEST_SHA256
    ),
    Path("tools/verify_admin_dependency_cache_export_plan_v14.py"): (
        V14_PLAN_VERIFIER_SHA256
    ),
}

V14_CHECKPOINT_FILES = tuple(Path(value) for value in (
    ".codex/handoffs/current-task.md",
    ".codex/notes/risk-register.md",
...
def _load_v12_terminal_verifier():
    ...
    module_name = "_noteai_v12_terminal_for_v15"
...
def _load_v15_terminal_verifier():
    source = _read_frozen_bytes(
        V15_TERMINAL_VERIFIER_PATH,
        label="V15 terminal verifier",
    )
    if hashlib.sha256(source).hexdigest() != V15_TERMINAL_VERIFIER_SHA256:
        raise ValueError("V15 terminal verifier hash drift")
    module_name = "_noteai_v15_terminal_for_v15"
    module = types.ModuleType(module_name)
    module.__file__ = str(V15_TERMINAL_VERIFIER_PATH)
    module.__package__ = ""
    sys.modules[module_name] = module
    try:
        exec(
...
def _load_v13_plan_verifier():
    ...
    module_name = "_noteai_v13_plan_for_v15"
    module = types.ModuleType(module_name)
...
def _load_v14_plan_verifier():
    source = _read_frozen_bytes(
        V14_PLAN_VERIFIER_PATH,
        label="V14 plan verifier",
        required_mode=0o644,
    )
    if hashlib.sha256(source).hexdigest() != V14_PLAN_VERIFIER_SHA256:
        raise ValueError("V14 plan verifier hash drift")
    module_name = "_noteai_v14_plan_for_v15"
    module = types.ModuleType(module_name)
    module.__file__ = str(V14_PLAN_VERIFIER_PATH)
    module.__package__ = ""
    sys.modules[module_name] = module
    try:
        exec(
            compile(source, str(V14_PLAN_VERIFIER_PATH), "exec"),
            module.__dict__,
        )
    except BaseException:
        sys.modules.pop(module_name, None)
...
def _validate_v14_superseded_predecessor(*, root: Path = ROOT) -> list[str]:
    errors = _validate_v13_superseded_predecessor(root=root)
    try:
        if root.resolve() == ROOT.resolve():
            v14 = _load_v14_plan_verifier()
            ...
            template = v14._strict_json_bytes(template_bytes, "V14 template")
            if not isinstance(template, dict):
                errors.append("V14 frozen template root changed")
            else:
                semantic_errors = v14._template_errors(template)
                if semantic_errors:
                    errors.append(
                        "V14 frozen template validation failed: "
                        + "; ".join(semantic_errors[:5])
                    )
            workflow_errors = v14._workflow_errors(
                workflow_bytes.decode("utf-8")
            )
            if workflow_errors:
                errors.append(
                    "V14 frozen workflow validation failed: "
                    + "; ".join(workflow_errors[:5])
                )
        if _commit_parents(root, V14_INERT_CHECKPOINT) != [V13_FAILURE_RECEIPT]:
            errors.append("V14 inert checkpoint parent changed")
        if _changed_paths(
            root,
            V13_FAILURE_RECEIPT,
            V14_INERT_CHECKPOINT,
        ) != set(V14_CHECKPOINT_FILES):
            errors.append("V14 inert checkpoint is not the exact 11-file delta")
        for relative in V14_CHECKPOINT_FILES:
            entry = _tree_entry(root, V14_INERT_CHECKPOINT, relative).split()
            if len(entry) < 3 or entry[0:2] != ["100644", "blob"]:
                errors.append(f"V14 inert checkpoint mode changed: {relative}")
        historical_ci = subprocess.run(
            [
                "git",
                "show",
                f"{V14_INERT_CHECKPOINT}:.github/workflows/ci.yml",
            ],
            cwd=root,
            check=True,
            stdout=subprocess.PIPE,
        ...
        if hashlib.sha256(historical_ci).hexdigest() != V14_CI_WORKFLOW_SHA256:
            errors.append("V14 historical CI hash drift")
        if _commit_parents(root, V14_FAILURE_RECEIPT) != [V14_INERT_CHECKPOINT]:
            errors.append("V14 failure receipt parent changed")
        if _changed_paths(
            root,
            V14_INERT_CHECKPOINT,
            V14_FAILURE_RECEIPT,
        ) != set(INERT_RECEIPT_FILES):
            errors.append("V14 failure receipt is not the exact 4-file delta")
        for relative in INERT_RECEIPT_FILES:
            entry = _tree_entry(root, V14_FAILURE_RECEIPT, relative).split()
            if len(entry) < 3 or entry[0:2] != ["100644", "blob"]:
                errors.append(f"V14 failure receipt mode changed: {relative}")
        branch_head = _canonical_branch_head(root)
        if not _on_first_parent_chain(root, V14_FAILURE_RECEIPT, head=branch_head):
            errors.append("V14 failure receipt is not on first-parent chain")
        for relative, expected in V14_CORE_AUTHORITIES.items():
            payload = _read_frozen_bytes(
                root / relative,
                label=f"V14 authority {relative}",
                required_mode=0o644,
            )
            if hashlib.sha256(payload).hexdigest() != expected:
                errors.append(f"V14 superseded authority hash drift: {relative}")
            additions = _true_additions(root=root, path=relative)
            if additions != [V14_INERT_CHECKPOINT]:
                errors.append(f"V14 authority addition history changed: {relative}")
            if _lineage_path_changes(
                root=root,
                anchor=V14_INERT_CHECKPOINT,
                paths=(relative,),
                head=branch_head,
            ):
                errors.append(f"V14 authority changed after checkpoint: {relative}")
        if _true_additions(
            root=root,
            path=Path(
                ".github/release-requests/"
                "admin-5335bda-dependency-cache-v14.json"
            ),
        ):
            errors.append("V14 request history is not zero")
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        errors.append(f"cannot verify V14 superseded predecessor: {exc}")
    return errors
...
def _authority_anchor(
    *,
    ...
    if len(present) != len(SAME_ANCHOR_FILES):
        return None, ["V15 frozen authorities are only partially present"]
    anchors: list[str] = []
    for path in SAME_ANCHOR_FILES:
        additions = _true_additions(root=root, path=path)
        if len(additions) != 1:
            errors.append(f"V15 frozen addition history is not unique: {path}")
        else:
            anchors.append(additions[0])
    if errors:
        return None, errors
    if len(set(anchors)) != 1:
        return None, ["V15 frozen authorities have different add anchors"]
    return anchors[0], []
...
def _validate_same_anchor(*, root: Path = ROOT) -> list[str]:
    ...
    try:
        ...
        if anchor is None:
            errors.append("V15 inert checkpoint is missing")
            return errors
        ...
        if _changed_paths(root, PREDECESSOR_COMMIT, anchor) != set(
            ...
            errors.append("V15 inert checkpoint is not the exact 11-file delta")
        if not _on_first_parent_chain(root, anchor, head=branch_head):
            errors.append("V15 inert checkpoint is not on first-parent chain")
        for relative in INERT_CHECKPOINT_FILES:
            entry = _tree_entry(root, anchor, relative).split()
            if len(entry) < 3 or entry[0:2] != ["100644", "blob"]:
                errors.append(f"V15 inert checkpoint mode changed: {relative}")
        if _lineage_path_changes(
            root=root,
            anchor=anchor,
            ...
            errors.append("V15 immutable checkpoint authority changed")
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        errors.append(f"cannot verify V15 inert checkpoint: {exc}")
    return errors
    ...
    *,
    ...
    try:
        ...
        if not changes:
            return None, []
        if len(changes) != 1:
            return None, ["V15 inert receipt history is not unique"]
        receipt = changes[0]
        if _commit_parents(root, receipt) != [anchor]:
            errors.append("V15 inert receipt is not the direct child of anchor")
        elif _changed_paths(root, anchor, receipt) != set(INERT_RECEIPT_FILES):
            errors.append("V15 inert receipt is not the exact 4-file delta")
        for relative in INERT_RECEIPT_FILES:
            entry = _tree_entry(root, receipt, relative).split()
            if len(entry) < 3 or entry[0:2] != ["100644", "blob"]:
                errors.append(f"V15 inert receipt mode changed: {relative}")
        if not _on_first_parent_chain(root, receipt, head=branch_head):
            errors.append("V15 inert receipt is not on first-parent chain")
        return receipt, errors
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        return None, [f"cannot verify V15 inert receipt: {exc}"]
...
def _active_request(
    *,
    ...
    try:
        ...
        payload = _read_frozen_bytes(
            path,
            label="V15 active request",
            required_mode=0o644,
        )
        value = _strict_json_bytes(payload, "V15 active request")
        if not isinstance(value, dict):
            return True, None, ["V15 active request root changed"]
        return True, value, []
    except (OSError, ValueError) as exc:
        return True, None, [f"cannot load V15 active request: {exc}"]
...
def _validate_active_git(
    ...
    *,
    ...
    try:
        ...
        additions = _true_additions(root=root, path=relative)
        if len(additions) != 1:
            return ["V15 active request addition history is not unique"]
        activation = additions[0]
        if not _on_first_parent_chain(root, activation, head=branch_head):
            errors.append("V15 activation is not on first-parent chain")
        parents = _commit_parents(root, activation)
        if len(parents) != 1:
            return ["V15 activation is not single-parent"]
        parent = parents[0]
        receipt, receipt_errors = _validate_inert_receipt(root=root)
        errors.extend(receipt_errors)
        if receipt is None or parent != receipt:
            errors.append("V15 activation parent is not the exact receipt")
        if active.get("plan_checkpoint_commit") != parent:
            errors.append("V15 active request does not bind direct parent")
        if _changed_paths(root, parent, activation) != {relative}:
            errors.append("V15 activation changes more than its request")
        status = _git_at(
            root,
            "diff-tree",
            "--no-commit-id",
        ...
        if status != f"A\t{relative.as_posix()}":
            errors.append("V15 request was not a unique addition")
        entry = _tree_entry(root, activation, relative).split()
        if len(entry) < 3 or entry[0:2] != ["100644", "blob"]:
            errors.append("V15 request activation mode changed")
        template = _read_frozen_bytes(TEMPLATE_PATH, label="V15 template")
        ...
        if payload != expected:
            errors.append("V15 activation request differs from template")
        if _lineage_path_changes(
            root=root,
            anchor=activation,
            paths=(relative,),
            ...
            errors.append("V15 active request changed after activation")
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        errors.append(f"cannot verify V15 active request Git state: {exc}")
    return errors
    ...
    *,
    ...
    try:
        ...
        return [] if branch_head == expected else [
            "V15 canonical branch HEAD is not the latest exact stage"
        ]
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        return [f"cannot verify V15 latest exact stage: {exc}"]
...
def _template_errors(template: dict[str, Any]) -> list[str]:
    ...
    require(template.get("schema_version") == 15, "V15 schema changed")
    require(template.get("task") == "PROD-FIRST-LAUNCH-ADMIN-INTERNAL-001", "V15 task changed")
    require(template.get("release_commit") == RELEASE_COMMIT, "V15 release changed")
    require(template.get("plan_checkpoint_commit") == "__DIRECT_PARENT_COMMIT__", "V15 parent placeholder changed")
    require(template.get("trigger_mode") == "one_shot_added_request_v15", "V15 trigger mode changed")
    predecessor = template.get("predecessor")
    require(isinstance(predecessor, dict), "V15 predecessor missing")
    if isinstance(predecessor, dict):
        require(
            predecessor == {
                "state": "V14_INERT_CHECKPOINT_PR_CONTEXT_FAILED_RECEIPT_EXACT",
                "checkpoint_parent_commit": V13_FAILURE_RECEIPT,
                "inert_checkpoint_commit": V14_INERT_CHECKPOINT,
                "failure_receipt_commit": V14_FAILURE_RECEIPT,
                "workflow_path": V14_WORKFLOW_PATH,
                "workflow_sha256": V14_WORKFLOW_SHA256,
                "template_sha256": V14_TEMPLATE_SHA256,
                "plan_verifier_sha256": V14_PLAN_VERIFIER_SHA256,
                "plan_test_sha256": V14_PLAN_TEST_SHA256,
                "ci_workflow_sha256": V14_CI_WORKFLOW_SHA256,
                "active_request_path": (
                    ".github/release-requests/"
                    "admin-5335bda-dependency-cache-v14.json"
                ),
                ...
                "ordinary_ci_failure_count": 1,
                "push_ci_run_id": 30706546764,
                "push_ci_job_id": 91386553347,
                "push_ci_conclusion": "success",
                "pull_request_ci_run_id": 30706547955,
                "pull_request_ci_job_id": 91386556528,
                "pull_request_ci_conclusion": "failure",
                "receipt_created": True,
                "activation_created": False,
                "supersession_reason": (
                    "MODULE_WIDE_GITHUB_CONTEXT_ISOLATION_BROKE_"
                    "PR_PREDECESSOR_PROJECTION"
                ),
            },
            "V15 V14-supersession predecessor contract changed",
        )
    recovery = template.get("recovery_basis")
    require(isinstance(recovery, dict), "V15 recovery basis missing")
    ...
    require(
        ...
        and control.get("v14_pr_context_failure_receipt_required") is True
        and control.get("v14_historical_tree_validation_required") is True
        and control.get("v15_current_attempt1_artifact0_required") is True
        and control.get("run_ledger_snapshot_count") == 2,
        "V15 control-plane contract changed",
    )
    ledger = template.get("run_ledger_contract")
    legacy = ledger.get("legacy") if isinstance(ledger, dict) else None
    v11 = legacy[9] if isinstance(legacy, list) and len(legacy) == 13 else None
    v12 = legacy[10] if isinstance(legacy, list) and len(legacy) == 13 else None
    v13 = legacy[11] if isinstance(legacy, list) and len(legacy) == 13 else None
    v14 = legacy[12] if isinstance(legacy, list) and len(legacy) == 13 else None
    ...
    require(
        ...
        and ledger.get("current", {}).get("version") == "v15"
        and ledger.get("current", {}).get("workflow_path") == V15_WORKFLOW_PATH,
        "V15 run-ledger contract changed",
    )
    ...
    require(
        ...
        "V15 cache evidence contract changed",
    )
    require(
        ...
        "V15 authorization boundary changed",
    )
    return errors
...
def _workflow_errors(workflow: str) -> list[str]:
    errors: list[str] = []
    header = f"""name: Admin dependency cache export V15
...
"""
    if not workflow.startswith(preamble):
        errors.append("V15 workflow trigger header changed")
    job_keys = re.findall(
        r"^  ([A-Za-z0-9_-]+):[ \t]*$",
        workflow.split("\njobs:\n", 1)[1]
        if "\njobs:\n" in workflow
    ...
    if job_keys != ["export-cache"]:
        errors.append("V15 workflow job inventory changed")
    if "\n    permissions:" in workflow:
        errors.append("V15 workflow job-level permissions changed")
    required = (
        "permissions:\n  contents: read\n  actions: read",
        "group: admin-dependency-cache-export-v10",
        "cancel-in-progress: false",
        "timeout-minutes: 120",
        "Prove this is the unique V15 workflow run",
        "Snapshot V2 through V15 ledgers before resource creation",
        "Export V15 dependency-only BuildKit local cache",
        "Prove V15 fresh-consumer import and cacheless replay",
        "Remove V15 builders and all transient Docker state",
        "Snapshot V2 through V15 ledgers after cleanup and before upload",
        "Validate final V15 portable bundle after cleanup",
        "Upload one-day V15 public-repository dependency cache artifact",
        "scripts/ci/import_admin_dependency_cache_v13.sh",
        "tools/verify_admin_dependency_cache_bundle_v13.py",
        "tools/verify_admin_dependency_cache_export_plan_v15.py",
        'ledger_contract_copy="${RUNNER_TEMP}/noteai-cache-v15-ledger-contract.json"',
        'cp "${NOTEAI_REQUEST_TEMPLATE}" "${ledger_contract_copy}"',
        "NOTEAI_V15_LEDGER_CONTRACT_PATH: ${{ steps.control.outputs.ledger_contract_copy }}",
        ...
        "v14_run_zero_required == true",
        "v14_pr_context_failure_receipt_required == true",
        "v14_historical_tree_validation_required == true",
        "V14_INERT_CHECKPOINT_PR_CONTEXT_FAILED_RECEIPT_EXACT",
        "NOTEAI_V14_INERT_CHECKPOINT: " + V14_INERT_CHECKPOINT,
        "NOTEAI_V14_FAILURE_RECEIPT: " + V14_FAILURE_RECEIPT,
        "python3 \"$"
        "{NOTEAI_V12_FAILURE_VERIFIER_PATH}\"",
        "- name: Remove V15 builders and all transient Docker state\n"
        "        id: cleanup\n"
        "        if: always()",
        "- name: Snapshot V2 through V15 ledgers after cleanup and before upload\n"
        "        id: late_ledger\n"
        "        if: ${{ always() && steps.cleanup.outputs.cleanup_passed == 'true' }}",
        "- name: Validate final V15 portable bundle after cleanup\n"
        "        id: bundle\n"
        "        if: ${{ success() && steps.cleanup.outputs.cleanup_passed == 'true' }}",
        "- name: Upload one-day V15 public-repository dependency cache artifact\n"
        "        id: upload\n"
        "        if: ${{ success() && steps.cleanup.outputs.cleanup_passed == 'true' }}",
    )
    for token in required:
        if token not in workflow:
            errors.append(f"V15 workflow contract missing: {token}")
    if workflow.count(
        "NOTEAI_V15_LEDGER_CONTRACT_PATH: "
        "${{ steps.control.outputs.ledger_contract_copy }}"
    ) != 2:
        errors.append("V15 workflow ledger contract handoff count changed")
    if workflow.count(
        'cp "${NOTEAI_REQUEST_TEMPLATE}" "${ledger_contract_copy}"'
    ) != 1:
        errors.append("V15 workflow ledger contract copy count changed")
    for forbidden in (
        "workflow_dispatch:",
        "pull_request:",
        ...
        "scripts/ci/import_admin_dependency_cache_v15.sh",
        "tools/verify_admin_dependency_cache_bundle_v15.py",
        ...
        if forbidden in workflow:
            errors.append(f"V15 workflow forbidden token: {forbidden}")
    cleanup = workflow.find("Remove V15 builders and all transient Docker state")
    late = workflow.find("Snapshot V2 through V15 ledgers after cleanup and before upload")
    final = workflow.find("Validate final V15 portable bundle after cleanup")
    upload = workflow.find("Upload one-day V15 public-repository dependency cache artifact")
    if not 0 <= cleanup < late < final < upload:
        errors.append("V15 cleanup, late-ledger and upload ordering changed")
    return errors
...
def _validate_frozen_worktree() -> list[str]:
    ...
    for path, expected in FROZEN_WORKTREE_FILES:
        try:
            ...
            if hashlib.sha256(payload).hexdigest() != expected:
                errors.append(f"V15 authority hash drift: {path.relative_to(ROOT)}")
        except (OSError, ValueError) as exc:
            errors.append(f"cannot read V15 authority {path}: {exc}")
    return errors
...
def validate_plan(
    *,
    ...
    errors: list[str] = []
    try:
        workflow_bytes = _read_frozen_bytes(WORKFLOW_PATH, label="V15 workflow")
        template_bytes = _read_frozen_bytes(TEMPLATE_PATH, label="V15 template")
        fixture_bytes = _read_frozen_bytes(SOURCE_FIXTURE_PATH, label="V15 source fixture")
        template = _strict_json_bytes(template_bytes, "V15 template")
        fixture = _strict_json_bytes(fixture_bytes, "V15 source fixture")
    except (OSError, ValueError) as exc:
        return [f"cannot load V15 dependency-cache plan: {exc}"]
    if hashlib.sha256(workflow_bytes).hexdigest() != WORKFLOW_SHA256:
        errors.append("V15 workflow hash drift")
    if hashlib.sha256(template_bytes).hexdigest() != TEMPLATE_SHA256:
        errors.append("V15 template hash drift")
    if hashlib.sha256(fixture_bytes).hexdigest() != SOURCE_FIXTURE_SHA256:
        errors.append("V15 source fixture hash drift")
    if isinstance(template, dict):
        errors.extend(_template_errors(template))
    else:
        errors.append("V15 template root changed")
    ...
    if not (
        ...
        errors.append("V15 source fixture semantics changed")
    ...
    if present and active is not None:
        ...
        if (
            ...
            errors.append("V15 active request differs from reviewed template")
        if verify_git_state and not supplied:
            errors.extend(_validate_active_git(active))
    elif present:
        errors.append("V15 active request is invalid")
    if verify_git_state:
        errors.extend(_validate_v14_superseded_predecessor())
        try:
            terminal = _load_v15_terminal_verifier()
            ...
            errors.append(f"cannot load V15 terminal supersession: {exc}")
        if terminal_checkpoint is not None and terminal is not None:
            try:
                ...
                errors.append(f"cannot verify V15 terminal supersession: {exc}")
        else:
            ...
            if not present:
                try:
                    if _true_additions(
                        ...
                        errors.append(
                            "inactive V15 request has prior addition history"
                        )
                except (
                    OSError,
                    ValueError,
                    ...
                    errors.append(
                        f"cannot verify inactive V15 request history: {exc}"
                    )
    return errors
...
def classify_plan_state(
    *,
    ...
    if additions:
        return "V15_CONSUMED_OR_INVALID"
    return "PREPARED_V15_NOT_TRIGGERED"
...
def _activation_plan_state() -> str:
    ...
    if not present and additions:
        git_errors.append("inactive V15 request has prior history")
    base_errors.extend(_validate_v14_superseded_predecessor())
...
def evaluate_plan() -> tuple[list[str], str, list[str]]:
    """Evaluate the V15 plan and terminal overlay with one terminal Git pass.

    The third tuple member contains terminal-evidence errors for the dedicated
    repository-gate disposition. Before a terminal checkpoint exists it is
    intentionally empty and the activation lifecycle remains authoritative.
    """
    try:
        terminal = _load_v15_terminal_verifier()
        ...
        terminal_errors = [f"cannot load V15 terminal supersession: {exc}"]
        plan_errors = validate_plan(verify_git_state=False)
        plan_errors.extend(_validate_v14_superseded_predecessor())
    ...
    try:
        ...
        terminal_errors = [f"cannot verify V15 terminal supersession: {exc}"]

    # Static/current authority validation and the frozen V14 predecessor are
    # still checked. The prevalidated terminal result replaces only the two
    # otherwise duplicated full terminal Git walks.
    plan_errors = validate_plan(verify_git_state=False)
    plan_errors.extend(_validate_v14_superseded_predecessor())
    plan_errors.extend(terminal_errors)
    if plan_errors:
    ...
    try:
        ...
        plan_errors.append(f"cannot classify V15 terminal lifecycle: {exc}")
        return plan_errors, "INVALID", terminal_errors
    state = (
...
def _legacy_plan_state(effective_state: str) -> str:
    if effective_state in {
        ...
        return "V15_ARMED_OR_TRIGGERED_EXACT"
    return effective_state


def plan_state() -> str:
    """Preserve the legacy request-activation view for frozen consumers."""
...
def _expected_ledger_contract() -> dict[str, Any]:
    configured = os.environ.get("NOTEAI_V15_LEDGER_CONTRACT_PATH")
    path = Path(configured) if configured else TEMPLATE_PATH
    payload = _read_frozen_bytes(
        path,
        label="V15 ledger contract template",
        required_mode=0o400 if configured else 0o644,
    )
    _require(
        hashlib.sha256(payload).hexdigest() == TEMPLATE_SHA256,
        "V15 ledger contract template hash drift",
    )
    template = _strict_json_bytes(payload, "V15 ledger contract template")
    _require(isinstance(template, dict), "V15 ledger template root changed")
    contract = template.get("run_ledger_contract")
    _require(isinstance(contract, dict), "V15 ledger contract missing")
    legacy = contract.get("legacy")
    _require(isinstance(legacy, list) and len(legacy) == 13, "V15 legacy ledger changed")
    _require(
        hashlib.sha256(_canonical_json_bytes(legacy)).hexdigest()
        == LEGACY_LEDGER_SHA256,
        "V15 legacy ledger history changed",
    )
    v11 = legacy[9]
    v12 = legacy[10]
    v13 = legacy[11]
...
def _collect_current_ledger(
    ...
    *,
    ...
    _require(
        current.get("path") == V15_WORKFLOW_PATH,
    ...
    _require(len(runs) == 1, "V15 workflow run count changed")
    run = runs[0]
    ...
    _require(run.get("path") == V15_WORKFLOW_PATH, "current workflow path changed")
...
def _validate_prior_snapshot(
    ...
    *,
    ...
    _require(current.get("path") == V15_WORKFLOW_PATH, "pre-resource path changed")
    ...
    *,
    ...
    def fetch(path: str) -> dict[str, Any]:
        ...
        for attempt in range(3):
            ...
            _require(remaining > 0, "V15 ledger deadline exceeded")
            try:
...
def _live_ledger_main(arguments: list[str]) -> int:
    ...
    print(f"admin_dependency_cache_v15_live_ledger=PASS phase={phase}")
    return 0
...
def main() -> int:
    try:
        if len(sys.argv) > 1:
            _require(sys.argv[1] == "live-ledger", "V15 command changed")
        ...
        if errors:
            ...
            return 1
        print(
            "admin_dependency_cache_export_plan_v15=PASS "
