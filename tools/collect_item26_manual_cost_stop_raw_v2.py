#!/usr/bin/env python3
"""Offline, root-only v2 importer for Item 26 post-action readback bytes.
...
import extract_item26_manual_cost_stop_raw_v2 as extractor
...
OPERATION_ID = extractor.CAPTURE_OPERATION_ID
HISTORICAL_OPERATION_ID = extractor.HISTORICAL_OPERATION_ID
AUTHORITY_GENERATION_ID = extractor.AUTHORITY_GENERATION_ID
...
PROVIDER_FILE = "provider-raw-v2.json"
ACTIONTRAIL_FILE = "actiontrail-raw-v2.json"
...
JOURNAL_SCHEMA = "noteai.item26.manual-cost-stop-import-journal.v2"
COLLECTOR_STATUS_SCHEMA = "noteai.item26.manual-cost-stop-collector-status.v2"
UNKNOWN_MARKER_SCHEMA = (
    "noteai.item26.manual-cost-stop-offline-response-unknown.v2"
)
ACTIVATION_RECEIPT_SCHEMA = authority.ACTIVATION_RECEIPT_SCHEMA
CAPTURE_TRANSPORT = "OFFLINE_OFFICIAL_RESPONSE_IMPORT_V2"
SOURCE_FILES = {
...
def _require_authority_v2_finalized() -> None:
    if (
        authority.AUTHORITY_V2_FINALIZED is not True
        or type(authority.EXPECTED_AUTHORITY_ROOT_FILE_SHA256) is not str
        or re.fullmatch(
            r"[0-9a-f]{64}",
        ...
        raise CollectorError("authority_v2_not_finalized")
    directories = (
        AUTHORITY_DIRECTORY,
        RUNTIME_DIRECTORY,
        JOURNAL_DIRECTORY,
    ...
    if (
        any(not path.is_absolute() for path in directories)
        ...
        raise CollectorError("directory_partition")
    ...
    *,
    ...
    try:
        ...
        binding = authority.load_activation_root(
            ...
            expected_inventory=tuple(sorted(inventory)),
        )
    except CollectorError:
        raise
    ...
    value = binding.get("root_value")
    keys = binding.get("authority_keys")
    if (
        ...
        or binding.get("authority_root_file_sha256") != _sha(root_raw)
        or binding.get("authority_root_git_blob_sha256")
        != _sha(root_raw)
...
def _validate_runtime_activation(
    *,
    ...
    authority_root_git_blob_sha256: str,
    authority_epoch: str,
    ...
    try:
        ...
        if type(binding) is not dict:
            raise CollectorError("activation_receipt")
        if _utc(
            _utc_now(),
        ...
        if (
            binding.get("authority_epoch_id") != authority_epoch
            or binding.get("authority_root_file_sha256")
            != authority_root_file_sha256
            or binding.get("authority_root_git_blob_sha256")
            != authority_root_git_blob_sha256
        ):
            raise CollectorError("activation_receipt_binding")
    except CollectorError:
        raise
    except (KeyError, OSError, TypeError, ValueError) as exc:
    ...
    return {
        ...
        "activation_receipt_sha256": binding[
...
def _trusted_runtime(
    *,
    ...
    receipt_binding = _validate_runtime_activation(
        control_revision=control_revision,
        authority_root_file_sha256=root_binding[
            "authority_root_file_sha256"
        ],
        authority_root_git_blob_sha256=root_binding[
            "authority_root_git_blob_sha256"
        ],
        authority_epoch=root_binding["authority_epoch"],
...
def _decode_event(raw: bytes) -> dict[str, Any]:
    ...
    if (
        ...
        or value.get("historical_operation_id")
        != HISTORICAL_OPERATION_ID
        ...
        or type(value.get("authority_root_file_sha256")) is not str
        or len(value["authority_root_file_sha256"]) != 64
        or type(value.get("authority_root_git_blob_sha256")) is not str
        or len(value["authority_root_git_blob_sha256"]) != 64
        or value["authority_root_file_sha256"]
        != value["authority_root_git_blob_sha256"]
        or value.get("activation_receipt_schema")
        != ACTIVATION_RECEIPT_SCHEMA
...
def _pairs(events: list[dict[str, Any]]) -> list[tuple[dict[str, Any], dict[str, Any]]]:
    ...
    for index in range(0, len(events), 2):
        ...
        if (
            ...
            or first["authority_epoch"] != second["authority_epoch"]
            or first["authority_root_file_sha256"]
            != second["authority_root_file_sha256"]
            or first["authority_root_git_blob_sha256"]
            != second["authority_root_git_blob_sha256"]
            or first["activation_receipt_schema"]
            != second["activation_receipt_schema"]
...
def _capture_envelopes(
    ...
    source_hashes = {
        ...
        "authority_epoch": events[0]["authority_epoch"],
        "authority_root_file_sha256": events[0][
            "authority_root_file_sha256"
        ],
        "authority_root_git_blob_sha256": events[0][
            "authority_root_git_blob_sha256"
        ],
        "activation_receipt_schema": events[0][
            "activation_receipt_schema"
        ],
...
def _blocked_status(code: str) -> dict[str, Any]:
    ...
    if code in {
        ...
        "activation_receipt_future",
    }:
        value["journal_write_count"] = 0
    return value
...
def main(argv: Optional[list[str]] = None) -> int:
    try:
        ...
        if args.command in {"begin", "finish"}:
            _trusted_runtime(
        ...
        _write_status(sys.stderr, _blocked_status(exc.code))
        return 2
