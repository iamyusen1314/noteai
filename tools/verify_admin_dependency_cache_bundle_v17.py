#!/usr/bin/env python3
"""Validate the source-backed V17 BuildKit cache export successor.

V17 changes the operational main context from a per-solve local source to one
...
SOURCE_IDENTITY_FIXTURE_SHA256 = (
    "5c020473f8f8e7cc189c3e936a8b41c7"
    "6bdc2a9fde38fc79c7d11d31e9050b54"
)
MAXIMUM_SOURCE_BYTES = 4_194_304
MAXIMUM_COMMAND_ENVELOPE_BYTES = 4096
MAXIMUM_LIFECYCLE_DIAGNOSTIC_BYTES = 16 * 1024
MAXIMUM_GIT_SOURCE_INTERVALS = 32
...
def _load_v13_verifier():
    ...
    name = "_noteai_admin_dependency_cache_bundle_v13_for_v17"
...
class V17BundleError(_v13.V11BundleError):
    """A fixed-code V17 rejection safe for a public workflow log."""


def _fail(code: str, message: str) -> None:
    raise V17BundleError(code, message)
...
def _write_json(path: Path | None, payload: dict[str, Any]) -> None:
    try:
        ...
        _fail(
            getattr(exc, "failure_code", "V17_OUTPUT_INVALID"),
            "V17 output write failed",
        )
...
def _canonical_pretty_bytes(value: Any) -> bytes:
    try:
        ...
        raise V17BundleError(
            "V17_DIAGNOSTIC_INVALID",
            "V17 diagnostic serialization failed",
        ) from exc


def _rfc3339_ns(value: Any, label: str) -> int:
    _require(
        isinstance(value, str) and RFC3339_NS_RE.fullmatch(value) is not None,
        "V17_COMMAND_ENVELOPE_INVALID",
        f"{label} changed",
    )
    match = RFC3339_NS_RE.fullmatch(value)
    if match is None:
        _fail("V17_COMMAND_ENVELOPE_INVALID", f"{label} changed")
    try:
...
def _read_command_envelope(
    path: Path,
    *,
    expected_phase: str,
) -> tuple[dict[str, Any], bytes, int, int]:
    state = os.lstat(path)
    payload = _v13._regular_bytes(
        Path(path),
        maximum_bytes=MAXIMUM_COMMAND_ENVELOPE_BYTES,
        failure_code="V17_COMMAND_ENVELOPE_INVALID",
        label="V17 command envelope",
    )
    value = _v13._strict_json_bytes(payload, "V17 command envelope")
    _require(
        isinstance(value, dict)
        and set(value) == {"schema_version", "phase", "started", "completed"}
        and value["schema_version"]
        == "noteai.admin-dependency-cache-command-envelope.v17"
        and value["phase"] == expected_phase
        and payload == _canonical_pretty_bytes(value),
        "V17_COMMAND_ENVELOPE_INVALID",
        "V17 command envelope changed",
    )
    _require(
        ...
        "V17_COMMAND_ENVELOPE_INVALID",
        "V17 command envelope file contract changed",
    )
    started_ns = _rfc3339_ns(value["started"], "command start")
    completed_ns = _rfc3339_ns(value["completed"], "command completion")
    _require(
        started_ns <= completed_ns
        and completed_ns - started_ns <= 7_200 * 1_000_000_000,
        "V17_COMMAND_ENVELOPE_INVALID",
        "V17 command envelope changed",
    )
    return value, payload, started_ns, completed_ns


def _write_exclusive_diagnostic(path: Path, payload: bytes) -> None:
    _require(
        0 < len(payload) <= MAXIMUM_LIFECYCLE_DIAGNOSTIC_BYTES,
        "V17_DIAGNOSTIC_INVALID",
        "V17 lifecycle diagnostic size changed",
    )
    descriptor = -1
    try:
        descriptor = os.open(
        ...
        raise V17BundleError(
            "V17_DIAGNOSTIC_INVALID",
            "V17 lifecycle diagnostic write failed",
        ) from exc
    finally:
        if descriptor >= 0:
            os.close(descriptor)
    state = os.lstat(path)
    observed = _v13._regular_bytes(
        path,
        maximum_bytes=MAXIMUM_LIFECYCLE_DIAGNOSTIC_BYTES,
        failure_code="V17_DIAGNOSTIC_INVALID",
        label="V17 lifecycle diagnostic",
    )
    _require(
        observed == payload
        ...
        and state.st_nlink == 1,
        "V17_DIAGNOSTIC_INVALID",
        "V17 lifecycle diagnostic file contract changed",
    )


def _expected_diagnostic_bytes(path: Path) -> bytes:
    state = os.lstat(path)
    payload = _v13._regular_bytes(
        path,
        maximum_bytes=MAXIMUM_LIFECYCLE_DIAGNOSTIC_BYTES,
        failure_code="V17_DIAGNOSTIC_INVALID",
        label="V17 retained lifecycle diagnostic",
    )
    value = _v13._strict_json_bytes(
        payload,
        "V17 retained lifecycle diagnostic",
    )
    _require(
        isinstance(value, dict)
        and payload == _canonical_pretty_bytes(value)
        ...
        and state.st_nlink == 1,
        "V17_DIAGNOSTIC_INVALID",
        "V17 retained lifecycle diagnostic changed",
    )
    return payload


def _git_source_interval(
    *,
    metadata_path: Path,
    progress_path: Path,
    command_envelope_path: Path,
    phase: str,
    dockerfile_kind: str,
    target: str,
    ...
    command_completed_ns: int,
    step_id: str,
    digest: str,
    vertex: dict[str, Any],
    build_window: Any,
    diagnostic_output_path: Path | None,
    expected_diagnostic_path: Path | None,
    ...
    projected = [
        ...
        for _started_ns, interval in intervals[:MAXIMUM_GIT_SOURCE_INTERVALS]
    ]
    completion_present = all(
        interval["completed"] is not None
        for _started_ns, interval in intervals
    )
    start_before_completion = all(
        interval["completed"] is not None
        and started_ns <= interval["completed_ns"]
        for started_ns, interval in intervals
    )
    command_encloses_provenance = (
        command_started_ns
        <= build_window.started_ns
        <= build_window.finished_ns
        <= command_completed_ns
    )
    starts_inside_command = all(
        command_started_ns <= started_ns
        for started_ns, _interval in intervals
    )
    completes_by_provenance_finish = all(
        interval["completed"] is not None
        and interval["completed_ns"] <= build_window.finished_ns
        for _started_ns, interval in intervals
    )
    completes_inside_command = all(
        interval["completed"] is not None
        and interval["completed_ns"] <= command_completed_ns
        for _started_ns, interval in intervals
    )
    nonempty = bool(intervals)
    interval_limit = len(intervals) <= MAXIMUM_GIT_SOURCE_INTERVALS
    predicate_passed = all(
        (
            nonempty,
            interval_limit,
            completion_present,
            start_before_completion,
            command_encloses_provenance,
            starts_inside_command,
            completes_by_provenance_finish,
            completes_inside_command,
        )
    )
    if predicate_passed:
        failure_code = "PASS"
    elif not nonempty:
        failure_code = "GIT_SOURCE_LIFECYCLE_INCOMPLETE"
    elif not interval_limit:
        failure_code = "GIT_SOURCE_INTERVAL_LIMIT_EXCEEDED"
    elif not completion_present:
        failure_code = "GIT_SOURCE_COMPLETION_MISSING"
    elif not command_encloses_provenance:
        failure_code = "COMMAND_DOES_NOT_ENCLOSE_PROVENANCE"
    elif not starts_inside_command or not completes_inside_command:
        failure_code = "GIT_SOURCE_OUTSIDE_COMMAND"
    elif not completes_by_provenance_finish:
        failure_code = "GIT_SOURCE_COMPLETES_AFTER_PROVENANCE"
    else:
        failure_code = "GIT_SOURCE_LIFECYCLE_INVALID"
    ...
    diagnostic: dict[str, Any] = {
        ...
        "git_step_id_sha256": hashlib.sha256(step_id.encode("utf-8")).hexdigest(),
        "vertex_digest": digest,
        "command_window": {
            "started": datetime.fromtimestamp(
                command_started_ns // 1_000_000_000,
                tz=timezone.utc,
            ).strftime("%Y-%m-%dT%H:%M:%S")
            + f".{command_started_ns % 1_000_000_000:09d}Z",
            "completed": datetime.fromtimestamp(
                command_completed_ns // 1_000_000_000,
                tz=timezone.utc,
            ).strftime("%Y-%m-%dT%H:%M:%S")
            + f".{command_completed_ns % 1_000_000_000:09d}Z",
        },
        "provenance_window": {
            "started": provenance_metadata["buildStartedOn"],
            "completed": provenance_metadata["buildFinishedOn"],
        },
        ...
        "predicate": {
            "nonempty": nonempty,
            "interval_limit_satisfied": interval_limit,
            "all_completions_present": completion_present,
            "all_starts_before_completions": start_before_completion,
            "command_encloses_provenance": command_encloses_provenance,
            "all_starts_inside_command": starts_inside_command,
            "all_completions_by_provenance_finish": completes_by_provenance_finish,
            "all_completions_inside_command": completes_inside_command,
            "git_start_before_provenance_start_allowed": True,
            "passed": predicate_passed,
        },
        "failure_code": failure_code,
        "verdict": "pass" if predicate_passed else "fail",
    }
    diagnostic["diagnostic_sha256"] = _canonical_sha256(diagnostic)
    diagnostic_bytes = _canonical_pretty_bytes(diagnostic)
    _require(
        diagnostic_output_path is not None
        or expected_diagnostic_path is not None,
        "V17_DIAGNOSTIC_INVALID",
        "V17 lifecycle diagnostic destination missing",
    )
    if diagnostic_output_path is not None:
        _write_exclusive_diagnostic(diagnostic_output_path, diagnostic_bytes)
    if expected_diagnostic_path is not None:
        _require(
            _expected_diagnostic_bytes(expected_diagnostic_path)
            == diagnostic_bytes,
            "V17_DIAGNOSTIC_INVALID",
            "V17 retained lifecycle diagnostic changed",
        )
    reread_metadata = _v13._regular_bytes(
        metadata_path,
        maximum_bytes=_v13._v10._v6.MAXIMUM_METADATA_BYTES,
        failure_code="V17_EVIDENCE_CHANGED_DURING_VALIDATION",
        label="BuildKit metadata",
    )
    reread_progress = _v13._regular_bytes(
        progress_path,
        maximum_bytes=_v13._v10._v6.MAXIMUM_PROGRESS_BYTES,
        failure_code="V17_EVIDENCE_CHANGED_DURING_VALIDATION",
        label="BuildKit rawjson",
    )
    reread_envelope = _v13._regular_bytes(
        command_envelope_path,
        maximum_bytes=MAXIMUM_COMMAND_ENVELOPE_BYTES,
        failure_code="V17_EVIDENCE_CHANGED_DURING_VALIDATION",
        label="V17 command envelope",
    )
    _require(
        hashlib.sha256(reread_metadata).hexdigest() == metadata_sha256
        and hashlib.sha256(reread_progress).hexdigest() == progress_sha256
        and hashlib.sha256(reread_envelope).hexdigest()
        == command_envelope_sha256,
        "V17_EVIDENCE_CHANGED_DURING_VALIDATION",
        "V17 lifecycle evidence changed during validation",
    )
    _require(
        predicate_passed,
        "NETWORK_VERTEX_LIFECYCLE_OUTSIDE_BUILD",
        "BuildKit git_main_context lifecycle changed",
    )
    return (
        {
...
def _identity_projection(
    ...
    *,
    dockerfile_kind: str,
    require_cached: bool,
    lifecycle_diagnostic_output_path: Path | None = None,
    expected_lifecycle_diagnostic_path: Path | None = None,
    ...
    _require(
        ...
        "V17_TARGET_INVALID",
        "V17 Dockerfile target changed",
    )
    phase = (
        "producer"
        if dockerfile_kind == "prefix" and not require_cached
        else "import"
        if dockerfile_kind == "prefix"
        else "replay"
    )
    (
        ...
        command_started_ns,
        command_completed_ns,
    ) = _read_command_envelope(
        Path(command_envelope_path),
        expected_phase=phase,
    )
    metadata_sha256 = hashlib.sha256(metadata_bytes).hexdigest()
    progress_sha256 = hashlib.sha256(progress_bytes).hexdigest()
    command_envelope_sha256 = hashlib.sha256(command_envelope_bytes).hexdigest()
    ...
    try:
        ...
        raise V17BundleError(exc.failure_code, str(exc)) from None
    git_steps: list[str] = []
    ...
    _require(
        ...
        and git_step["inputs"] == [],
        "V17_GIT_SOURCE_INVALID",
        "canonical Git SourceOp changed",
    )
    git_digest = _one_step_digest(
        step_to_digests,
        git_step_id,
        code="V17_GIT_SOURCE_DIGEST_AMBIGUOUS",
        label="Git main context",
    )
    _require(
        git_digest in vertices,
        "V17_GIT_SOURCE_VERTEX_MISSING",
        "Git source progress vertex missing",
    )
    git_interval, lifecycle_diagnostic_sha256 = _git_source_interval(
        metadata_path=Path(metadata_path),
        progress_path=Path(progress_path),
        command_envelope_path=Path(command_envelope_path),
        phase=phase,
        dockerfile_kind=dockerfile_kind,
        target=target,
        metadata=metadata,
        ...
        progress_sha256=progress_sha256,
        command_envelope_sha256=command_envelope_sha256,
        command_started_ns=command_started_ns,
        command_completed_ns=command_completed_ns,
        step_id=git_step_id,
        digest=git_digest,
        vertex=vertices[git_digest],
        build_window=build_window,
        diagnostic_output_path=lifecycle_diagnostic_output_path,
        expected_diagnostic_path=expected_lifecycle_diagnostic_path,
    )
    try:
    ...
    copy_digest = _one_step_digest(
        step_to_digests,
        copy_step_id,
        code="V17_REQUIREMENTS_COPY_DIGEST_AMBIGUOUS",
        label="requirements copy",
    )
    _require(
        copy_digest in vertices,
        "V17_REQUIREMENTS_COPY_VERTEX_MISSING",
    ...
    runtime_parent_digest = _one_step_digest(
        ...
        code="V17_REQUIREMENTS_COPY_PARENT_DIGEST_AMBIGUOUS",
    ...
    pip_digest = _one_step_digest(
        step_to_digests,
        pip_step_id,
        code="V17_RUNTIME_PIP_DIGEST_AMBIGUOUS",
        label="runtime_pip",
    )
    ...
    _require(
        ...
        "V17_RUNTIME_PIP_IDENTITY_INVALID",
    ...
    identity = {
        ...
        "progress_sha256": progress_sha256,
        "command_envelope_sha256": command_envelope_sha256,
        "git_source_lifecycle_diagnostic_sha256": lifecycle_diagnostic_sha256,
        "dockerfile_kind": dockerfile_kind,
        "target": target,
        "cache_predicate": (
        ...
        "source": {
            ...
            "lifecycle_policy": "COMMAND_ENVELOPE_GIT_FRONTEND_V17",
    ...
    if enforce_cache_predicates:
        ...
        _require(
            role_predicate,
            "V17_NETWORK_CACHE_PREDICATE_FAILED",
...
def _v17_summary(
    ...
    *,
    ...
    summary["schema_version"] = "noteai.admin-dependency-cache-build.v17"
    ...
    summary["legacy_v13_execution_boundary"] = {
        ...
        "original_v17_git_identity_validated_first": True,
        "high_level_delegate_executed_on_original_metadata": False,
        "legacy_verdict_transformed_for_v17_summary": True,
...
def _legacy_git_context_projection(
    ...
    *,
    ...
    one local://context SourceOp and only the two base-image materials.  V17
    validates the original Git graph before entering this function.  The
    projection is a private deep copy, never an authority for V17 identity.
    """
    ...
    _require(
        replacements == 1,
        "V17_LEGACY_PROJECTION_INVALID",
        "V17 legacy Git-source projection changed",
    )
    ...
    temporary_root = Path(tempfile.mkdtemp(prefix="noteai-v17-legacy-"))
    ...
    try:
        ...
        _require(
            ...
            "V17_LEGACY_PROJECTION_INVALID",
            "V17 legacy projection mode changed",
        )
        yield (
            legacy_path,
        ...
        _require(
            ...
            "V17_LEGACY_PROJECTION_CLEANUP_FAILED",
            "V17 legacy projection cleanup incomplete",
        )
...
def _legacy_producer_summary(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(
        ...
        "V17_PRODUCER_SUMMARY_INVALID",
        "V17 producer summary schema changed",
    )
    _require(
        ...
        "V17_PRODUCER_SUMMARY_INVALID",
        "V17 producer identity or target changed",
    )
...
def classify_pair(
    ...
    _require(
        ...
        "V17_PAIR_TARGET_INVALID",
        "V17 producer or consumer summary changed",
    )
    ...
    _require(
        ...
        "V17_PAIR_IDENTITY_MISSING",
        "V17 identity chain missing",
    )
    ...
    _require(
        ...
        "V17_PAIR_IDENTITY_INVALID",
        "V17 identity digest changed",
    )
    producer_noncached = (
    ...
    return {
        ...
        "v17_boundary_status": "DETERMINED_BY_V17",
    }
...
def validate_pair(
    ...
    _require(
        ...
        "V17_PAIR_CACHE_PREDICATE_FAILED",
        "V17 producer/consumer identity chain did not prove a cache hit",
    )
    return pair
...
def validate_build_evidence(
    ...
    *,
    ...
    identity, roles = _identity_projection(
        ...
        lifecycle_diagnostic_output_path=lifecycle_diagnostic_output_path,
        expected_lifecycle_diagnostic_path=expected_lifecycle_diagnostic_path,
    ...
    if dockerfile_kind == "prefix" and require_network_vertices_cached:
        _require(
            ...
            "V17_PAIR_EVIDENCE_MISSING",
            "V17 producer summary is missing",
        )
        ...
        _require(
            ...
            == "SAME_SOURCE_COPY_PIP_DIGESTS_CACHED",
            "V17_PAIR_CACHE_PREDICATE_FAILED",
            "V17 fresh consumer identity chain did not prove a cache hit",
        )
    ...
    with _legacy_git_context_projection(
        ...
        _require(
            ...
            "V17_METADATA_CHANGED_DURING_VALIDATION",
...
def validate_cache_record(
    ...
    with tempfile.TemporaryDirectory(prefix="noteai-v17-record-") as temporary:
...
def validate_manifest(bundle: Path) -> dict[str, Any]:
    ...
    _require(
        ...
        == "noteai.admin-dependency-cache-export.v17"
        and manifest["task"] == "PROD-FIRST-LAUNCH-ADMIN-INTERNAL-001",
        "V17_MANIFEST_INVALID",
        "V17 manifest identity changed",
    )
    ...
    _require(
        ...
        "V17_MANIFEST_INVALID",
        "V17 Git main-context contract changed",
    )
    build = _require_exact_dict(
        manifest["build"],
        {
            ...
            "progress_sha256",
            "command_envelope_sha256",
            "git_source_lifecycle_diagnostic_sha256",
            "duration_seconds",
            "base_images",
            "source_identity_fixture_sha256",
    ...
    producer_summary = validate_build_evidence(
        ...
        bundle / "producer" / "command-envelope.json",
        dockerfile_kind="prefix",
        require_network_vertices_cached=False,
        expected_lifecycle_diagnostic_path=(
            bundle / "producer" / "git-source-lifecycle.json"
        ),
    )
    ...
    _require(
        ...
        and producer_summary["command_envelope_sha256"]
        == build["command_envelope_sha256"]
        and producer_summary["git_source_lifecycle_diagnostic_sha256"]
        == build["git_source_lifecycle_diagnostic_sha256"]
        and producer_summary["duration_seconds"] == build["duration_seconds"]
        and _base.sha256_file(bundle / "producer" / "command-envelope.json")
        == build["command_envelope_sha256"]
        ...
        "V17_MANIFEST_INVALID",
        "V17 producer evidence binding changed",
    )
    ...
    _require(
        ...
        == SOURCE_IDENTITY_FIXTURE_SHA256,
        "V17_MANIFEST_INVALID",
        "V17 bounded authorization changed",
    )
    for key in (
        "request_sha256",
        "workflow_sha256",
    ...
    _require(
        ...
        "V17_MANIFEST_INVALID",
        "V17 bundled execution authority changed",
    )
    _require(
        manifest["next_gate"]
        == {
...
FINAL_ADDITIONS = {
    ...
    "portability/import-command-envelope.json",
    "portability/import-git-source-lifecycle.json",
    ...
    "portability/replay-command-envelope.json",
    "portability/replay-git-source-lifecycle.json",
...
def validate_core(
    ...
    *,
    ...
    _require(bundle.is_dir(), "V17_BUNDLE_INVALID", "bundle directory missing")
    ...
    _require(
        ...
        "V17_BUNDLE_INVALID",
        "partial V17 portability evidence present",
    )
    ...
    expected_core = {
        ...
        "producer/command-envelope.json",
        "producer/git-source-lifecycle.json",
    ...
    _require(
        ...
        "V17_BUNDLE_INVALID",
        "V17 core bundle file set changed",
    )
    ...
    try:
        ...
        retained_record_bytes = _v13._regular_bytes(
            ...
            failure_code="V17_SUMMARY_FILE_INVALID",
            label="V17 cache record",
        )
        ...
        _require(
            ...
            "V17 cache record provenance changed",
        )
        if validation_context is not None:
            _require(
                validation_context == {},
                "V17_CACHE_RECORD_TRUST_INVALID",
                "V17 core validation context was not empty",
            )
...
def validate_portability(
    ...
    *,
    ...
    retained_import = _read_summary(
        portability / "import-summary.json",
        label="V17 retained import summary",
    )
    retained_replay = _read_summary(
        portability / "replay-summary.json",
        label="V17 retained replay summary",
    )
    retained_import_identity = _read_summary(
        portability / "import-build-identity.json",
        label="V17 retained import identity",
    )
    retained_replay_identity = _read_summary(
        portability / "replay-build-identity.json",
        label="V17 retained replay identity",
    )
    retained_diagnostic = _read_summary(
        portability / "import-diagnostic.json",
        label="V17 retained V13 compatibility diagnostic",
    )
    retained_pre_predicate = _read_summary(
        portability / "import-pre-predicate-diagnostic.json",
        label="V17 retained V13 compatibility pre-predicate diagnostic",
    )
    pair = validate_pair(
        bundle / "producer" / "cache-build-summary.json",
        portability / "import-summary.json",
    ...
    _require(
        ...
        "V17_PORTABILITY_BINDING_INVALID",
        "V17 retained portability evidence changed",
    )
    _require(
        retained_diagnostic == import_summary["v11_rawjson_diagnostic"],
        "V17_PORTABILITY_BINDING_INVALID",
        "V17 retained compatibility diagnostic changed",
    )
    ...
    _require(
        ...
        "V17_REPLAY_IDENTITY_DRIFT",
        "V17 replay identity chain changed",
    )
    proof = _read_summary(
        portability / "proof.json",
        label="V17 portability proof",
    )
    _require_exact_dict(
        proof,
        {
        ...
        "V17 portability proof",
        code="V17_PORTABILITY_INVALID",
    )
    _require(
        ...
        "V17_PORTABILITY_INVALID",
        "V17 portability proof changed",
    )
    ...
    _require(
        ...
        "V17_PORTABILITY_INVALID",
        "V17 producer/consumer BuildKit identity changed",
    )
    _require(
        proof["controls"]
        == {
        ...
        "V17_PORTABILITY_INVALID",
        "V17 portability controls changed",
    )
    return proof
...
def validate_final(
    ...
    _require(
        ...
        "V17_BUNDLE_INVALID",
        "V17 portability evidence missing",
    )
    ...
    _require(
        ...
        "V17_CACHE_RECORD_TRUST_INVALID",
        "V17 core validation did not capture the cache record",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    build.add_argument("--command-envelope", type=Path, required=True)
    ...
    build.add_argument(
        "--git-lifecycle-diagnostic-output",
        type=Path,
        required=True,
    )
        ...
        if args.command == "verify-build":
            payload = validate_build_evidence(
                ...
                lifecycle_diagnostic_output_path=(
                    args.git_lifecycle_diagnostic_output.resolve()
                ),
            )
