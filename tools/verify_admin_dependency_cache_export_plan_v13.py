#!/usr/bin/env python3
"""Verify the append-only V13 Admin dependency-cache successor plan."""
...
WORKFLOW_PATH = ROOT / ".github" / "workflows" / "admin-dependency-cache-export-v13.yml"
TEMPLATE_PATH = ROOT / "deploy" / "production" / "plans" / "admin-dependency-cache-export-request-v13.json"
ACTIVE_REQUEST_PATH = ROOT / ".github" / "release-requests" / "admin-5335bda-dependency-cache-v13.json"
...
PLAN_TEST_PATH = ROOT / "tests" / "test_admin_dependency_cache_export_plan_v13.py"
...
WORKFLOW_SHA256 = "d0abfed05cd57ab297d3864d13147a46d88f8241887040e64695ec797d01bc87"
TEMPLATE_SHA256 = "e86dd938c98048e0d647c2803abb16e2d9bbd3a041fdf011fd83c860cc098edd"
...
PLAN_TEST_SHA256 = "c01a6b6b4b7fd63397b335514a44c9eb87cc1175194b51f766836bf7dae34b44"
...
PREDECESSOR_COMMIT = "ae7ce751d842b2880cdb2d31213a983bcb1f7484"
...
LEDGER_SCHEMA = "noteai.admin-dependency-cache-ledger.v13"
...
LEGACY_LEDGER_SHA256 = "538d10f74adb688be40e665e4246f4878737a842974e1d7a344d86021a9413d6"
...
def _load_v12_terminal_verifier():
    ...
    module_name = "_noteai_v12_terminal_for_v13"
...
def _validate_v12_terminal_predecessor() -> list[str]:
    ...
    try:
        ...
        if _git_at(ROOT, "rev-parse", f"{PREDECESSOR_COMMIT}^") != (
            "81730a511acb553c9a1e29af834924f71650dc9a"
        ):
            errors.append("V12 correction receipt parent changed")
        if _git_at(ROOT, "rev-parse", PREDECESSOR_COMMIT) != PREDECESSOR_COMMIT:
...
def _authority_anchor(
    *,
    ...
    if len(present) != len(SAME_ANCHOR_FILES):
        return None, ["V13 frozen authorities are only partially present"]
    anchors: list[str] = []
    for path in SAME_ANCHOR_FILES:
        additions = _true_additions(root=root, path=path)
        if len(additions) != 1:
            errors.append(f"V13 frozen addition history is not unique: {path}")
        else:
            anchors.append(additions[0])
    if errors:
        return None, errors
    if len(set(anchors)) != 1:
        return None, ["V13 frozen authorities have different add anchors"]
    return anchors[0], []
...
def _validate_same_anchor(*, root: Path = ROOT) -> list[str]:
    ...
    try:
        ...
        if anchor is None:
            errors.append("V13 inert checkpoint is missing")
            return errors
        ...
        if not _on_first_parent_chain(root, anchor, head=branch_head):
            errors.append("V13 inert checkpoint is not on first-parent chain")
        ...
        if _lineage_path_changes(
            ...
            errors.append("V13 immutable checkpoint authority changed")
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        errors.append(f"cannot verify V13 inert checkpoint: {exc}")
    return errors
    ...
    *,
    ...
    try:
        ...
        if not changes:
            return None, []
        if len(changes) != 1:
            return None, ["V13 inert receipt history is not unique"]
        receipt = changes[0]
        if _commit_parents(root, receipt) != [anchor]:
            errors.append("V13 inert receipt is not the direct child of anchor")
        elif _changed_paths(root, anchor, receipt) != set(INERT_RECEIPT_FILES):
            errors.append("V13 inert receipt is not the exact 4-file delta")
        for relative in INERT_RECEIPT_FILES:
            entry = _tree_entry(root, receipt, relative).split()
            if len(entry) < 3 or entry[0:2] != ["100644", "blob"]:
                errors.append(f"V13 inert receipt mode changed: {relative}")
        if not _on_first_parent_chain(root, receipt, head=branch_head):
            errors.append("V13 inert receipt is not on first-parent chain")
        return receipt, errors
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        return None, [f"cannot verify V13 inert receipt: {exc}"]
    ...
    *,
    ...
    try:
        ...
        payload = _read_frozen_bytes(
            path,
            label="V13 active request",
            required_mode=0o644,
        )
        value = _strict_json_bytes(payload, "V13 active request")
        if not isinstance(value, dict):
            return True, None, ["V13 active request root changed"]
        return True, value, []
    except (OSError, ValueError) as exc:
        return True, None, [f"cannot load V13 active request: {exc}"]
...
def _validate_active_git(
    ...
    *,
    ...
    try:
        ...
        if len(additions) != 1:
            return ["V13 active request addition history is not unique"]
        activation = additions[0]
        if not _on_first_parent_chain(root, activation, head=branch_head):
            errors.append("V13 activation is not on first-parent chain")
        parents = _commit_parents(root, activation)
        if len(parents) != 1:
            return ["V13 activation is not single-parent"]
        parent = parents[0]
        receipt, receipt_errors = _validate_inert_receipt(root=root)
        errors.extend(receipt_errors)
        if receipt is None or parent != receipt:
            errors.append("V13 activation parent is not the exact receipt")
        if active.get("plan_checkpoint_commit") != parent:
            errors.append("V13 active request does not bind direct parent")
        if _changed_paths(root, parent, activation) != {relative}:
            errors.append("V13 activation changes more than its request")
        status = _git_at(
            root,
            "diff-tree",
            "--no-commit-id",
        ...
        if status != f"A\t{relative.as_posix()}":
            errors.append("V13 request was not a unique addition")
        entry = _tree_entry(root, activation, relative).split()
        if len(entry) < 3 or entry[0:2] != ["100644", "blob"]:
            errors.append("V13 request activation mode changed")
        template = _read_frozen_bytes(TEMPLATE_PATH, label="V13 template")
        ...
        if payload != expected:
            errors.append("V13 activation request differs from template")
        if _lineage_path_changes(
            root=root,
            anchor=activation,
            paths=(relative,),
            ...
            errors.append("V13 active request changed after activation")
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        errors.append(f"cannot verify V13 active request Git state: {exc}")
    return errors
    ...
    *,
    ...
    try:
        ...
        return [] if branch_head == expected else [
            "V13 canonical branch HEAD is not the latest exact stage"
        ]
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        return [f"cannot verify V13 latest exact stage: {exc}"]
...
def _template_errors(template: dict[str, Any]) -> list[str]:
    ...
    require(template.get("schema_version") == 13, "V13 schema changed")
    require(template.get("task") == "PROD-FIRST-LAUNCH-ADMIN-INTERNAL-001", "V13 task changed")
    require(template.get("release_commit") == RELEASE_COMMIT, "V13 release changed")
    require(template.get("plan_checkpoint_commit") == "__DIRECT_PARENT_COMMIT__", "V13 parent placeholder changed")
    require(template.get("trigger_mode") == "one_shot_added_request_v13", "V13 trigger mode changed")
    predecessor = template.get("predecessor")
    require(isinstance(predecessor, dict), "V13 predecessor missing")
    if isinstance(predecessor, dict):
        require(
            predecessor.get("state") == "V12_TRIGGERED_ATTEMPT1_FAILED_TERMINAL_RECEIPT_EXACT"
            and predecessor.get("correction_receipt_commit") == PREDECESSOR_COMMIT
            and predecessor.get("workflow_run_id") == 30696298423
            and predecessor.get("workflow_job_id") == 91359681758
            and predecessor.get("workflow_conclusion") == "failure"
            and predecessor.get("artifact_count") == 0
            and predecessor.get("terminal_verifier_sha256") == V12_TERMINAL_VERIFIER_SHA256
            and predecessor.get("failure_evidence_sha256") == V12_EVIDENCE_SHA256,
            "V13 predecessor terminal contract changed",
        )
    recovery = template.get("recovery_basis")
    require(isinstance(recovery, dict), "V13 recovery basis missing")
    ...
    require(
        ...
        and ledger.get("current", {}).get("version") == "v13"
        and ledger.get("current", {}).get("workflow_path") == V13_WORKFLOW_PATH,
        "V13 run-ledger contract changed",
    )
    ...
    require(
        ...
        "V13 cache evidence contract changed",
    )
    require(
        ...
        "V13 authorization boundary changed",
    )
    return errors
...
def _workflow_errors(workflow: str) -> list[str]:
    errors: list[str] = []
    header = f"""name: Admin dependency cache export V13
...
"""
    if not workflow.startswith(preamble):
        errors.append("V13 workflow trigger header changed")
    job_keys = re.findall(
        r"^  ([A-Za-z0-9_-]+):[ \t]*$",
        workflow.split("\njobs:\n", 1)[1]
        if "\njobs:\n" in workflow
    ...
    if job_keys != ["export-cache"]:
        errors.append("V13 workflow job inventory changed")
    if "\n    permissions:" in workflow:
        errors.append("V13 workflow job-level permissions changed")
    required = (
        "permissions:\n  contents: read\n  actions: read",
        "group: admin-dependency-cache-export-v10",
        "cancel-in-progress: false",
        "timeout-minutes: 120",
        "Prove this is the unique V13 workflow run",
        "Snapshot V2 through V13 ledgers before resource creation",
        "Export V13 dependency-only BuildKit local cache",
        "Prove V13 fresh-consumer import and cacheless replay",
        "Remove V13 builders and all transient Docker state",
        "Snapshot V2 through V13 ledgers after cleanup and before upload",
        "Validate final V13 portable bundle after cleanup",
        "Upload one-day V13 public-repository dependency cache artifact",
        "scripts/ci/import_admin_dependency_cache_v13.sh",
        "tools/verify_admin_dependency_cache_bundle_v13.py",
        "tools/verify_admin_dependency_cache_export_plan_v13.py",
        'ledger_contract_copy="${RUNNER_TEMP}/noteai-cache-v13-ledger-contract.json"',
        'cp "${NOTEAI_REQUEST_TEMPLATE}" "${ledger_contract_copy}"',
        "NOTEAI_V13_LEDGER_CONTRACT_PATH: ${{ steps.control.outputs.ledger_contract_copy }}",
        ".pair.classification == \"SAME_DIGEST_CACHED\"",
        ".cache_record.schema_version ==",
        "\"noteai.admin-dependency-cache-record.v13\"",
        "inert_checkpoint_exact_changed_path_count == 14",
        "checkpoint_nonreceipt_immutable_path_count == 10",
        "python3 \"$"
        "{NOTEAI_V12_FAILURE_VERIFIER_PATH}\"",
        "- name: Remove V13 builders and all transient Docker state\n"
        "        id: cleanup\n"
        "        if: always()",
        "- name: Snapshot V2 through V13 ledgers after cleanup and before upload\n"
        "        id: late_ledger\n"
        "        if: ${{ always() && steps.cleanup.outputs.cleanup_passed == 'true' }}",
        "- name: Validate final V13 portable bundle after cleanup\n"
        "        id: bundle\n"
        "        if: ${{ success() && steps.cleanup.outputs.cleanup_passed == 'true' }}",
        "- name: Upload one-day V13 public-repository dependency cache artifact\n"
        "        id: upload\n"
        "        if: ${{ success() && steps.cleanup.outputs.cleanup_passed == 'true' }}",
    )
    for token in required:
        if token not in workflow:
            errors.append(f"V13 workflow contract missing: {token}")
    if workflow.count(
        "NOTEAI_V13_LEDGER_CONTRACT_PATH: "
        "${{ steps.control.outputs.ledger_contract_copy }}"
    ) != 2:
        errors.append("V13 workflow ledger contract handoff count changed")
    if workflow.count(
        'cp "${NOTEAI_REQUEST_TEMPLATE}" "${ledger_contract_copy}"'
    ) != 1:
        errors.append("V13 workflow ledger contract copy count changed")
    for forbidden in (
        "workflow_dispatch:",
        "pull_request:",
        ...
        if forbidden in workflow:
            errors.append(f"V13 workflow forbidden token: {forbidden}")
    cleanup = workflow.find("Remove V13 builders and all transient Docker state")
    late = workflow.find("Snapshot V2 through V13 ledgers after cleanup and before upload")
    final = workflow.find("Validate final V13 portable bundle after cleanup")
    upload = workflow.find("Upload one-day V13 public-repository dependency cache artifact")
    if not 0 <= cleanup < late < final < upload:
        errors.append("V13 cleanup, late-ledger and upload ordering changed")
    return errors
    ...
    for path, expected in FROZEN_WORKTREE_FILES:
        try:
            ...
            if hashlib.sha256(payload).hexdigest() != expected:
                errors.append(f"V13 authority hash drift: {path.relative_to(ROOT)}")
        except (OSError, ValueError) as exc:
            errors.append(f"cannot read V13 authority {path}: {exc}")
    return errors
...
def validate_plan(
    *,
    ...
    errors: list[str] = []
    try:
        workflow_bytes = _read_frozen_bytes(WORKFLOW_PATH, label="V13 workflow")
        template_bytes = _read_frozen_bytes(TEMPLATE_PATH, label="V13 template")
        fixture_bytes = _read_frozen_bytes(SOURCE_FIXTURE_PATH, label="V13 source fixture")
        template = _strict_json_bytes(template_bytes, "V13 template")
        fixture = _strict_json_bytes(fixture_bytes, "V13 source fixture")
    except (OSError, ValueError) as exc:
        return [f"cannot load V13 dependency-cache plan: {exc}"]
    if hashlib.sha256(workflow_bytes).hexdigest() != WORKFLOW_SHA256:
        errors.append("V13 workflow hash drift")
    if hashlib.sha256(template_bytes).hexdigest() != TEMPLATE_SHA256:
        errors.append("V13 template hash drift")
    if hashlib.sha256(fixture_bytes).hexdigest() != SOURCE_FIXTURE_SHA256:
        errors.append("V13 source fixture hash drift")
    if isinstance(template, dict):
        errors.extend(_template_errors(template))
    else:
        errors.append("V13 template root changed")
    ...
    if not (
        ...
        errors.append("V13 source fixture semantics changed")
    ...
    if present and active is not None:
        ...
        if (
            ...
            errors.append("V13 active request differs from reviewed template")
        if verify_git_state and not supplied:
            errors.extend(_validate_active_git(active))
    elif present:
        errors.append("V13 active request is invalid")
    if verify_git_state:
        errors.extend(_validate_v12_terminal_predecessor())
        ...
        if not present:
            try:
                if _true_additions(
                    ...
                    errors.append("inactive V13 request has prior addition history")
            except (OSError, ValueError, subprocess.CalledProcessError) as exc:
                errors.append(f"cannot verify inactive V13 request history: {exc}")
    return errors
...
def classify_plan_state(
    *,
    ...
    if additions:
        return "V13_CONSUMED_OR_INVALID"
    return "PREPARED_V13_NOT_TRIGGERED"
...
def plan_state() -> str:
    ...
    if not present and additions:
        git_errors.append("inactive V13 request has prior history")
    base_errors.extend(_validate_v12_terminal_predecessor())
...
def _expected_ledger_contract() -> dict[str, Any]:
    configured = os.environ.get("NOTEAI_V13_LEDGER_CONTRACT_PATH")
    path = Path(configured) if configured else TEMPLATE_PATH
    payload = _read_frozen_bytes(
        path,
        label="V13 ledger contract template",
        required_mode=0o400 if configured else 0o644,
    )
    _require(
        hashlib.sha256(payload).hexdigest() == TEMPLATE_SHA256,
        "V13 ledger contract template hash drift",
    )
    template = _strict_json_bytes(payload, "V13 ledger contract template")
    _require(isinstance(template, dict), "V13 ledger template root changed")
    contract = template.get("run_ledger_contract")
    _require(isinstance(contract, dict), "V13 ledger contract missing")
    legacy = contract.get("legacy")
    _require(isinstance(legacy, list) and len(legacy) == 11, "V13 legacy ledger changed")
    _require(
        hashlib.sha256(_canonical_json_bytes(legacy)).hexdigest()
        == LEGACY_LEDGER_SHA256,
        "V13 legacy ledger history changed",
    )
    v11 = legacy[9]
    v12 = legacy[10]
...
def _collect_legacy_ledger(
    ...
    *,
    ...
    for expected_version in contract["legacy"]:
        if expected_version.get("version") == "v11":
            continue
    ...
    _require(
        v13_run_ids == {current_run_id},
        "V13 workflow repository inventory changed",
    )
    result: list[dict[str, Any]] = []
    for expected_version in contract["legacy"]:
        if expected_version["version"] == "v11":
            result.append(
                {
                    "version": "v11",
                    "workflow_path": V11_WORKFLOW_PATH,
...
def _collect_current_ledger(
    ...
    *,
    ...
    _require(
        current.get("path") == V13_WORKFLOW_PATH,
    ...
    _require(len(runs) == 1, "V13 workflow run count changed")
    run = runs[0]
    ...
    _require(run.get("path") == V13_WORKFLOW_PATH, "current workflow path changed")
...
def _validate_prior_snapshot(
    ...
    *,
    ...
    _require(current.get("path") == V13_WORKFLOW_PATH, "pre-resource path changed")
    ...
    *,
    ...
    def fetch(path: str) -> dict[str, Any]:
        ...
        for attempt in range(3):
            ...
            _require(remaining > 0, "V13 ledger deadline exceeded")
            try:
...
def _live_ledger_main(arguments: list[str]) -> int:
    ...
    print(f"admin_dependency_cache_v13_live_ledger=PASS phase={phase}")
    return 0
...
def main() -> int:
    try:
        if len(sys.argv) > 1:
            _require(sys.argv[1] == "live-ledger", "V13 command changed")
        ...
        if errors:
            ...
            return 1
        print(
            "admin_dependency_cache_export_plan_v13=PASS "
            f"state={plan_state()}"
        )
        return 0
