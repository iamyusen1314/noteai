#!/usr/bin/env python3
"""Strict v2 raw projection for the Item 26 manual post-action cost stop.
...
CAPTURE_OPERATION_ID = AUTHORITY_GENERATION_ID
...
PROVIDER_CAPTURE_SCHEMA = "noteai.item26.manual-cost-stop-provider-raw.v2"
ACTIONTRAIL_CAPTURE_SCHEMA = (
    "noteai.item26.manual-cost-stop-actiontrail-raw.v2"
)
PROVIDER_PROJECTION_SCHEMA = (
    "noteai.item26.manual-cost-stop-provider-projection.v2"
)
ACTIONTRAIL_PROJECTION_SCHEMA = (
    "noteai.item26.manual-cost-stop-actiontrail-projection.v2"
)
...
EXPECTED_CAPTURE_TRANSPORT = "OFFLINE_OFFICIAL_RESPONSE_IMPORT_V2"
EXPECTED_AUTHORITY_EPOCH = (
    "noteai.item26.manual-cost-stop-authority-generation.v2"
)
AUTHORITY_EPOCH_ID = EXPECTED_AUTHORITY_EPOCH
EXPECTED_ACTIVATION_RECEIPT_SCHEMA = (
...
def _capture_header(
    ...
    *,
    ...
    keys = {
        "schema", "task_id", "operation_id", "historical_operation_id",
        "phase",
        ...
        "authority_epoch", "authority_root_file_sha256",
        "authority_root_git_blob_sha256", "activation_receipt_schema",
    ...
    if (
        ...
        or value["operation_id"] != CAPTURE_OPERATION_ID
        or value["historical_operation_id"] != HISTORICAL_OPERATION_ID
        ...
        or value["authority_epoch"] != EXPECTED_AUTHORITY_EPOCH
        or HEX64.fullmatch(
            value["authority_root_file_sha256"] or ""
        ) is None
        or HEX64.fullmatch(
            value["authority_root_git_blob_sha256"] or ""
        ) is None
        or value["authority_root_file_sha256"]
        != value["authority_root_git_blob_sha256"]
        or value["activation_receipt_schema"]
        != EXPECTED_ACTIVATION_RECEIPT_SCHEMA
...
def project_provider_readback(
    ...
    *,
    ...
    return {
        ...
        "authority_epoch": capture["authority_epoch"],
        "authority_root_file_sha256": capture[
            "authority_root_file_sha256"
        ],
        "authority_root_git_blob_sha256": capture[
            "authority_root_git_blob_sha256"
        ],
        "activation_receipt_schema": capture[
...
__all__ = [
    ...
    "AUTHORITY_EPOCH_ID", "AUTHORITY_GENERATION_ID",
    "CAPTURE_OPERATION_ID", "CAPTURE_SCHEMA", "CLONE_CREATE_LOOKUP_END",
    "CLONE_CREATE_LOOKUP_START",
    "COST_STOP_LOOKUP_END", "COST_STOP_LOOKUP_START",
    "EXPECTED_ACTIONTRAIL_ACTIONS", "EXPECTED_AUTHORITY_EPOCH",
    "EXPECTED_CAPTURE_TRANSPORT", "EXPECTED_ACTIVATION_RECEIPT_SCHEMA",
    "ExtractionError", "LOOKUP_MAX_RESULTS", "MAX_LOOKUP_PAGE_EVENTS",
    "HISTORICAL_OPERATION_ID", "M0_ANCHOR_REVISION", "OPERATION_ID",
    "PROJECTION_SCHEMA",
    ...
    "VerifiedManualProjection", "canonical_bytes",
    "consumed_mutation_identity_set_sha256", "decode_canonical_json",
