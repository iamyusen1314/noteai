#!/usr/bin/env python3
"""Verify the Secret-free terminal receipt for the unique V8 cache run."""
...
EVIDENCE_FILE_SHA256 = (
    "e4b69bab7c607ec71f44ec390be9acb6"
    "93de53d05c33a62cb21b6691255e206b"
)
EXPECTED_SEMANTIC_SHA256 = (
    "78bdb3bc843cec7c9eb192dde896cf792"
    "96edab2d46561a17ac4065324489645"
)

INERT_CHECKPOINT_COMMIT = "3381c2fdbcb32480bb364a400250f8d24de74c84"
DIRECT_PARENT_COMMIT = "cf253f9b42b096f45405ac55fe546c281a2a748d"
CONTROL_COMMIT = "354bec3b2d36bfaabc5c3307d49f5dc66255cbbf"
REQUEST_PATH = Path(
    ".github/release-requests/"
    "admin-5335bda-dependency-cache-v8.json"
)
REQUEST_SHA256 = (
    "5283d1044e3fb783e4951019f2c137a2"
    "3530fb6c09fc290718c305ee3e449cd5"
)
TEMPLATE_PATH = Path(
...
FROZEN_PATHS = {
    Path(".github/workflows/admin-dependency-cache-export-v8.yml"): (
        "370e17b59ce457efda8b5a6cd23d40937"
        "379d23d1a2aeb3f36ac7b88f72333b3"
    ),
    TEMPLATE_PATH: (
        "ba2f705eff90285e377e8d683a059f7f"
        "23aef088dc7df2498d167909fff48e7c"
    ),
    Path("tools/verify_admin_dependency_cache_export_plan_v8.py"): (
        "69fde0852832a2b6cdfc4e3542b835a7"
        "4c3c939377220deb3009430292ef5915"
    ),
    ...
    Path(
        ...
        "b5f5e806b7e822b536a850eca8861bb7"
        "b6f8934832d7bdb4e898e698d7f20eeb"
    ),
    Path(
        "tests/fixtures/"
        "admin_dependency_cache_buildkit_v0.31.2_"
        ...
        "fb5c944a0006cb8e04f32e3ac98e949e"
        "27036b10077e4ce10bc0a6c8b4fb5de1"
    ),
    Path(
        "deploy/production/evidence/"
        "admin-dependency-cache-v7-attempt1-failed-20260731.json"
    ): (
        "91950799263f19b8422534b685cd1e46"
        "856e9d03137976ccec08ce1d644e3ed4"
    ),
    Path("tools/verify_admin_dependency_cache_v7_failure_evidence.py"): (
        "c1ae9caa594c48ff1a6d46f78d4aef84"
        "cb3a33e410ea9c2ae6bb2c17bf6ae476"
    ),
...
def verify_frozen_git() -> list[str]:
    ...
    try:
        ...
        if parents != [CONTROL_COMMIT, DIRECT_PARENT_COMMIT]:
            errors.append("V8 controller parent binding changed")
        ...
        if receipt_parents != [DIRECT_PARENT_COMMIT, INERT_CHECKPOINT_COMMIT]:
            errors.append("V8 inert remote receipt parent binding changed")
        ...
        if changed != f"A\t{request_name}":
            errors.append("V8 controller is no longer request-only")
        ...
        if additions != [CONTROL_COMMIT]:
            errors.append("V8 request addition history changed")

        control_request = _git_bytes("show", f"{CONTROL_COMMIT}:{request_name}")
        if sha256_bytes(control_request) != REQUEST_SHA256:
            errors.append("V8 control request bytes changed")
        if _git_bytes("show", f"HEAD:{request_name}") != control_request:
            errors.append("V8 request is not retained byte-exact at HEAD")

        tree_fields = _git_text("ls-tree", "HEAD", "--", request_name).split()
        if tree_fields[:2] != ["100644", "blob"]:
            errors.append("V8 request retained mode changed")
        ...
        for commit, label in (
            ...
            for frozen_path, expected_sha256 in FROZEN_PATHS.items():
                ...
                if sha256_bytes(historical) != expected_sha256:
                    errors.append(
                        f"V8 {label} tree file hash changed: "
                        f"{frozen_path.as_posix()}"
                    )
                historical_mode = _git_text(
                    "ls-tree", commit, "--", frozen_path.as_posix()
                ).split()
                if historical_mode[:2] != ["100644", "blob"]:
                    errors.append(
                        f"V8 {label} tree file mode changed: "
                        f"{frozen_path.as_posix()}"
                    )

        for frozen_path, expected_sha256 in FROZEN_PATHS.items():
            if sha256_bytes((ROOT / frozen_path).read_bytes()) != expected_sha256:
                errors.append(
                    f"frozen V8 file hash changed: {frozen_path.as_posix()}"
                )
            head_bytes = _git_bytes(
                "show", f"HEAD:{frozen_path.as_posix()}"
            )
            if sha256_bytes(head_bytes) != expected_sha256:
                errors.append(
                    "V8 HEAD frozen file hash changed: "
                    f"{frozen_path.as_posix()}"
                )
            head_mode = _git_text(
                "ls-tree", "HEAD", "--", frozen_path.as_posix()
            ).split()
            if head_mode[:2] != ["100644", "blob"]:
                errors.append(
                    f"frozen V8 file mode changed: {frozen_path.as_posix()}"
                )
        ...
        if template_bytes.count(b"__DIRECT_PARENT_COMMIT__") != 1:
            errors.append("V8 frozen template placeholder count changed")
        ...
        if control_request != expected_request:
            errors.append("V8 control request differs from its frozen template")
        request = json.loads(
        ...
        if request.get("plan_checkpoint_commit") != DIRECT_PARENT_COMMIT:
            errors.append("V8 request embedded parent changed")
    except (
        OSError,
        UnicodeDecodeError,
        ...
        errors.append(f"cannot verify frozen V8 Git state: {exc}")
    return errors
...
def verify(
    ...
    *,
    ...
    if semantic_sha256(payload) != EXPECTED_SEMANTIC_SHA256:
        errors.append("V8 failure evidence semantics changed")
    try:
        if sha256_bytes(EVIDENCE_PATH.read_bytes()) != EVIDENCE_FILE_SHA256:
            errors.append("V8 failure evidence file bytes changed")
    except OSError as exc:
        errors.append(f"cannot load frozen V8 evidence closure: {exc}")
    ...
    for name in ("producer_diagnostic", "import_diagnostic"):
        ...
        if (
            ...
            errors.append(f"V8 {name.replace('_', ' ')} hash does not derive")
    ...
    if (
        ...
        errors.append("V8 cleanup compact payload hash does not derive")
    return errors
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("admin_dependency_cache_v8_failure_evidence=VERIFIED")
    return 0
