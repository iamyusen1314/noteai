#!/usr/bin/env python3
"""Pure generation-v2 builder for Item 26 manual cost-stop artifacts."""
...
from verify_item26_manual_cost_stop_evidence_v2 import (
    AUTHORITY_EPOCH_ID,
...
def build_receipt(
    *,
    ...
    if (
        ...
        or provider.get("authority_epoch") != AUTHORITY_EPOCH_ID
        or trail.get("authority_epoch") != AUTHORITY_EPOCH_ID
        or provider.get("authority_root_file_sha256")
        != trail.get("authority_root_file_sha256")
        or provider.get("authority_root_git_blob_sha256")
        != trail.get("authority_root_git_blob_sha256")
        or provider.get("activation_receipt_schema")
        != trail.get("activation_receipt_schema")
    ):
        raise ValueError("manual generation-v2 authority binding mismatch")
    receipt: dict[str, Any] = {
        ...
        "raw_closure": {
            "authority_epoch_id": AUTHORITY_EPOCH_ID,
            "authority_root_file_sha256": provider[
                "authority_root_file_sha256"
            ],
            "authority_root_git_blob_sha256": provider[
                "authority_root_git_blob_sha256"
            ],
            "activation_receipt_schema": provider[
