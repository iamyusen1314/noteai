#!/usr/bin/env python3
"""Verify the append-only V16 Admin dependency-cache successor plan."""
...
WORKFLOW_PATH = ROOT / ".github" / "workflows" / "admin-dependency-cache-export-v16.yml"
CI_WORKFLOW_PATH = ROOT / ".github" / "workflows" / "ci.yml"
TEMPLATE_PATH = ROOT / "deploy" / "production" / "plans" / "admin-dependency-cache-export-request-v16.json"
ACTIVE_REQUEST_PATH = ROOT / ".github" / "release-requests" / "admin-5335bda-dependency-cache-v16.json"
EXPORT_HELPER_PATH = ROOT / "scripts" / "ci" / "export_admin_dependency_cache_v16.sh"
IMPORT_HELPER_PATH = ROOT / "scripts" / "ci" / "import_admin_dependency_cache_v16.sh"
SOURCE_FIXTURE_PATH = ROOT / "tests" / "fixtures" / (
    "admin_dependency_cache_buildkit_v0.31.2_"
    "git_main_context_identity_projection.json"
)
BUNDLE_VERIFIER_PATH = ROOT / "tools" / "verify_admin_dependency_cache_bundle_v16.py"
BUNDLE_TEST_PATH = ROOT / "tests" / "test_admin_dependency_cache_bundle_verifier_v16.py"
PLAN_TEST_PATH = ROOT / "tests" / "test_admin_dependency_cache_export_plan_v16.py"
PLAN_VERIFIER_PATH = Path(__file__).resolve()
V15_TERMINAL_VERIFIER_PATH = ROOT / "tools" / "verify_admin_dependency_cache_v15_failure_evidence.py"
V15_TERMINAL_TEST_PATH = ROOT / "tests" / "test_admin_dependency_cache_v15_failure_evidence.py"
...
EXPORT_HELPER_SHA256 = "84100721415675268a3617398b1b58d8751cede9adfa3acd2351811538090ed0"
IMPORT_HELPER_SHA256 = "41150005187726281752ed5b39e03ceba7618d77319c9a92d134a3a106546ba6"
...
V15_WORKFLOW_SHA256 = "33a57116c7462bb4f73f48f0d94f41158b8bf676886c19518610d172fc6ba457"
...
V15_TERMINAL_CHECKPOINT = "46595fef4916ca2881dd38c0a14ee630bba79c32"
V15_TERMINAL_RECEIPT = "a94ee2b2feb81eafbcb523be2c95da4ee952cbc4"
PREDECESSOR_COMMIT = V15_TERMINAL_RECEIPT
...
LEDGER_SCHEMA = "noteai.admin-dependency-cache-ledger.v16"
...
LEGACY_LEDGER_SHA256 = "f04e248ac1086c847f2f0975cb4029184ae41932b1beaa19fb4d01db51fef50f"
...
def _load_v15_terminal_verifier():
    ...
    module_name = "_noteai_v15_terminal_for_v16"
...
def _validate_v15_predecessor(*, root: Path = ROOT) -> list[str]:
    errors: list[str] = []
    try:
        if _commit_parents(root, V15_TERMINAL_CHECKPOINT) != [V15_ACTIVATION]:
            errors.append("V15 terminal checkpoint parent changed")
        if _commit_parents(root, V15_TERMINAL_RECEIPT) != [V15_TERMINAL_CHECKPOINT]:
            errors.append("V15 terminal receipt parent changed")
        if not _on_first_parent_chain(
            root,
            V15_TERMINAL_RECEIPT,
            head=_canonical_branch_head(root),
        ):
            errors.append("V15 terminal receipt is not on first-parent chain")
        if root.resolve() == ROOT.resolve():
            evidence = _read_frozen_bytes(
                V15_EVIDENCE_PATH,
                label="V15 terminal evidence",
                required_mode=0o644,
            )
            if hashlib.sha256(evidence).hexdigest() != V15_EVIDENCE_SHA256:
                errors.append("V15 terminal evidence hash drift")
            else:
                ...
                if semantic_errors:
                    errors.append(
                        "V15 terminal semantic validation failed: "
                        + "; ".join(semantic_errors[:5])
                    )
                # The frozen V15 verifier intentionally treats CI and the
                # other V15 control files as immutable through its own
                # terminal receipt. A versioned successor must update CI, so
                # evaluate that historical contract at the fixed receipt,
                # never against the mutable V16 branch head. The module is a
                # private hash-pinned copy; restore its resolver immediately.
                original_canonical_head = terminal._canonical_head
                try:
                    terminal._canonical_head = lambda: V15_TERMINAL_RECEIPT
                    historical_git_errors = terminal.verify_frozen_git()
                finally:
                    terminal._canonical_head = original_canonical_head
                if historical_git_errors:
                    errors.append(
                        "V15 terminal historical Git validation failed: "
                        + "; ".join(historical_git_errors[:5])
                    )
                if terminal.terminal_checkpoint() != V15_TERMINAL_CHECKPOINT:
                    errors.append("V15 terminal checkpoint changed")
                elif terminal.terminal_receipt(V15_TERMINAL_CHECKPOINT) != V15_TERMINAL_RECEIPT:
                    errors.append("V15 terminal receipt changed")
    except (
        OSError,
        UnicodeDecodeError,
        ...
        errors.append(f"cannot verify V15 terminal predecessor: {exc}")
    return errors
...
def _authority_anchor(
    *,
    ...
    if len(present) != len(SAME_ANCHOR_FILES):
        return None, ["V16 frozen authorities are only partially present"]
    anchors: list[str] = []
    for path in SAME_ANCHOR_FILES:
        additions = _true_additions(root=root, path=path)
        if len(additions) != 1:
            errors.append(f"V16 frozen addition history is not unique: {path}")
        else:
            anchors.append(additions[0])
    if errors:
        return None, errors
    if len(set(anchors)) != 1:
        return None, ["V16 frozen authorities have different add anchors"]
    return anchors[0], []
...
def _validate_same_anchor(*, root: Path = ROOT) -> list[str]:
    ...
    try:
        ...
        if anchor is None:
            errors.append("V16 inert checkpoint is missing")
            return errors
        ...
        if _changed_paths(root, PREDECESSOR_COMMIT, anchor) != set(
            ...
            errors.append("V16 inert checkpoint is not the exact 16-file delta")
        if not _on_first_parent_chain(root, anchor, head=branch_head):
            errors.append("V16 inert checkpoint is not on first-parent chain")
        for relative in INERT_CHECKPOINT_FILES:
            ...
            if len(entry) < 3 or entry[0:2] != ["100644", "blob"]:
                errors.append(f"V16 inert checkpoint mode changed: {relative}")
        if _lineage_path_changes(
            ...
            errors.append("V16 immutable checkpoint authority changed")
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        errors.append(f"cannot verify V16 inert checkpoint: {exc}")
    return errors
    ...
    *,
    ...
    try:
        ...
        if not changes:
            return None, []
        if len(changes) != 1:
            return None, ["V16 inert receipt history is not unique"]
        receipt = changes[0]
        if _commit_parents(root, receipt) != [anchor]:
            errors.append("V16 inert receipt is not the direct child of anchor")
        elif _changed_paths(root, anchor, receipt) != set(INERT_RECEIPT_FILES):
            errors.append("V16 inert receipt is not the exact 4-file delta")
        for relative in INERT_RECEIPT_FILES:
            entry = _tree_entry(root, receipt, relative).split()
            if len(entry) < 3 or entry[0:2] != ["100644", "blob"]:
                errors.append(f"V16 inert receipt mode changed: {relative}")
        if not _on_first_parent_chain(root, receipt, head=branch_head):
            errors.append("V16 inert receipt is not on first-parent chain")
        return receipt, errors
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        return None, [f"cannot verify V16 inert receipt: {exc}"]
    ...
    *,
    ...
    try:
        ...
        payload = _read_frozen_bytes(
            path,
            label="V16 active request",
            required_mode=0o644,
        )
        value = _strict_json_bytes(payload, "V16 active request")
        if not isinstance(value, dict):
            return True, None, ["V16 active request root changed"]
        return True, value, []
    except (OSError, ValueError) as exc:
        return True, None, [f"cannot load V16 active request: {exc}"]
...
def _validate_active_git(
    ...
    *,
    ...
    try:
        ...
        if len(additions) != 1:
            return ["V16 active request addition history is not unique"]
        activation = additions[0]
        if not _on_first_parent_chain(root, activation, head=branch_head):
            errors.append("V16 activation is not on first-parent chain")
        parents = _commit_parents(root, activation)
        if len(parents) != 1:
            return ["V16 activation is not single-parent"]
        parent = parents[0]
        receipt, receipt_errors = _validate_inert_receipt(root=root)
        errors.extend(receipt_errors)
        if receipt is None or parent != receipt:
            errors.append("V16 activation parent is not the exact receipt")
        if active.get("plan_checkpoint_commit") != parent:
            errors.append("V16 active request does not bind direct parent")
        if _changed_paths(root, parent, activation) != {relative}:
            errors.append("V16 activation changes more than its request")
        status = _git_at(
            root,
            "diff-tree",
            "--no-commit-id",
        ...
        if status != f"A\t{relative.as_posix()}":
            errors.append("V16 request was not a unique addition")
        entry = _tree_entry(root, activation, relative).split()
        if len(entry) < 3 or entry[0:2] != ["100644", "blob"]:
            errors.append("V16 request activation mode changed")
        template = _read_frozen_bytes(TEMPLATE_PATH, label="V16 template")
        ...
        if payload != expected:
            errors.append("V16 activation request differs from template")
        if _lineage_path_changes(
            root=root,
            anchor=activation,
            paths=(relative,),
            ...
            errors.append("V16 active request changed after activation")
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        errors.append(f"cannot verify V16 active request Git state: {exc}")
    return errors
    ...
    *,
    ...
    try:
        ...
        return [] if branch_head == expected else [
            "V16 canonical branch HEAD is not the latest exact stage"
        ]
    except (OSError, ValueError, subprocess.CalledProcessError) as exc:
        return [f"cannot verify V16 latest exact stage: {exc}"]


def _template_errors(template: dict[str, Any]) -> list[str]:
    """Validate the V16 request as a closed, one-shot authorization contract."""
    errors: list[str] = []
    ...
    require(
        ...
        "V16 request root key set changed",
    )
    require(template.get("schema_version") == 16, "V16 schema changed")
    require(
        template.get("task") == "PROD-FIRST-LAUNCH-ADMIN-INTERNAL-001",
        "V16 task changed",
    )
    require(template.get("release_commit") == RELEASE_COMMIT, "V16 release changed")
    require(
        template.get("plan_checkpoint_commit") == "__DIRECT_PARENT_COMMIT__",
        "V16 parent placeholder changed",
    )
    require(
        template.get("trigger_mode") == "one_shot_added_request_v16",
        "V16 trigger mode changed",
    )
    require(
        template.get("predecessor")
        == {
            "state": "V15_TRIGGERED_ATTEMPT1_FAILED_TERMINAL_RECEIPT_EXACT",
            "activation_commit": V15_ACTIVATION,
            "terminal_checkpoint_commit": V15_TERMINAL_CHECKPOINT,
            "terminal_receipt_commit": V15_TERMINAL_RECEIPT,
            "workflow_path": V15_WORKFLOW_PATH,
            "workflow_sha256": V15_WORKFLOW_SHA256,
            "template_sha256": V15_TEMPLATE_SHA256,
            "plan_verifier_sha256": V15_PLAN_VERIFIER_SHA256,
            "plan_test_sha256": V15_PLAN_TEST_SHA256,
            "failure_evidence_sha256": V15_EVIDENCE_SHA256,
            "active_request_path": (
            ...
            "supersession_reason": (
                "RUNTIME_PIP_DIGEST_DRIFT_WITH_LOCAL_MAIN_CONTEXT_"
                "IDENTITY_NOT_RETAINED"
            ),
        },
        "V16 V15 predecessor contract changed",
    )

    recovery = template.get("recovery_basis")
    require(isinstance(recovery, dict), "V16 recovery basis missing")
    if isinstance(recovery, dict):
        require(
            ...
            and recovery.get("v16_export_helper_sha256") == EXPORT_HELPER_SHA256
            and recovery.get("v16_import_helper_sha256") == IMPORT_HELPER_SHA256
            and recovery.get("v16_bundle_verifier_sha256")
            ...
            "V16 Git-context recovery contract changed",
        )
    ...
    require(
        ...
        "V16 frozen V13 chain changed",
    )
    control = template.get("control_plane")
    require(
        isinstance(control, dict)
        and control.get("inert_checkpoint_exact_changed_path_count") == 16
        and control.get("inert_receipt_exact_changed_path_count") == 4
        and control.get("activation_exact_changed_path_count") == 1
        and control.get("checkpoint_nonreceipt_immutable_path_count") == 12
        ...
        and control.get("v16_current_attempt1_artifact0_required") is True,
        "V16 control-plane contract changed",
    )

    ledger = template.get("run_ledger_contract")
    legacy = ledger.get("legacy") if isinstance(ledger, dict) else None
    v11 = legacy[9] if isinstance(legacy, list) and len(legacy) == 14 else None
    v12 = legacy[10] if isinstance(legacy, list) and len(legacy) == 14 else None
    v13 = legacy[11] if isinstance(legacy, list) and len(legacy) == 14 else None
    v14 = legacy[12] if isinstance(legacy, list) and len(legacy) == 14 else None
    v15 = legacy[13] if isinstance(legacy, list) and len(legacy) == 14 else None
    require(
        ...
        "V16 run-ledger contract changed",
    )
    ...
    require(
        ...
        "V16 build-evidence contract changed",
    )
    require(
        ...
        "V16 authorization boundary changed",
    )
    return errors


def _workflow_errors(workflow: str) -> list[str]:
    """Validate the inert V16 controller before any external resource exists."""
    errors: list[str] = []
    header = f"""name: Admin dependency cache export V16
...
"""
        ...
        errors.append("V16 workflow trigger or permission header changed")
    ...
    if job_keys != ["export-cache"]:
        errors.append("V16 workflow job inventory changed")
    required = (
        "group: admin-dependency-cache-export-v10",
        "cancel-in-progress: false",
        "runs-on: ubuntu-24.04",
        "timeout-minutes: 120",
        "NOTEAI_V15_ACTIVATION: " + V15_ACTIVATION,
        "NOTEAI_V15_TERMINAL_CHECKPOINT: " + V15_TERMINAL_CHECKPOINT,
        "NOTEAI_V15_TERMINAL_RECEIPT: " + V15_TERMINAL_RECEIPT,
        "NOTEAI_EXPORT_HELPER_PATH: scripts/ci/export_admin_dependency_cache_v16.sh",
        "NOTEAI_IMPORT_HELPER_PATH: scripts/ci/import_admin_dependency_cache_v16.sh",
        "NOTEAI_V16_CACHE_IDENTITY_SOURCE_PROJECTION_PATH: tests/fixtures/admin_dependency_cache_buildkit_v0.31.2_git_main_context_identity_projection.json",
        "NOTEAI_V13_BUNDLE_VERIFIER_PATH: tools/verify_admin_dependency_cache_bundle_v13.py",
        "NOTEAI_BUNDLE_VERIFIER_PATH: tools/verify_admin_dependency_cache_bundle_v16.py",
        "tools/verify_admin_dependency_cache_export_plan_v16.py",
        "Prove this is the unique V16 workflow run for the branch lifecycle",
        "Resolve exact V16 recovery request",
        "Snapshot V2 through V16 ledgers before resource creation",
        "Export V16 dependency-only BuildKit local cache",
        "Prove V16 import and external-cache-removed same-builder replay",
        "Remove V16 builders and all transient Docker state",
        "Snapshot V2 through V16 ledgers after cleanup and before upload",
        "Validate final V16 portable bundle after cleanup",
        "Upload one-day V16 public-repository dependency cache artifact",
        "V15_TRIGGERED_ATTEMPT1_FAILED_TERMINAL_RECEIPT_EXACT",
        "inert_checkpoint_exact_changed_path_count == 16",
        "inert_receipt_exact_changed_path_count == 4",
        "activation_exact_changed_path_count == 1",
        "checkpoint_nonreceipt_immutable_path_count == 12",
        "(.run_ledger_contract.legacy | length) == 14",
        ".run_ledger_contract.legacy[13].version == \"v15\"",
        ".run_ledger_contract.legacy[13].runs[0].id == 30724578319",
        '"SAME_SOURCE_COPY_PIP_DIGESTS_CACHED"',
        ".external_cache_removed_same_consumer_builder_replay",
        ".true_empty_cache_replay_claimed == false",
        'ledger_contract_copy="${RUNNER_TEMP}/noteai-cache-v16-ledger-contract.json"',
        'cp "${NOTEAI_REQUEST_TEMPLATE}" "${ledger_contract_copy}"',
        "NOTEAI_V16_LEDGER_CONTRACT_PATH: ${{ steps.control.outputs.ledger_contract_copy }}",
        'BUILDKIT_NO_CLIENT_TOKEN: "1"',
        "NOTEAI_V13_BUNDLE_VERIFIER_PATH: ${{ steps.control.outputs.v13_verifier_copy }}",
    )
    for token in required:
        if token not in workflow:
            errors.append(f"V16 workflow contract missing: {token}")
    if workflow.count(
        "NOTEAI_V16_LEDGER_CONTRACT_PATH: "
        "${{ steps.control.outputs.ledger_contract_copy }}"
    ) != 2:
        errors.append("V16 workflow ledger handoff count changed")
    if workflow.count(
        'cp "${NOTEAI_REQUEST_TEMPLATE}" "${ledger_contract_copy}"'
    ) != 1:
        errors.append("V16 workflow ledger contract copy count changed")
    for forbidden in (
        "workflow_dispatch:",
        "pull_request:",
        ...
        if forbidden in workflow:
            errors.append(f"V16 workflow forbidden token: {forbidden}")
    cleanup = workflow.find("Remove V16 builders and all transient Docker state")
    late = workflow.find(
        "Snapshot V2 through V16 ledgers after cleanup and before upload"
    )
    final = workflow.find("Validate final V16 portable bundle after cleanup")
    upload = workflow.find(
        "Upload one-day V16 public-repository dependency cache artifact"
    )
    if not 0 <= cleanup < late < final < upload:
        errors.append("V16 cleanup, late-ledger, final and upload ordering changed")
    return errors
    ...
    for path, expected in FROZEN_WORKTREE_FILES:
        try:
            ...
            if hashlib.sha256(payload).hexdigest() != expected:
                errors.append(f"V16 authority hash drift: {path.relative_to(ROOT)}")
        except (OSError, ValueError) as exc:
            errors.append(f"cannot read V16 authority {path}: {exc}")
    return errors
...
def classify_plan_state(
    *,
    ...
    if additions:
        return "V16_CONSUMED_OR_INVALID"
    return "PREPARED_V16_NOT_TRIGGERED"
    ...
    *,
    ...
    try:
        ...
        fixture_bytes = _read_frozen_bytes(
            SOURCE_FIXTURE_PATH,
            label="V16 source fixture",
            required_mode=0o644,
        )
        template = _strict_json_bytes(template_bytes, "V16 template")
        fixture = _strict_json_bytes(fixture_bytes, "V16 source fixture")
    except (OSError, ValueError) as exc:
        return [f"cannot load V16 dependency-cache plan: {exc}"]
    if hashlib.sha256(workflow_bytes).hexdigest() != WORKFLOW_SHA256:
        errors.append("V16 workflow hash drift")
    if hashlib.sha256(template_bytes).hexdigest() != TEMPLATE_SHA256:
        errors.append("V16 template hash drift")
    if hashlib.sha256(fixture_bytes).hexdigest() != SOURCE_FIXTURE_SHA256:
        errors.append("V16 source fixture hash drift")
    if isinstance(template, dict):
        errors.extend(_template_errors(template))
    else:
        errors.append("V16 template root changed")
    try:
        workflow = workflow_bytes.decode("utf-8")
    except UnicodeDecodeError as exc:
        errors.append(f"V16 workflow is not UTF-8: {exc}")
    else:
        errors.extend(_workflow_errors(workflow))
    if not (
        isinstance(fixture, dict)
        and fixture.get("schema_version")
        == "noteai.buildkit-git-main-context-identity.v16"
        and fixture.get("classification")
        == "SOURCE_PROVEN_GIT_MAIN_CONTEXT_IDENTITY_CHAIN_NOT_RUNTIME_EVIDENCE"
        and fixture.get("fixed_context", {}).get("release_commit")
        == RELEASE_COMMIT
        and fixture.get("fixed_context", {}).get("buildx_send_git_query_as_input")
        is False
        and fixture.get("fixed_context", {}).get("dockerfile_transport")
        == "stdin"
        and fixture.get("requirements_copy", {}).get("dockerfile_line") == 75
        and len(fixture.get("requirements_copy", {}).get("actions", [])) == 2
        and fixture.get("runtime_pip", {}).get("dockerfile_line") == 76
        and fixture.get("runtime_pip", {}).get("accepted_pair_classification")
        == "SAME_SOURCE_COPY_PIP_DIGESTS_CACHED"
        ...
        is False
        and fixture.get("claims", {}).get(
            "source_review_is_runtime_cache_portability_evidence"
        )
        is False
        ...
        errors.append("V16 source fixture semantics changed")
    ...
    if present and active is not None:
        ...
        if COMMIT_RE.fullmatch(parent) is None or active_bytes != expected:
            errors.append("V16 active request differs from reviewed template")
        if verify_git_state and not supplied:
            errors.extend(_validate_active_git(active))
    elif present:
        errors.append("V16 active request is invalid")
...
def _activation_plan_state() -> str:
    ...
    if not present and additions:
        git_errors.append("inactive V16 request has prior history")
    base_errors.extend(_validate_v15_predecessor())
...
def _expected_ledger_contract() -> dict[str, Any]:
    configured = os.environ.get("NOTEAI_V16_LEDGER_CONTRACT_PATH")
    path = Path(configured) if configured else TEMPLATE_PATH
    payload = _read_frozen_bytes(
        path,
        label="V16 ledger contract template",
        required_mode=0o400 if configured else 0o644,
    )
    _require(
        hashlib.sha256(payload).hexdigest() == TEMPLATE_SHA256,
        "V16 ledger contract template hash drift",
    )
    template = _strict_json_bytes(payload, "V16 ledger contract template")
    _require(isinstance(template, dict), "V16 ledger template root changed")
    contract = template.get("run_ledger_contract")
    _require(isinstance(contract, dict), "V16 ledger contract missing")
    legacy = contract.get("legacy")
    _require(isinstance(legacy, list) and len(legacy) == 14, "V16 legacy ledger changed")
    _require(
        hashlib.sha256(_canonical_json_bytes(legacy)).hexdigest()
        == LEGACY_LEDGER_SHA256,
        "V16 legacy ledger history changed",
    )
    v11 = legacy[9]
    v12 = legacy[10]
    v13 = legacy[11]
...
def _collect_current_ledger(
    ...
    *,
    ...
    _require(
        current.get("path") == V16_WORKFLOW_PATH,
    ...
    _require(len(runs) == 1, "V16 workflow run count changed")
    run = runs[0]
    ...
    _require(run.get("path") == V16_WORKFLOW_PATH, "current workflow path changed")
...
def _validate_prior_snapshot(
    ...
    *,
    ...
    _require(current.get("path") == V16_WORKFLOW_PATH, "pre-resource path changed")
    ...
    *,
    ...
    def fetch(path: str) -> dict[str, Any]:
        ...
        for attempt in range(3):
            ...
            _require(remaining > 0, "V16 ledger deadline exceeded")
            try:
...
def _live_ledger_main(arguments: list[str]) -> int:
    ...
    print(f"admin_dependency_cache_v16_live_ledger=PASS phase={phase}")
    return 0
...
def main() -> int:
    try:
        if len(sys.argv) > 1:
            _require(sys.argv[1] == "live-ledger", "V16 command changed")
