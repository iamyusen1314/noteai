#!/usr/bin/env python3
"""Verify the Secret-free terminal receipt for cache-export V7 attempt 1."""
...
EXPECTED_SEMANTIC_SHA256 = (
    "6ea653978234820dad155784d22887dfd6"
    "1352e28f97571e3a18423b3893ad9a"
)
INERT_CHECKPOINT_COMMIT = "27dcccf1736c40a832a3ca4875aa9ff7bdbe72d5"
DIRECT_PARENT_COMMIT = "71692d5f25f6a2d3f248cc62a568f4a2bd5af2cd"
CONTROL_COMMIT = "4494f50bf9a18422cef6252792bb9c6bbeaf1135"
REQUEST_PATH = Path(
    ".github/release-requests/admin-5335bda-dependency-cache-v7.json"
)
REQUEST_SHA256 = (
    "161dc1cfbc8683cbc18816baffd01edfa"
    "50c5d4fca8edb2e85054fb79eeb37dd"
)
TEMPLATE_PATH = Path(
    "deploy/production/plans/admin-dependency-cache-export-request-v7.json"
)
FROZEN_PATHS = {
    Path(".github/workflows/admin-dependency-cache-export-v7.yml"): (
        "3b4ec5ed8841253afe62ff54b7de7dfd"
        "d523d48d9dd3ce5837394fb4c5e9c427"
    ),
    TEMPLATE_PATH: (
        "e690b968c935ffa721535a0282b3d108f"
        "e6ce651431622f262894225cd0a29e7"
    ),
    Path("tools/verify_admin_dependency_cache_export_plan_v7.py"): (
        "bed5ee29dd367caa4faafb7c711fd4d9"
        "7d740319622aa459fcc5000a866ebc6b"
    ),
...
def verify_frozen_git() -> list[str]:
    ...
    try:
        ...
        if parents != [CONTROL_COMMIT, DIRECT_PARENT_COMMIT]:
            errors.append("V7 controller parent binding changed")
        ...
        if receipt_parents != [
            ...
            errors.append("V7 inert remote receipt parent binding changed")
        ...
        if changed != f"A\t{request_name}":
            errors.append("V7 controller is no longer request-only")
        ...
        if additions != [CONTROL_COMMIT]:
            errors.append("V7 request addition history changed")
        ...
        if sha256_bytes(control_request) != REQUEST_SHA256:
            errors.append("V7 control request bytes changed")
        if _git_bytes("show", f"HEAD:{request_name}") != control_request:
            errors.append("V7 request is not retained byte-exact at HEAD")
        ...
        if tree_fields[:2] != ["100644", "blob"]:
            errors.append("V7 request retained mode changed")
        ...
        for frozen_path, expected_sha256 in FROZEN_PATHS.items():
            if (
                ...
                errors.append(
                    f"frozen V7 file hash changed: {frozen_path.as_posix()}"
                )
            head_mode = _git_text(
                "ls-tree",
                "HEAD",
            ...
            if head_mode[:2] != ["100644", "blob"]:
                errors.append(
                    f"frozen V7 file mode changed: {frozen_path.as_posix()}"
                )
        ...
        if template_bytes.count(b"__DIRECT_PARENT_COMMIT__") != 1:
            errors.append("V7 frozen template placeholder count changed")
        ...
        if control_request != expected_request:
            errors.append("V7 control request differs from its frozen template")
        request = json.loads(
        ...
        if request.get("plan_checkpoint_commit") != DIRECT_PARENT_COMMIT:
            errors.append("V7 request embedded parent changed")
    except (
        OSError,
        UnicodeDecodeError,
        ...
        errors.append(f"cannot verify frozen V7 Git state: {exc}")
    return errors
...
def verify(
    ...
    *,
    ...
    if semantic_sha256(payload) != EXPECTED_SEMANTIC_SHA256:
        errors.append("V7 failure evidence semantics changed")
    try:
        if sha256_bytes(EVIDENCE_PATH.read_bytes()) != EVIDENCE_FILE_SHA256:
            errors.append("V7 failure evidence file bytes changed")
    except OSError as exc:
        errors.append(f"cannot load frozen V7 evidence closure: {exc}")
    ...
    if (
        ...
        errors.append("V7 diagnostic hash does not derive")
    ...
    if (
        ...
        errors.append("V7 cleanup compact payload hash does not derive")
    return errors
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("admin_dependency_cache_v7_failure_evidence=VERIFIED")
    return 0
